...
// なぜかこっちにもバージョン振ってます。
#define DTB_VERSION 1060
#define STB_VERSION 1060
...
public:
  CCharacterInfo(LPCTSTR CharName = "名無しさん",
...
#define SET_CHARACTERINFO(cinfo, Type)                                                                       \
  extern "C" BOOL CharacterInfo(LPVOID info) { return cinfo.DllCharacterInfo(info); }                        \
...
protected:
  ...
  static void LoadCellData(char *pathname,
                           GCD_CELL2_070 *pCells,
                           GCD_RECT *pRects,
                           GCD_HANTEI *pHanteis); //セルデータ読み込み関数。ver0.90以降では非推奨
  ...
  static void CellDraw(MYSURFACE **pBmps,
                       GCD_CELL2 *cdat,
                       GCD_RECT *rdat, //セル描画関数
                       DWORD cnum,
                       int x,
                       int y,
                       float z,
  ...
  static void CkBlt(MYSURFACE *pBmp,
                    int x1,
                    int y1,
                    RECT bltrect,
                    double magx,
  ...
  static void
  SetParentMatrix(LPD3DXMATRIX pMatrix, BOOL erase, LPD3DXMATRIX OldMatrix); //「親」の変換行列設定
  static DWORD
  CreateCellData(char *filename,
                 GCD_CELL2 **cdat,
                 GCD_RECT **rdat,
                 GCD_HANTEI **hdat); //セルデータ読み込み。ver0.90以降ではLoadCellDatからこちらに変更するべき
  static void DestroyCellData(GCD_CELL2 **cdat,
                              GCD_RECT **rdat,
  ...
  static DWORD CreateCellData2(char *filename,
                               GCD_CELL2 **cdat,
                               GCD_RECT **rdat,
                               GCD_HANTEI **hdat); // セルデータ読み込み（２）、圧縮形式対応に伴い追加。
 ...
 */
#define CREATE_GOBJ(pointer, obj)                                                                            \
  {                                                                                                          \
    pointer = new obj;                                                                                       \
    if (pointer && !pointer->GetGObject()) {                                                                 \
      delete pointer;                                                                                        \
      pointer = NULL;                                                                                        \
    }                                                                                                        \
  }
 ...
 *	引数を持つコンストラクタを呼び出す場合、クラス名と同時に設定してください。
 */
#define CREATE_GOBJ_FX(obj)                                                                                  \
  {                                                                                                          \
    CGoluahObject *pointer = new obj;                                                                        \
    if (pointer && !pointer->GetGObject()) {                                                                 \
      delete pointer;                                                                                        \
    }                                                                                                        \
  }
...
public:
  ...
  void SetHitMessage(DWORD msgid, DWORD param) {
    hitmsg = msgid;
    hitprm = param;
  }
  void SetGuardMessage(DWORD msgid, DWORD param) {
    grdmsg = msgid;
    grdprm = param;
  }
...
public:
  ...
  */
  void SetSpeed(double vx, double vy = 0) {
    this->vx = vx;
    this->vy = vy;
  }
...
public:
        ...
        void LoadBitmap(char* FileName, char* PalName = NULL) { Abonnu(); pBmp =
CGoluahObject::LoadBitmap(FileName, PalName); } operator MYSURFACE*() { return pBmp; } virtual ~CGBitmap() {
Abonnu(); }
