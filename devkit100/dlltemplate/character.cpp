...
void CCharacter::InitAttackInfo() {
  ...
  /*	//設定例
          ...
          aif[i].hit		=HITINFO_FUTTOBI | HITINFO_SIV3 | HITINFO_MARK3 | HITINFO_SNDHIT3 |
     HITINFO_EFCTSINDO;//ヒット情報 aif[i].guard	=GUARDINFO_REACT3 | GUARDINFO_SIV3 |
     GUARDINFO_XJAMP;//ガード情報 aif[i].id		=ATTACK_STRONG;//ゲージ増加時に使用するID
          ...
          aif[i].hit		=HITINFO_FUTTOBI2 | HITINFO_SIV3 | HITINFO_MARK3 | HITINFO_SNDHIT3
     ;//ヒット情報 aif[i].guard	=GUARDINFO_REACT3 | GUARDINFO_SIV3 | GUARDINFO_XJAMP;//ガード情報 aif[i].id
     =ATTACK_STRONG | ATTACK_EREAL;//ゲージ増加時に使用するID aif[i].damage	=50;//ダメージ aif[i].kezuri
     =0;//削り
...
{
  GOBJECT *pdat = GetGObject();
  CCharacterBase::InitWazInfo(); //基本動作の設定
