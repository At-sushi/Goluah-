...
LPDIRECT3DTEXTURE8 CTexManager::LoadTexture(const char *filename) {
  ...
  //新規に読み込み
  HRESULT loadret = D3DXCreateTextureFromFileEx(d3ddev, path, 0, 0, 0, 0, D3DFMT_A1R5G5B5, D3DPOOL_MANAGED,
                                                D3DX_FILTER_POINT, D3DX_FILTER_POINT, 0, NULL, NULL, &ret);
  delete[] path;
...
void CTristripBody::Render(D3DMATRIX *l2w) {
  ...
  try {
    ...
    //描画
    if (D3D_OK != d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, //プリミティブ種類
...
void CParticleBody::Render(D3DMATRIX *l2w) {
  ...
  for (i = 0; i < num; i++) {
    //色設定
    vb[i * 4].color = vb[i * 4 + 1].color = vb[i * 4 + 2].color = vb[i * 4 + 3].color = *((DWORD *)&col[i]);
  ...
  //描画
  {
    ...
    //描画
    d3ddev->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, // D3DPRIMITIVETYPE PrimitiveType,
  ...
  //描画
  {
    ...
    //描画
    try {
      if (D3D_OK != d3ddev->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, // D3DPRIMITIVETYPE PrimitiveType,
                                                   0,                  // UINT MinVertexIndex,
                                                   num * 4,            // UINT NumVertexIndices,
                                                   num * 2,            // UINT PrimitiveCount,
                                                   index_arr,          // const void *pIndexData,
...
void CMeshBody::Create(const char *x_filename) {
  ...
  // 読み込み
  hr = D3DXLoadMeshFromX((LPSTR)x_filename, D3DXMESH_MANAGED, d3ddev, NULL, &mtrbuf, &node_num, &x_mesh);
