...
D3DFORMAT CConfig::TexFormat() {
  switch (tex_format) {
  case CONFIG2_USETEXFMT_A1R5G5B5:
...
void CConfig::SetDeviceType(D3DDEVTYPE dt) {
  switch (dt) {
  case D3DDEVTYPE_HAL:
    device_type = CONFIG2_DEVICE_HAL;
    break;
  case D3DDEVTYPE_REF:
    device_type = CONFIG2_DEVICE_REF;
    break;
  case D3DDEVTYPE_SW:
    device_type = CONFIG2_DEVICE_SW;
    break;
  }
}
...
void CConfig::SetTexFormat(D3DFORMAT tf) {
  ...
  case D3DFMT_A1R5G5B5:
    tex_format = CONFIG2_USETEXFMT_A1R5G5B5;
    break;
  case D3DFMT_A4R4G4B4:
    tex_format = CONFIG2_USETEXFMT_A4R4G4B4;
    break;
  case D3DFMT_A8R3G3B2:
    tex_format = CONFIG2_USETEXFMT_A8R3G3B2;
    break;
  case D3DFMT_A8R8G8B8:
    tex_format = CONFIG2_USETEXFMT_A8R8G8B8;
    break;
  }
}
...
BOOL CConfig::LoadConfig() {
  ...
  if (filepath1 != NULL) {
    ...
    strcpy(filepath, ".\\system\\config.dat");
...
BOOL CConfig::LoadConfig2() {
  ...
  if (filepath2 != NULL) {
    ...
    strcpy(filepath, ".\\system\\config2.dat");
  ...
  if ((lc_key ^ ('aki')) == lc_key2) {
    OutputDebugString("キー一致、限界突破許可\n");
    limcut_pass = TRUE;
    limiter_cut_enable = 1;
  } else if (lc_key == lc_key2) {
    OutputDebugString("キー一致\n");
    limcut_pass = TRUE;
    limiter_cut_enable = 0;
  } else {
    OutputDebugString("キー不一致\n");
    limiter_cut = 0;
    limiter_cut_enable = 0;
    limcut_pass = FALSE;
  }
