...
public:
#ifdef GCD_EDITER
  static BOOL
  GCDLoadDlg(GCD_CELL2 *pcdat, GCD_RECT *prdat, GCD_HANTEI *phdat, GCD_CELLNAMES *pfhnames = NULL);
#endif // GCD_EDITER

  //! GCDファイルを読み込み
  static int GCDLoad(
      char *filename,         //!< [in] 読み込むファイル名
  ...
  //!バージョン0.70のファイルを読み込み
  static int GCDLoad070(
      HANDLE hFile,           //!< [in] 読み込むファイル
      LPVOID pcdat,           //!< [out] セルデータ読込先バッファ
      LPVOID prdat,           //!< [out] 切り取り矩形データ読込先バッファ
      LPVOID phdat,           //!< [out] あたり判定矩形読込先バッファ
      LPVOID pfhnames = NULL, //!< [out] セル定義名読込先バッファ
...
#ifdef GCD_EDITER
  static void
  GCDSetInitialData(GCD_CELL2 *pcdat, GCD_RECT *prdat, GCD_HANTEI *phdat, GCD_CELLNAMES *pfhnames);
#endif // GCD_EDITER
