...
typedef TCHAR*	(*PFUNCS_GETENEMYNAME)(DWORD);			//!< "敵"の名前を取得する
...
typedef void	(*PFUNCS_PUSHDLLEXECTAG)(const TCHAR*);	//!< 実行個所のタグ付けを行う(エラーログ時に出力)
typedef void	(*PFUNCS_POPDLLEXECTAG)();				//!< 実行個所のタグをひとつ削除する
typedef void	(*PFUNCS_LOGERROR)(const TCHAR*);		//!< エラーログを記録する
typedef void	(*PFUNCS_LOGWARNING)(const TCHAR*);		//!< 警告ログを記録する
typedef void	(*PFUNCS_LOGDEBUG)(const TCHAR*);		//!< デバッグログを記録する
typedef void	(*PFUNCS_LOGINFO)(const TCHAR*);			//!< 情報ログを記録する
...
typedef TCHAR*	(*PFUNCO_GETCHARNAME)(DWORD);							//!< 指定キャラの名前を取得する
...
typedef void   (*PFUNCD_LOADCELLDAT)(TCHAR*,LPVOID,LPVOID,LPVOID);//!< GCDデータ読み込み関数
typedef LPVOID (*PFUNCD_LOADBMP)(TCHAR*,TCHAR*);				//!< ビットマップ読み込み関数
...
typedef DWORD  (*PFUNCD_CREATEGCDDAT)(TCHAR* filename,LPVOID *cdat,LPVOID *rdat,LPVOID *hdat);//!< セルデータ読み込み。ver0.90以降ではLoadCellDatからこちらに変更するべき
typedef void   (*PFUNCD_DESTROYGCDDAT)(LPVOID *cdat,LPVOID *rdat,LPVOID *hdat);//!< CreateCellDatで生成されたバッファをクリア
typedef LPVOID (*PFUNCD_LOADIMAGE)(TCHAR*,TCHAR*);			//!< 画像読み込み関数、PNG対応に伴い追加。
typedef DWORD  (*PFUNCD_CREATEGCDDAT2)(TCHAR* filename,LPVOID *cdat,LPVOID *rdat,LPVOID *hdat);//!< セルデータ読み込み。ver0.90以降ではLoadCellDatからこちらに変更するべき
...
struct CHARACTER_LOAD_OPTION
{
	...
	TCHAR name[64];		//!< オプション名
	TCHAR point;			//!< オプション消費ポイント
};
