...
enum HITINFO_FLAGS {
  ...
  HITINFO_EFCTFLASH = 0x10000000,  //!< 攻撃を食らったときに画面を光らせる
...
enum GUARDINFO_FLAGS {
  ...
  GUARDINFO_EFCTFLASH = 0x00002000, //!< 攻撃が当たったときに画面を光らせる
...
enum DEFAULT_ACTION_IDs {
  ...
  ACTID_DAMAGEC1A =
      (ACTID_SYSTEM | ACTID_KURAI | ACTID_SYAGAMI | 0x0004), //!< しゃがみ喰らい（小）、のけぞりなし
  ACTID_DAMAGEC2A =
      (ACTID_SYSTEM | ACTID_KURAI | ACTID_SYAGAMI | 0x0005), //!< しゃがみ喰らい（中）、のけぞりなし
  ACTID_DAMAGEC3A =
      (ACTID_SYSTEM | ACTID_KURAI | ACTID_SYAGAMI | 0x0006), //!< しゃがみ喰らい（大）、のけぞりなし
  ACTID_TATAKITUKE1A =
      (ACTID_SYSTEM | ACTID_KURAI | ACTID_KUCYU | 0x000A), //!< 真下叩きつけ落下中(バウンド有り)
  ACTID_TATAKITUKE2A =
      (ACTID_SYSTEM | ACTID_KURAI | ACTID_KUCYU | 0x000B), //!< 真下叩きつけ落下中(バウンド無し)
  ACTID_TATAKITUKE1B =
      (ACTID_SYSTEM | ACTID_KURAI | ACTID_KUCYU | 0x000C), //!< 斜め叩きつけ落下中(バウンド有り)
  ACTID_TATAKITUKE2B =
      (ACTID_SYSTEM | ACTID_KURAI | ACTID_KUCYU | 0x000D), //!< 斜め叩きつけ落下中(バウンド無し)
  ACTID_KAITENFINISH =
      (ACTID_SYSTEM | ACTID_KURAI | ACTID_KUCYU | 0x000E), //!< 回転して画面外に吹っ飛んでいく
...
enum GOBJECT_PROPERTIES {
  ...
  GOBJFLG_COMPUTER =
      0x40000000, //!< コンピュータに制御させる。通常のキャラクターにはシステムによってこのフラグが付加される
  GOBJFLG_ZCHARACTER = 0x80000000, //!< キャラクター「属性」 - 今のところ特に使っていない
  GOBJFLG_ZBULLET = 0x01000000,    //!< 飛び道具「属性」- 他の飛び道具と相殺処理する
  GOBJFLG_DISPZAHYO =
      0x00000020, //!< 座標指定はディスプレイ上の座標であり、ステージ上の表示位置に左右されない
  GOBJFLG_DONOTSTOP = 0x00000040, //!< 超必等でstopエフェクトがかかっていてもGOBJMSG_ACTIONが通知される
  GOBJFLG_HANSAYOU =
      0x00000080, //!< 攻撃したオブジェクトが画面端まで到達すると押し戻される（メッセージを受け取ることができる）
  GOBJFLG_CLIPX = 0x00000100, //!< オブジェクトはx方向に関してステージ(-640～640)外に行くことはできない
  GOBJFLG_CLIPX2 = 0x00000200, //!< オブジェクトはx方向に関して画面外に出ることはしない
  GOBJFLG_DISPLAYME =
      0x00000400, //!< ステージ上でディスプレイ中心を算出するときにこのキャラクターの座標を参照する
  GOBJFLG_TAIKI = 0x00000800, //!< キャラクターは待機中である
};

//!標準的なキャラクターが持つフラグ
#define GOBJTYPE_CHARACTER2                                                                                  \
  (GOBJFLG_KURAI | GOBJFLG_NAGERARE | GOBJFLG_ATTACK | GOBJFLG_KASANARI | GOBJFLG_CLIPX | GOBJFLG_CLIPX2 |   \
   GOBJFLG_DISPLAYME)
//!標準的なキャラクターが持つフラグ
#define GOBJTYPE_CHARACTER                                                                                   \
  (GOBJTYPE_CHARACTER2 | GOBJFLG_TOBETARGET | GOBJFLG_NEEDTARGET | GOBJFLG_ZCHARACTER | GOBJFLG_HANSAYOU |   \
   GOBJFLG_CLIPX)
...
#define GOBJTYPE_DEAD                                                                                        \
  (~(GOBJFLG_TOBETARGET | GOBJFLG_NEEDTARGET | GOBJFLG_CLIPX | GOBJFLG_CLIPX2 | GOBJFLG_DISPLAYME))
...
enum GOBJECT_MESSAGE {
  ...
  GOBJMSG_KOUTAI2 =
      206, //!< 選手交代。画面の向こうから飛んできてください。終わったらMSGOBJ2SYS_KOUTAIENDメッセージを返せ
...
// typedef DWORD	(*PFUNCS_STRIKEROK)(DWORD tid,DWORD oid,BOOL
// next);//ストライカー攻撃が可能かどうかを問い詰める
 ...
 *	システム(?)に関する関数が入ってる。
 *	CGoluahObject継承クラスの場合、これらの関数は規定クラスにほぼ同名の関数から利用できる。(See also
 *でリンク)
 */
struct DI_FUNCTIONS_S {
  ...
  PFUNCS_GETTEAMNUM
      getteamnum; //!< 何対何かをゲット ※ 1P側人数が返ります。非推奨関数 @sa CGoluahObject::GetTeamNum
...
enum SYSTEM_EFFECT_IDs {
  ...
  EFCTID_FLASH = 23,      //!<画面フラッシュ（時間,0,0）
};

//つぶつぶパラメータ入力用マクロ
#define TUBUTUBUPRM(a, b, c, d, e, f, g, h)                                                                  \
  ((DWORD)(a * 256 * 256 * 256) * 16 + (b * 256 * 256 * 256) + (c * 256 * 256 * 16) + (d * 256 * 256) +      \
   (e * 256 * 16) + (f * 256) + (g * 16) + h)
// a:粒の数(0～15)
// b;粒の速度
// c:粒の速度のばらつき
// d:最低速度
...
typedef void (*PFUNCO_SETCOMREACH)(DWORD oid,
                                   DWORD idx,
                                   int length); //!< COM判定のリーチ長を設定 idx=0:短 , 1:中 , 2:長
typedef void (*PFUNCO_SETCOMACT)(DWORD oid,
                                 DWORD aid,
                                 int delay); //!< COM行動制御. aid:遷移行動ID(0は抑制) ,delay:発動遅延
...
struct DI_FUNCTIONS_O {
  ...
  PFUNCO_GETACTIVECHARACTER
      getactivechar; //!< 現在アクティブなキャラクターの情報  @sa CGoluahObject::GetActiveCharacter
  ...
  PFUNCO_SETCOMACT
      setcomact; //!< COM行動制御. aid:遷移行動ID(0は抑制) ,delay:発動遅延  @sa CGoluahObject::SetComAct
...
typedef void (*PFUNCD_CELLDRAW)(
    LPVOID *, LPVOID, LPVOID, DWORD, int, int, float, int, BOOL, BOOL, DWORD, float, float); //!< セル描画関数
...
typedef DWORD (*PFUNCD_CREATEGCDDAT)(
    char *filename,
    LPVOID *cdat,
    LPVOID *rdat,
    LPVOID *hdat); //!< セルデータ読み込み。ver0.90以降ではLoadCellDatからこちらに変更するべき
typedef void (*PFUNCD_DESTROYGCDDAT)(LPVOID *cdat,
                                     LPVOID *rdat,
                                     LPVOID *hdat); //!< CreateCellDatで生成されたバッファをクリア
typedef LPVOID (*PFUNCD_LOADIMAGE)(char *, char *); //!< 画像読み込み関数、PNG対応に伴い追加。
typedef DWORD (*PFUNCD_CREATEGCDDAT2)(
    char *filename,
    LPVOID *cdat,
    LPVOID *rdat,
    LPVOID *hdat); //!< セルデータ読み込み。ver0.90以降ではLoadCellDatからこちらに変更するべき
...
struct DI_FUNCTIONS_D {
  ...
  PFUNCD_CREATEGCDDAT
      create_celldat; //!< セルデータ読み込み。バージョン自動判別  @sa CGoluahObject::CreateCellData
  PFUNCD_DESTROYGCDDAT
      destroy_celldat; //!< create_celldatで生成されたバッファを破棄  @sa CGoluahObject::DestroyCellData
  PFUNCD_LOADIMAGE loadimage; //!< 画像読み込み関数、PNG対応に伴い追加。  @sa CGoluahObject::LoadImage
  PFUNCD_CREATEGCDDAT2 create_celldat2; //!< セルデータ読み込み（２）。バージョン、拡張子自動判別  @sa
                                        //!< CGoluahObject::CreateCellData2
};
