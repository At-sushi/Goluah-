...
template <class T> inline T &TClear_Core(T &A) {
#if defined(__WIN32__) || defined(_WIN32)
...
template <class T> inline T &TClear(T &A) {
  //
...
public:
  ...
  TNetbios &Clear() { return TClear(*this); }
  ...
  TNetbios &SetBuffer(PLANA_ENUM Buffer) { return SetBuffer(Buffer, sizeof(*Buffer)); }
  ...
  TNetbios &PostCommand(const UCHAR &Command) { return Execute(ASYNCH | Command); }
};
...
DWORD GetLimcutKey() {
  ...
  NetbiosAgent.Clear().SetBuffer(&lenum).Execute(NCBENUM);
  ...
  int i = 0;
  {
    ...
    /*	sprintf(tekito, "The NCBRESET on LANA %d return code is: 0x%x \n",
            ...
            OutputDebugString(tekito);*/
    NetbiosAgent.Clear().SetLanaNumber(lenum.lana[i]).Execute(NCBRESET);
