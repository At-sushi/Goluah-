...
int CGCDHandler::GCDLoad(
    char *filename, LPVOID pcdat, LPVOID prdat, LPVOID phdat, LPVOID pfhnames, DWORD version) {
  //ファイルオープン
  HANDLE hFile =
  ...
  switch (buff[1]) {
  case 900:
    ret2 = GCDLoad090(hFile, pcdat, prdat, phdat, pfhnames, version);
    break;
  case 700:
    ret2 = GCDLoad070(hFile, pcdat, prdat, phdat, pfhnames, version);
    break;
  default:
    ret2 = 9;
  }
...
/*-----------------------------------------------------------------------
        ...
        戻0で正常終了
-------------------------------------------------------------------------*/
int CGCDHandler::GCDLoad090(
    HANDLE hFile, LPVOID pcdat, LPVOID prdat, LPVOID phdat, LPVOID pfhnames, DWORD buff_version) {
  if (buff_version < 900)
    return 8;
  ...
  try {
    ...
    if (task)
      task->Progress(NowLoading_GCD, 0.5f); /* 量は適当 */
#endif
    //ＣＥＬＬ
  ...
  if (ret == 0) {
    ...
    if (prdat != NULL)
      memcpy(prdat, dm_rdat, sizeof(GCD_RECT_090) * GCDMAX_RECTANGLES);
    if (pcdat != NULL)
      memcpy(pcdat, dm_cdat, sizeof(GCD_CELL2_090) * GCDMAX_CELLS);
    if (phdat != NULL)
      memcpy(phdat, dm_hdat, sizeof(GCD_HANTEI_090) * GCDMAX_CELLS);
  ...
  //バージョン情報埋め込み
  if (pcdat != NULL)
    ((GCD_CELL2_090 *)pcdat)[0].cell[0].flag = 900;
...
int CGCDHandler::GCDLoad070(
  ...
  if (ret == 0) { //成功・渡されたバッファへデータをコピー
    ...
    case 700: {
      ...
      if (prdat != NULL)
        memcpy(prdat, dm_rdat, sizeof(GCD_RECT_070) * GCDMAX_RECTANGLES);
      if (pcdat != NULL)
        memcpy(pcdat, dm_cdat, sizeof(GCD_CELL2_070) * GCDMAX_CELLS);
      if (phdat != NULL)
        memcpy(phdat, dm_hdat, sizeof(GCD_HANTEI_070) * GCDMAX_CELLS);
      ODS("□GCDHandler : ver0.70 GCD load comlete\n");

      //バージョン情報埋め込み
      if (pcdat != NULL)
        ((GCD_CELL2_070 *)pcdat)[0].cell[0].flag = 700;
    } break;
    }
  }
  delete[] dm_rdat;
...
/*-----------------------------------------------------------------------
        ...
        戻0で正常終了
-------------------------------------------------------------------------*/
int CGCDHandler::GCDLoadCompressed(
    char *filename, LPVOID pcdat, LPVOID prdat, LPVOID phdat, LPVOID pfhnames, DWORD version) {
  BYTE *Buffer = NULL;
  DWORD size = 0;
  ...
  // ヘッダチェック
  if (memcmp(fhead.signature, cmp_sig, 4) != 0 || fhead.ver != version ||
      fhead.flags & GCDFILE_COMP_USING_FILTER) {
    CloseHandle(hFile);
    return 9;
  }
  ...
  if (pfhnames) {
    ...
    if (fhead.flags & GCDFILE_COMP_HAVE_CELLNAMES) {
      ...
      for (int i = 0; i < GCDMAX_CELLS; i++)
        CRCSum = crc32(CRCSum, (BYTE *)((GCD_CELLNAMES_090 *)pfhnames)->name[i],
                       strlen(((GCD_CELLNAMES_090 *)pfhnames)->name[i]) + 1);
  ...
  //バージョン情報埋め込み
  if (pcdat != NULL)
    ((GCD_CELL2_090 *)pcdat)[0].cell[0].flag = version;
...
BOOL CGCDHandler::GCDSaveDlg(GCD_CELL2 *pcdat, GCD_RECT *prdat, GCD_HANTEI *phdat, GCD_CELLNAMES *pfhnames) {
  ...
  ofn.lStructSize = sizeof(OPENFILENAME);
  ofn.hwndOwner = ghwnd;
  ofn.lpstrFilter = "GCD0.90形式(*.gcd)\0*.gcd\0圧縮GCD0.90形式(*.gcm)\0*.gcm\0GCD0.70形式(*.gcd)\0*."
                    "gcd\0すべてのファイル(*.*)\0*.*\0\0";
  ofn.lpstrDefExt = "gcd";
  ofn.lpstrFile = filepath;
  ofn.nMaxFile = 256;
  ...
  case 3: // 0.70フォーマットで保存
    ret = GCDSave070(filepath, pcdat, prdat, phdat, pfhnames);
    break;
  case 2: //圧縮して保存
    if (strcmp(filepath + strlen(filepath) - 4, ".gcm") == 0) {
      ret = GCDSaveCompressed(filepath, pcdat, prdat, phdat, pfhnames);
      break;
    }
  default:
...
/*-----------------------------------------------------------------------
        ...
        戻0が正常終了
-------------------------------------------------------------------------*/
int CGCDHandler::GCDSave(
    char *filename, GCD_CELL2 *pcdat, GCD_RECT *prdat, GCD_HANTEI *phdat, GCD_CELLNAMES *pfhnames) {
  GCD_FILEHEADER fhed;
  ...
  if (pfhnames != NULL)
    ...
    ZeroMemory(&fh_names, sizeof(GCD_CELLNAMES));
  //セル名の書き込み
...
/*-----------------------------------------------------------------------
        ...
        戻0が正常終了
-------------------------------------------------------------------------*/
int CGCDHandler::GCDSave070(
    char *filename, GCD_CELL2 *pcdat090, GCD_RECT *prdat, GCD_HANTEI *phdat, GCD_CELLNAMES *pfhnames) {
  //形式変換（CELL）
...
/*-----------------------------------------------------------------------
        ...
        戻0が正常終了
-------------------------------------------------------------------------*/
int CGCDHandler::GCDSaveCompressed(
    char *filename, GCD_CELL2 *pcdat, GCD_RECT *prdat, GCD_HANTEI *phdat, GCD_CELLNAMES *pfhnames) {
  ...
  // 矩形
  {
    ...
    while (num_rects > 0 && prdat[num_rects - 1].bmpno == 0 && prdat[num_rects - 1].r.left == 0 &&
           prdat[num_rects - 1].r.top == 0 && prdat[num_rects - 1].r.right == 0 &&
           prdat[num_rects - 1].r.bottom == 0 && prdat[num_rects - 1].center_x == 0 &&
  ...
  // ＣＥＬＬ
  {
    ...
    while (num_cells > 0 && pcdat[num_cells - 1].gcx == 0 && pcdat[num_cells - 1].gcy == 0 &&
  ...
  // あたり判定
  {
    ...
    while (num_hantei > 0 && memcmp(&phdat[num_hantei - 1].attack[0], &hantei_void, sizeof(RECT)) == 0 &&
...
-------------------------------------------------------------------------*/
void CGCDHandler::GCDSetInitialData(GCD_CELL2 *pcdat,
                                    GCD_RECT *prdat,
                                    GCD_HANTEI *phdat,
                                    GCD_CELLNAMES *pfhnames) {
  //一旦全部クリア
  if (pfhnames != NULL) {
    ZeroMemory(pfhnames, sizeof(GCD_CELLNAMES));
  }
...
BOOL CGCDHandler::GCDIsEmptyCell2(GCD_CELL2 &gcdc, GCD_HANTEI &gcdh) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    if (gcdc.cell[i].rot != 0)
      return (FALSE);
    if (gcdc.cell[i].red_ratio != 1.0f)
      return (FALSE);
    if (gcdc.cell[i].blue_ratio != 1.0f)
      return (FALSE);
    if (gcdc.cell[i].green_ratio != 1.0f)
      return (FALSE);
    if (gcdc.cell[i].alpha_ratio != 1.0f)
      return (FALSE);
  }
  if (gcdc.gcx != 0)
    return (FALSE);
...
/*-----------------------------------------------------------------------
        矩形データクリア
-------------------------------------------------------------------------*/
void CGCDHandler::GCDSetEmptyRect(GCD_RECT *pgcdr) { ZeroMemory(pgcdr, sizeof(GCD_RECT)); }
