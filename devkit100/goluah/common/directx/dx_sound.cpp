...
BOOL CDirectSound::Initialize(HWND hwnd) {
  ...
  if (g_config.UseDSound()) {
    ...
    CSystem::Log("DirectSound (wav再生) 非使用", SYSLOG_INFO);
...
char *GetDSErrCode(HRESULT ret) {
  ...
  case VFW_S_AUDIO_NOT_RENDERED:
    return "オーディオ ストリームを再生できない。適切なレンダラが見つからなかった。 ";
  case VFW_S_DUPLICATE_NAME:
    return "名前が重複しているフィルタの追加が、変更後の名前で成功した。 ";
  case VFW_S_PARTIAL_RENDER:
    return "このムービーにサポートされないフォーマットのストリームが含まれている。 ";
  case VFW_S_VIDEO_NOT_RENDERED:
    return "ビデオ ストリームを再生できない。適切なレンダラが見つからなかった。 ";
  case E_ABORT:
    return "操作が中止された。";
  case E_FAIL:
    return "失敗。 ";
  ...
  case VFW_E_CANNOT_CONNECT:
    return "接続を確立する中間フィルタの組み合わせが見つからなかった。 ";
  case VFW_E_CANNOT_LOAD_SOURCE_FILTER:
    return "このファイルのソース フィルタをロードできない。 ";
  case VFW_E_CANNOT_RENDER:
    return "ストリームをレンダリングするフィルタの組み合わせが見つからなかった。 ";
  case VFW_E_INVALID_FILE_FORMAT:
    return "ファイル フォーマットが無効。 ";
  case VFW_E_NOT_FOUND:
    return "オブジェクトまたは名前が見つからなかった。 ";
  case VFW_E_NOT_IN_GRAPH:
    return "フィルタ グラフに存在しないオブジェクトに要求された関数を実行できない。";
  case VFW_E_UNKNOWN_FILE_TYPE:
    return "このファイルのメディア タイプが認識されない。 ";
  case VFW_E_UNSUPPORTED_STREAM:
    return "ファイルを再生できない。フォーマットがサポートされていない。";
  default:
    return "不明";
  }
}
  ...
  while (hr = pEvent->GetEvent(&evCode, &param1, &param2, 0), SUCCEEDED(hr)) {
    ...
    case EC_COMPLETE:
      ...
      if (bgmon && bgmloop) {
        ...
        pSeek->SetPositions(&zerotime, AM_SEEKING_AbsolutePositioning, NULL, AM_SEEKING_NoPositioning);
        pMediaControl->Run();
      } else if (videoon) {
        destroooooy = TRUE;
