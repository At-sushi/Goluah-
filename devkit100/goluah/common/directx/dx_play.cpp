...
const GUID g_guidApp = {0xd224ca9f, 0xa3bc, 0x4286, {0xa2, 0xe4, 0x38, 0x34, 0x74, 0xba, 0xc4, 0x0}};
...
CDirectPlay::~CDirectPlay() { Destroy(); }
...
HRESULT CDirectPlay::Initialize(const char *Name, const char *Port) {
  ...
  CoCreateInstance(CLSID_DirectPlay8Peer, NULL, CLSCTX_INPROC_SERVER, IID_IDirectPlay8Peer, (void **)&pDPlay);
  ...
  if (Port[0] != '\0') {
    ...
    hr = pMyAddr->AddComponent(DPNA_KEY_PORT, &DPort, sizeof(DWORD), DPNA_DATATYPE_DWORD);
  }
...
HRESULT CDirectPlay::Host(void) {
  ...
  if (SUCCEEDED(hr = pDPlay->Host(&dpdesc, &pMyAddr, 1, NULL, NULL, NULL, DPNHOST_OKTOQUERYFORADDRESSING))) {
    m_isonline = TRUE;
    m_ishost = TRUE;
  }
...
HRESULT CDirectPlay::Connect(const char *IP, DWORD Port) {
  ...
  hr = pHostAddr->AddComponent(DPNA_KEY_PORT, &Port, sizeof(DWORD), DPNA_DATATYPE_DWORD);
  ...
  if (SUCCEEDED(hr = pDPlay->Connect(&dpdesc, pHostAddr, pMyAddr, NULL, NULL, NULL, 0, NULL, NULL, NULL,
