...
private:
	...
	BOOL Load256Bitmapbits(LPBYTE *bits, DWORD *width, DWORD *height, TCHAR *bmpfilename, int& rowbytes, MYPALLET (&pal)[256], TCHAR *palfilename = NULL);
	BOOL Load256PNGbits(LPBYTE *pbits, DWORD *width, DWORD *height, TCHAR *pngfilename, int& rowbytes, MYPALLET (&pal)[256], TCHAR *palfilename);//PNG版
	...
	BOOL CopyBB2TS256(const MYPALLET(&pbb)[256], DWORD bbpitch, DWORD offset_x, DWORD offset_y, LPDIRECT3DTEXTURE9 ptex, const LPBYTE pbits, DWORD damex = 0, DWORD damey = 0);
