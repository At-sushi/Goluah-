...
#define MINIMUM_TEXSIZE                                                                                      \
  (64) //!<最小テクスチャーサイズ。コレ以下のサイズのテクスチャしか作れないような環境は知らん。
...
// D3Dのヘルプによると、処理コストはデカイらしい
  ...
  void CellDraw(MYSURFACE **psuf,
                ...
                float magy = 1.0f,
                BOOL shadowed = FALSE);
  void CellDraw090(MYSURFACE **psuf,
                   LPVOID pcdat,
                   LPVOID prdat,
                   DWORD cn,
                   int x,
                   ...
                   float magy = 1.0f,
                   BOOL shadowed = FALSE);
  void CellDraw070(MYSURFACE **psuf,
                   LPVOID pcdat,
                   LPVOID prdat,
                   DWORD cn,
                   int x,
...
#ifdef GCD_EDITER
  //指定矩形を強調して描画
  void CellDrawED(MYSURFACE **psuf,
                  LPVOID pcdat,
                  LPVOID prdat,
                  DWORD cn,
                  int x,
  ...
  // サーフェスに直接書き込み
  //	void Kakikomi(LPDIRECT3DSURFACE8 SouShin, RECT& r2, LPDIRECT3DSURFACE8 BackBuffer, POINT& point, BOOL
  //revx);
...
private:
  ...
  BOOL Load256Bitmapbits(
      MYPALLET **bits, DWORD *width, DWORD *height, char *bmpfilename, char *palfilename = NULL);
  BOOL Load256PNGbits(
      MYPALLET **pbits, DWORD *width, DWORD *height, char *pngfilename, char *palfilename); // PNG版
