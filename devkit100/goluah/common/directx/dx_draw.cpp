...
BOOL CDirectDraw::InitDirectDraw(HWND hwnd, BOOL win) {
  ...
  //フルスクリーンの場合の初期化
    ...
    // 使用可能な画面モードを探して選択
    if (dd->CheckDeviceType(D3DADAPTER_DEFAULT, devtypenow, D3DFMT_X8R8G8B8, D3DFMT_X8R8G8B8, FALSE) ==
        D3D_OK)
      d3dpp.BackBufferFormat = D3DFMT_X8R8G8B8;
    else if (dd->CheckDeviceType(D3DADAPTER_DEFAULT, devtypenow, D3DFMT_X1R5G5B5, D3DFMT_X1R5G5B5, FALSE) ==
             D3D_OK)
      d3dpp.BackBufferFormat = D3DFMT_X1R5G5B5;
    else if (dd->CheckDeviceType(D3DADAPTER_DEFAULT, devtypenow, D3DFMT_R5G6B5, D3DFMT_R5G6B5, FALSE) ==
             D3D_OK)
  ...
  HRESULT ret_devcreate =
      dd->CreateDevice(D3DADAPTER_DEFAULT, devtypenow, hwnd, vertexprocessmode, &d3dpp, &d3ddev);

  if (D3D_OK != ret_devcreate) { //ありゃま
    vertexprocessmode = D3DCREATE_SOFTWARE_VERTEXPROCESSING;
    ret_devcreate = dd->CreateDevice(D3DADAPTER_DEFAULT, devtypenow, hwnd, vertexprocessmode, &d3dpp,
                                     &d3ddev); // ソフト頂点処理でやり直し。
...
void CDirectDraw::ResetDirectDraw() {
  ...
  d3ddev->CreateVertexBuffer(sizeof(MYVERTEX3D) * 4, D3DUSAGE_DYNAMIC | D3DUSAGE_WRITEONLY, FVF_3DVERTEX,
                             D3DPOOL_DEFAULT, &pMyVertex);
 ...
 *	@param dmy
 *未使用。まだDirectDrawを使っていた頃、VRAMに読むかシステムメモリに読むかのフラグだったもの。
 *
 ...
 */
MYSURFACE *
CDirectDraw::CreateSurfaceFrom256Image(char *filename, char *pallet /*=NULL*/, BOOL dmy /*=TRUE*/) {
  if (filename) {
    ...
    if (Buffer) {
      ...
      if (Buffer_pal)
        sprintf(Buffer_pal, "%s%s", pallet, ".png");
  ...
  *br = 0;
  ...
  for (DWORD i = 0; i < sizeimage; i += 524288) {
    ...
    *br += br2;
    ...
    if (task)
      task->Progress(item, (float)((double)i / sizeimage));
#endif
  }
...
static void png_read_row_callback(png_structp strPNG, png_uint_32 row, int pass) {
  if (row * strPNG->rowbytes % (524288 * 8) < strPNG->rowbytes) {
    ...
    CTNowLoading *task = dynamic_cast<CTNowLoading *>(g_system.FindTask('LOAD'));
    if (task)
      task->Progress(NowLoading_Image, (float)((double)row / strPNG->height));
  }
}
...
static void png_read_row_callback_adam7(png_structp strPNG, png_uint_32 row, int pass) {
  if (row == 0 && pass >= 3) {
    ...
    CTNowLoading *task = dynamic_cast<CTNowLoading *>(g_system.FindTask('LOAD'));
    if (task)
      task->Progress(NowLoading_Image,
                     (float)((double)pass * pass / (8 * 8))); // 最大pass値の所得方法がないみたいなので･･･
  }
}
 ...
 */
BOOL CDirectDraw::Load256PNGbits(
    MYPALLET **pbits, DWORD *width, DWORD *height, char *pngfilename, char *palfilename) {
  BOOL ret2;

  //とりあえずパレットを確保
  MYPALLET pal[256];
  if (palfilename == NULL)
    palfilename = pngfilename;
  ret2 = GetPalletPNG(palfilename, pal);
  if (!ret2) {
    ret2 = GetPallet(palfilename, pal);
  ...
  if (!setjmp(PngErrJamp)) {
    ...
    if (!png_get_IHDR(strPNG, infoPNG, (UINT *)width, (UINT *)height, &bit_depth, &color_type, NULL, NULL,
                      NULL)) {
 ...
 *	@sa CheckBlt2
 */
void CDirectDraw::CheckBlt(MYSURFACE *dds,
                           int x,
                           int y,
                           RECT r,
                           BOOL revx,
...
void CDirectDraw::CheckBlt2(MYSURFACE *dds,
  ...
  // if(dds==NULL){return;}
  if (dds < &ms[0] || dds > &ms[MAXNUMGOLUAHTEXTURES - 1])
    return;
  ...
  else*/
  {
    ...
    D3DXMatrixTransformation(
        &tmpmat, NULL, NULL, &D3DXVECTOR3((float)magx, (float)magy, 1.0f), //拡大縮小
        NULL, NULL,
...
void CDirectDraw::MyBlt3D(MYSURFACE *dds, RECT src, MYRECT3D dst, DWORD flag, DWORD color) {
  ...
  else*/
  {
    ...
    D3DXMatrixTransformation(&matpres, NULL, NULL, &D3DXVECTOR3(sclx, scly, 1.0f), //拡大縮小
                             NULL, NULL,
      ...
      for (i = 0; i < dds->xsufnum; i++) {
        if (dds->pTex[j * dds->xsufnum + i] == NULL)
          ...
          if (r_left > dds->xsufindx[i])
            ...
            transx = (float)(dds->xsufindx[i] - r_left); // pixel単位
          if (r_top > dds->ysufindx[j])
            transy = 0;
          else
            transy = (float)(dds->ysufindx[j] - r_top); // pixel単位
                           ...
                           BOOL shadowed)    //!<影付き
{
  ...
  case 900:
    CellDraw090(pbuf, pcdat, prdat, cn, x, y, z, rot, revx, revy, color, magx, magy, shadowed);
    break;
  case 700:
    CellDraw070(pbuf, pcdat, prdat, cn, x, y, z, rot, revx, revy, color, magx, magy, shadowed);
    break;
#ifdef AKIDX_DEBUG
  default: {
    char tmp[64];
                              ...
                              BOOL shadowed)    //!<影付き
{
  ...
  if (devtypenow != D3DDEVTYPE_HAL) {
    for (int i = 0; i < 8; i++) {
      ...
      CheckBlt(pbuf[rdat[rn].bmpno], x + cdat[cn].cell[i].dx, y + cdat[cn].cell[i].dy, rdat[rn].r, revx, revy,
               0, z, color); //描画
    }
  } else {
    ...
    if (!(cdat[cn].flag & GCDCELL2_SCA_GCENTER)) {
      if (cdat[cn].flag & GCDCELL2_ROT_BASEPOINT) {
        //スケール：足元中心 回転：足元中心
        D3DXMatrixTransformation(&matp, NULL, NULL, &D3DXVECTOR3(magx, magy, 1.0f), //拡大
                                 NULL,
                                 &quat, //回転
                                 NULL);
      } else {
      ...
      if (cdat[cn].flag & GCDCELL2_ROT_BASEPOINT) {
        //スケール：重心中心 回転：足元中心
        D3DXMatrixTransformation(&matp, &center,
        ...
        //スケール：重心中心 回転：重心中心
        D3DXMatrixTransformation(&matp, &center, NULL, &D3DXVECTOR3(magx, magy, 1.0f), //拡大
                                 &center,
                                 &quat, //回転
                                 NULL);
      }
    ...
    if (revy) {
      D3DXMatrixTranslation(&tmt, (float)(cdat[cn].gcx) * ar2 * (-1.0f),
                            (float)(cdat[cn].gcy) * ar2 * (-1.0f), 0); //重心に移動
      matp *= tmt;
      d3dxplane_y.d = 0;
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (rn < GCDMAX_RECTANGLES && rn != 0 && rdat[rn].bmpno < GCDMAX_IMAGES) {
        //変換行列を計算
        const auto center =
            D3DXVECTOR3((float)(rdat[rn].center_x) * ar2, (float)(rdat[rn].center_y) * ar2, 0); //重心

        D3DXMatrixTransformation(&mat, &center, NULL,
                                 &D3DXVECTOR3(cdat[cn].cell[i].magx, cdat[cn].cell[i].magy, 1.0f), //拡大
                                 &center,
                                 D3DXQuaternionRotationAxis(&quat, &D3DXVECTOR3(0, 0, 1),
                                                            D3DXToRadian(cdat[cn].cell[i].rot)), //回転
                                 &D3DXVECTOR3((float)(cdat[cn].cell[i].dx) * ar2,
                                              (float)(cdat[cn].cell[i].dy) * ar2, 0) //表示位置への移動
        );
        ...
        CheckBlt(pbuf[rdat[rn].bmpno], 0, 0, rdat[rn].r, c1revx, c1revy, 0, z, rcolor); //描画
        ...
        if (shadowed) {
          ...
          D3DXMatrixShadow(&mat, &D3DXVECTOR4(0, -100, 100, 1), &Plane);
          matp = SetParentMatrix(mat, FALSE);
          CheckBlt(pbuf[rdat[rn].bmpno], 0, 0, rdat[rn].r, c1revx, c1revy, 0, z, 0xAA000000); //影描画
 ...
 *	CellDraw070とCellDraw090は、内部での座標変換の順番が微妙に入れ替わっているので注意
 */
void CDirectDraw::CellDraw070(MYSURFACE **pbuf, //!< GCDで利用するビットマップ配列
                              ...
                              BOOL shadowed)    //!<影付き
{
  ...
  if (devtypenow != D3DDEVTYPE_HAL) {
    ...
    D3DXMatrixTransformation(
        &matp, &center, NULL, &D3DXVECTOR3(magx, magy, 1.0f),                                 //拡大
        &center, D3DXQuaternionRotationAxis(&quat, &D3DXVECTOR3(0, 0, 1), D3DXToRadian(rot)), //回転
        NULL);
    if (revy) {
      D3DXMatrixTranslation(&tmt, (float)(cdat[cn].gcx) * ar2 * (-1.0f),
                            (float)(cdat[cn].gcy) * ar2 * (-1.0f), 0); //重心に移動
 ...
 *	@sa CellDraw
 */
void CDirectDraw::CellDrawED(MYSURFACE **pbuf,
                             LPVOID pcdat,
                             LPVOID prdat,
                             DWORD cn,
                             int x,
                             ...
                             BOOL is_edit_cell)     //!< *現在編集ちゅうのセルかどうか
{
  ...
  if (cn > 0 && !is_next_cell && (cdat[cn - 1].flag & GCDCELL2_LINK)) {
    CellDrawED(pbuf, pcdat, prdat, cn - 1, x, y, z, rot, revx, revy, color, magx, magy, ed_rect,
               color_modulat, TRUE, FALSE, FALSE);
  } //------------------------------------------------------------------------------------------------
  ...
  if (!is_previous_cell) {
    ...
    if (cdat[cn].flag & GCDCELL2_LINK) {
      CellDrawED(pbuf, pcdat, prdat, cn + 1, x, y, z, rot, revx, revy, color, magx, magy, ed_rect,
                 color_modulat, FALSE, TRUE, FALSE);
    }
  }
}
#endif
 ...
 */
void CDirectDraw::HanteiDraw(LPVOID pcdat,
  ...
  if (b_atr) { //当たり判定（黄色）の描画
    for (i = 0; i < 3; i++) {
      ...
      vb[0].color = vb[1].color = vb[2].color = vb[3].color = 0x77FFFF00;
      //描画
  ...
  if (b_kas) { //重なり判定（あか）の描画
    for (i = 0; i < 3; i++) {
      ...
      vb[0].color = vb[1].color = vb[2].color = vb[3].color = 0x77FF0000;
      //描画
      d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vb, sizeof(MYVERTEX3D));
    }
  }
  ...
  if (b_atk) { //攻撃判定（あお）の描画
    for (i = 0; i < 3; i++) {
      ...
      vb[0].color = vb[1].color = vb[2].color = vb[3].color = 0x770000FF;
      //描画
      d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vb, sizeof(MYVERTEX3D));
    }
  }
 ...
 *	@brief 単純線描画
 */
void CDirectDraw::DrawLine(int sx,   //!< 始点x
                           int sy,   //!< 始点y
                           int gx,   //!< 終点x
                           int gy,   //!< 終点y
                           DWORD col //!< 描画色
...
void CDirectDraw::HRectDraw(LPVOID pcdat,
  ...
  D3DXMatrixTranslation(&tmt, (float)(cdat[cn].gcx * magx) * ar2 * (-1.0f),
                        (float)(cdat[cn].gcy * magy) * ar2 * (-1.0f), 0); //重心に移動
  matp *= tmt;
  D3DXMatrixRotationZ(&tmt, D3DXToRadian(rot)); //回転
  matp *= tmt;
  //重心に戻す
...
void CDirectDraw::GCenterDraw(
  ...
  matp *= tmt;
  //表示位置への移動
  D3DXMatrixTranslation(&tmt, (float)cdat[cn].gcx * ar2 * magx + (x - cdat[cn].gcx) * ar2,
                        (float)cdat[cn].gcy * ar2 * magy + (y - cdat[cn].gcy) * ar2, 0);
  matp *= tmt;
...
void CDirectDraw::ResetTransformMatrix() {
  ...
  D3DXMatrixLookAtRH(&matv, &D3DXVECTOR3(camera_x + camera_zurax, camera_y + camera_zuray, camera_z),
                     &D3DXVECTOR3(camera_x, camera_y, 0), &D3DXVECTOR3(0, -3, 0));
  d3ddev->SetTransform(D3DTS_VIEW, &matv);

  // projection
  D3DXMatrixPerspectiveFovRH(&matp, (float)(2.0 * atan2(1.0, 3.0)), ASPECTRATIO, 0.1f, 50.0f);
 ...
 */
void CDirectDraw::SetWorldMatrix(D3DXMATRIX *pmat) { d3ddev->SetTransform(D3DTS_WORLD, pmat); }
 ...
 *	色はブルー固定らしい。（なんで？）
 */
void CDirectDraw::DrawBlueText(RECT &r,      //!< 描画する矩形領域
                               char *text,   //!< テキスト文字列
                               int len,      //!< テキスト文字数
                               ...
                               DWORD size)   //!< 描画サイズ。1～(AKIDX_FONTNUM-1)
{
  ...
  lpFont[size]->DrawText(text, len, &r, method, 0xFF5522FF);
}
...
void CDirectDraw::DrawRedText(RECT &r, char *text, int len, DWORD method, DWORD size) {
  ...
  lpFont[size]->DrawText(text, len, &r, method, 0xFFFF2255);
}
...
LPDIRECT3DTEXTURE8 CDirectDraw::GetFrontBufferCopy() {
  ...
  if (D3D_OK != d3ddev->CreateTexture(
                    ...
                    //Format,ひょっとしたらこっちかもしれないので試しに差し替えてみた
  ...
  do {
    ...
    if (D3D_OK !=
        d3ddev->CreateTexture(tex_width,       // UINT Width,
                              tex_height,      // UINT Height,
                              1,               // UINT Levels(mipmap),
                              0,               // DWORD Usage,
    ...
    for (UINT y = 0; y < tex_height; y++) {
      ...
      // A8R8G8B8→A1R5G5B5
      if (y + offset_y >= m_desktop_height)
        break;
      DWORD *src = (DWORD *)((BYTE *)lr.pBits + lr.Pitch * (y + offset_y) + offset_x * 4);
      WORD *dst = (WORD *)((BYTE *)lr2.pBits + lr2.Pitch * y);
      for (UINT x = 0; x < tex_width; x++) {
        if (x + offset_x >= m_desktop_width)
          break;
