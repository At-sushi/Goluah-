...
public:
  ...
  void SetWidth(UINT index, float value) {
    if (index < node_num)
      rad[index] = value * 2.0f;
  }
  void SetPos(UINT index, V3d &value) {
    if (index < node_num)
      pos[index] = value;
  }
  void SetColor(UINT index, ARGB value) {
    if (index < node_num)
      col[index] = value;
  }
  void SetColorDW(UINT index, DWORD value) {
    if (index < node_num)
      col[index].dwcol = value;
  }
...
public:
  ...
  void SetAxis(UINT index, V3d &value) {
    if (index < num)
      ax[index] = value;
  }
...
public:
  ...
  void SetAxis1(UINT index, V3d value) {
    if (index < num)
      ax1[index] = value;
  }
  void SetAxis2(UINT index, V3d value) {
    if (index < num)
      ax2[index] = value;
  }
  void SetColor(UINT index, ARGB value) {
...
protected:
  ...
  V2d					*vtemp;
  //テンポラリ頂点バッファ。以下のバッファはこれをオフセットして使うので、deleteしないこと V2d
  *vsubs;		//各ノード間のsubベクトルと垂直なベクトル
  V2d					*norms;		//各ノードの法線みたいなもの*/
