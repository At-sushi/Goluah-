...
/*---------------------------------------------------------
        構築
-----------------------------------------------------------*/
void Aki3d::Initialize(LPDIRECT3DDEVICE8 d3d_device, const char *tex_path, WarningReportFunctionPtr pwf) {
  if (d3ddev)
    return;
...
/*---------------------------------------------------------
        0～1 実数ランダム
-----------------------------------------------------------*/
float Aki3d::RandomOne() { return ((float)(rand() % 1000)) * 0.001f; }

/*---------------------------------------------------------
        -1～1 実数ランダム
-----------------------------------------------------------*/
float Aki3d::RandomOne2() { return RandomOne() * 2.0f - 1.0f; }
...
//長さ取得
float V2d::Length() { return sqrtf(x * x + y * y); }
