...
DWORD CCharacter::CmdCom_OnNormal(DWORD wid) {
  ...
  if (!(pdat->aid & ACTID_KUCYU) && (opt_exattack)) {
    ...
    if (GetGObject()->aid & ACTID_ATTACK) {
      switch (wid) {
      case ACTID_HKICK:
        return ChainCombo(CHAIN_HKICK);
        break;
      case ACTID_HAND:
        return ChainCombo(CHAIN_HAND);
        break;
      case ACTID_UPPER_STEP:
        return ChainCombo(CHAIN_UPPER);
        break;
      case ACTID_BAT:
        return ChainCombo(CHAIN_BAT);
        break;
      }
    }
  }
...
DWORD CCharacter::Action() {
  ...
  case ACTID_BAT:
    act_bat();
    ChainCombo(CHAIN_BAT);
    break;
  case ACTID_UPPER_STEP:
    act_upper_step();
    ChainCombo(CHAIN_UPPER);
    break;
  case ACTID_UPPER_JAMP:
    act_upper_jamp();
    break;
  case ACTID_UPPER_LANDING:
    act_upper_landing();
    break;
  case ACTID_HAND:
    act_hand();
    ChainCombo(CHAIN_HAND);
    break;
  case ACTID_HKICK:
    act_hkick();
    ChainCombo(CHAIN_HKICK);
    break;
  case ACTID_SAH_LANDING:
    act_sah_landing();
    break;
