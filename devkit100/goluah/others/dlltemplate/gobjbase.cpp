...
DWORD CCharacterBase::TouchA(ATTACKINFO *info, DWORD ta_eid) {
  ...
  if (auto_guard) {
    if (CANNOT_GUARD(info->guard) && (pdat->aid != ACTID_RAKKA))
      return 0; //どのガードもできない場合はガードできない
      ...
      /*			// サウンドがNULLの場合はパス。
                              if ( (i < sound_list.size()) && sound_list[i].sound )
                              {
                                      if ( ( (i > 0) && (sound_list[i - 1].number == sound_list[i].number) ) )
                                      {
                                              // 同じ番号があるので失敗、配列を１減らす。
                                              mysound_numbers--;
                                              mysounds = (LPMYSOUND)realloc(mysounds, sizeof(MYSOUND) *
         mysound_numbers); KillMySound(sound_list[i].sound);	// 使わないのであぼんぬ
                                      }
                                      else
                                      {
                                              mysounds[success_num] = sound_list[i];
