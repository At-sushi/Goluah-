...
CCharacterInfo::CCharacterInfo(LPCTSTR CharName,
                               DWORD CdiVersion,
...
CGoluahObject::CGoluahObject() { CGoluahObjectCreate(); }
...
void CGoluahObject::LoadCellData(char *pathname,
                                 ...
                                 GCD_RECT *pRects,
                                 GCD_HANTEI *pHanteis) { //セルデータ読み込み関数
...
void CGoluahObject::SetParentMatrix(LPD3DXMATRIX pMatrix,
                                    BOOL erase,
                                    LPD3DXMATRIX OldMatrix) { //「親」の変換行列設定
...
void CGoluahObject::SendSynchronizedMessage(DWORD size, LPVOID dat) {
  if (funcn)
    funcn->send_syncmsg(pdat->id, size, dat);
}
void CGoluahObject::SetSynchronize(BOOL enabled) {
  if (funcn)
    funcn->set_syncronize(pdat->id, enabled);
}
...
void CCharacterBase::InitGCDandBMP() {
  ...
  if (((GCD_CELL2_070 *)cells)[0].cell[0].flag >= 900) { // セルのバージョンが0.90以上
    if (cells != NULL) {
      free(cells);
      cells = NULL;
    }
...
void CCharacterBase::act_strikercomeon() { GetGObject()->aid = ACTID_NEUTRAL; }
...
void CCharacterBase::LoadAndSetKatiSerif() {
  ...
  if (GetWinSerif(katiserifu, enemyname, filepath)) {
    ...
    funcs->setkatiserif(pdat->tid, "セリフの取得に失敗");
}
...
CClassicalBullet::CClassicalBullet(
    ...
    : CBulletBase(parent, info, userID, userNo) {
  array_len_r = 0;
  array_len_d = 0;
