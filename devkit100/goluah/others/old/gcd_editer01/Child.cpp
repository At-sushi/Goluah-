...
void CldDraw(HWND hwnd) {
  ...
  //スクロール位置にあわせてビットマップを転送
    ...
    if (r.right - r.left < bmp_info[radio].hed.biWidth)
      bltw = r.right - r.left; //ビットマップの幅
    else
      bltw = bmp_info[radio].hed.biWidth;
    if (r.bottom - r.top < bmp_info[radio].hed.biHeight)
      blth = r.bottom - r.top; //ビットマップの高さ
    else
      blth = bmp_info[radio].hed.biHeight;

    ofsty = (bmp_info[radio].hed.biHeight) - si_y.nPos - (r.bottom - r.top);
    if (ofsty < 0)
      ofsty = 0;

    StretchDIBits(hdc, 0, 0, bltw, blth, si_x.nPos, ofsty, bltw, blth, bmp_bits[radio],
                  (BITMAPINFO *)&bmp_info[radio], DIB_RGB_COLORS, SRCCOPY);
  }

  if ((DWORD)radio == ed_r[edit_now].bmpno) { //このビットマップ# に矩形がある
    //矩形表示
    DrawRECT2(hdc, ed_r[edit_now].r.left - si_x.nPos, ed_r[edit_now].r.right - si_x.nPos,
              ed_r[edit_now].r.top - si_y.nPos, ed_r[edit_now].r.bottom - si_y.nPos, 2);
    //中心表示
    DrawPos(hdc, ed_r[edit_now].r.left - si_x.nPos + ed_r[edit_now].center_x,
            ed_r[edit_now].r.top - si_y.nPos + ed_r[edit_now].center_y, FALSE);
  }
...
LRESULT WINAPI CldMsgProc(HWND hWnd, UINT msg, WPARAM wparam, LPARAM lparam) {
  ...
  case WM_LBUTTONDOWN:
    if (radio2 == 0) {
      ...
      sx = gx = LOWORD(lparam);
      sy = gy = HIWORD(lparam);
    } else if (ed_r[edit_now].bmpno != 0 || ed_r[edit_now].r.left != ed_r[edit_now].r.right ||
  ...
  case WM_VSCROLL:
  case WM_HSCROLL:
    WMScroll(hWnd, msg, wparam, lparam);
    return (0);
...
void WMScroll(HWND hwnd, UINT msg, DWORD wp, DWORD lp) {
  ...
  /*	switch (LOWORD(wp)) {
                  case SB_LINEUP:
     MessageBox(ghwnd,"SB_LINEUP","",MB_OK);break;//ちっちゃいボタン（上） case SB_LINEDOWN:
     MessageBox(ghwnd,"SB_LINEDOWN","",MB_OK);break;//ちっちゃいボタン（下） case SB_PAGEUP:
     MessageBox(ghwnd,"SB_PAGEUP","",MB_OK);break;//空白部分（上） case SB_PAGEDOWN:
     MessageBox(ghwnd,"SB_PAGEDOWN","",MB_OK);break;//空白部分（下） case SB_THUMBTRACK:
     MessageBox(ghwnd,"SB_THUMBTRACK","",MB_OK);break;//ドラッグ（動かし中） case SB_THUMBPOSITION:
     MessageBox(ghwnd,"SB_THUMBPOSITION","",MB_OK);break;//ドラッグ（ボタン離した） default:dy = 0;break;
              }*/
  //----------------------------------------------------------------
  ...
  case WM_VSCROLL:
    ...
    if (dy > 0) {
      if (int(si_y.nPos + si_y.nPage + dy) > si_y.nMax)
        dy = si_y.nMax - si_y.nPage - si_y.nPos;
    }
    if (dy != 0) {
      si_y.nPos += dy;
      SetScrollInfo(hwnd, SB_VERT, &si_y, TRUE);
  ...
  case WM_HSCROLL:
    ...
    if (dx > 0) {
      if (int(si_x.nPos + si_x.nPage + dx) > si_x.nMax)
        dx = si_x.nMax - si_x.nPage - si_x.nPos;
    }
    if (dx != 0) {
      si_x.nPos += dx;
      SetScrollInfo(hwnd, SB_HORZ, &si_x, TRUE);
...
HWND CreateChildWindow(HWND hdlg) {
  ...
  HWND hWnd = CreateWindow("child", "child", WS_CHILD | WS_VSCROLL | WS_HSCROLL, 5, 5, MYCHILDWIDTH,
                           MYCHILDHEIGHT, hdlg, NULL, wc.hInstance, NULL);
  hwc = hWnd;
  return (hWnd);
}
