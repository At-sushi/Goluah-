...
INT WINAPI WinMain(HINSTANCE hInst, HINSTANCE, LPSTR, INT) {
  ...
  DialogBox(hInst, MAKEINTRESOURCE(IDD_DIALOG1), GetDesktopWindow(), DialogProc);
...
BOOL CALLBACK DialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case WM_INITDIALOG:
    ...
    break;
  case WM_COMMAND:
    return (DlgCommand(hwndDlg, uMsg, wParam, lParam));
    break;
  case WM_PAINT:
    DlgDraw(hwndDlg);
    break;
...
BOOL DlgCommand(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  case IDC_BUTTON_N:
    ...
    while (tobasilink && (ed_c[edit_now - 1].flag & GCDCELL2_LINK)) {
      edit_now++;
      if (edit_now > GCDMAX_RECTANGLES - 1)
        break;
    }
    if (edit_now > GCDMAX_RECTANGLES - 1)
      edit_now = GCDMAX_RECTANGLES;
    EditChanged();
    break;
  case IDC_BUTTON_P10:
    edit_now -= 10;
  ...
  //背景色指定
  case IDC_RADIO_BG1:
    pddraw->clearbgcolor = 0xFFF0F0F0;
    CldRedraw();
    break;
  case IDC_RADIO_BG2:
    pddraw->clearbgcolor = 0xFF0F0F0F;
    CldRedraw();
    break;
  case IDC_RADIO_BG3:
    pddraw->clearbgcolor = 0xFFF000F0;
    CldRedraw();
    break;
  case IDC_RADIO_BG4:
    pddraw->clearbgcolor = 0xFF00F0F0;
    CldRedraw();
    break;
  ...
  case IDC_BUTTON_BP:
    if (FindRect(FALSE, 1))
      DlgRedraw();
    break;
  case IDC_BUTTON_BP10:
    if (FindRect(FALSE, 10))
      DlgRedraw();
    break;
...
void RadioChanged() {
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO1), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio = 0; //移動
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO2), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio = 1; //判定矩形
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO3), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio = 2; //重心設定
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO4), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio = 3; //回転
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO5), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio = 4; //拡大縮小x
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO6), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio = 5; //拡大縮小y
  ...
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIOSEL1), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio2 = 0;
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIOSEL2), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio2 = 1;
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIOSEL3), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio2 = 2;
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIOSEL4), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio2 = 3;
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIOSEL5), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio2 = 4;
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIOSEL6), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio2 = 5;
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIOSEL7), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio2 = 6;
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIOSEL8), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radio2 = 7;

  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO_VIEW1), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radioview = 0; // 25%
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO_VIEW2), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radioview = 1; // 50%
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO_VIEW3), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radioview = 2; // 75%
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO_VIEW4), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radioview = 3; // 100%
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO_VIEW5), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radioview = 4; // 200%
  if (SendMessage(GetDlgItem(ghwnd, IDC_RADIO_VIEW6), BM_GETCHECK, 0L, 0L) == BST_CHECKED)
    radioview = 5; // 400%
