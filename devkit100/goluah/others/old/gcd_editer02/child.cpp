...
void CldDraw(HWND hwnd)
{
	...
	float cmz;
	switch(radioview){
	case 0:cmz=-4.0f;break;//25%
	case 1:cmz=-2.0f;break;//50%
	case 2:cmz=-1.5f;break;//75%
	case 4:cmz=-0.5f;break;//200%
	case 5:cmz=-0.25f;break;//400%
	default:cmz=-1.0f;break;//100%
	}
	pddraw->camera_z=cmz;
	...
	pddraw->CellDrawED(radio2,dds,ed_c,ed_r,edit_now,
		0,0,
		0,0,//(z/rot)
	...
	case WM_LBUTTONUP:
		if(mousedown){
			...
			if(radio==1){//判定設定
				...
				if(drawprv)DrawLINE(hWnd,sx,gx,sy,gy,0);//とりあえず線を消しとく
				gx=LOWORD(lparam);
				gy=HIWORD(lparam);
				drawprv=FALSE;
...
//				gx=LOWORD(lparam);
	...
	case WM_PAINT:CldDraw(hWnd);break;
