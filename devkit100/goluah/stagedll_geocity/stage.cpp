...
void CStage::InitVrtx() //頂点座標初期化
{
  ...
  //月
  if (SUCCEEDED(
          d3ddev->CreateVertexBuffer(sizeof(MYVERTEX3D) * 4, 0, FVF_3DVERTEX, D3DPOOL_MANAGED, &vb_moon)) &&
      SUCCEEDED(vb_moon->Lock(0, 0, (BYTE **)&vtx_tmp, 0))) {
    vtx_tmp[0].color = vtx_tmp[1].color = vtx_tmp[2].color = vtx_tmp[3].color = 0xFFFFFFFF;
    vtx_tmp[0].tu = 0;
    vtx_tmp[0].tv = 0;
    vtx_tmp[1].tu = 0;
    vtx_tmp[1].tv = 1;
    ...
    vtx_tmp[3].x = 10;
    vtx_tmp[3].y = -3;
    vtx_tmp[0].z = vtx_tmp[1].z = vtx_tmp[2].z = vtx_tmp[3].z = 30;
  ...
  //街
  if (SUCCEEDED(
          d3ddev->CreateVertexBuffer(sizeof(MYVERTEX3D) * 4, 0, FVF_3DVERTEX, D3DPOOL_MANAGED, &vb_city)) &&
    ...
    vtx_tmp[3].x = 6;
    vtx_tmp[3].y = 0;
    vtx_tmp[0].z = vtx_tmp[1].z = vtx_tmp[2].z = vtx_tmp[3].z = 20;
  ...
  //地面
  if (SUCCEEDED(
          d3ddev->CreateVertexBuffer(sizeof(MYVERTEX3D) * 4, 0, FVF_3DVERTEX, D3DPOOL_MANAGED, &vb_ground)) &&
      SUCCEEDED(vb_ground->Lock(0, 0, (BYTE **)&vtx_tmp, 0))) {
    vtx_tmp[0].color = vtx_tmp[1].color = vtx_tmp[2].color = vtx_tmp[3].color = 0xFF000000;
    vtx_tmp[0].tu = 0;
    vtx_tmp[0].tv = 1;
    vtx_tmp[1].tu = 0;
    vtx_tmp[1].tv = 0;
    ...
    vtx_tmp[3].x = 10;
    vtx_tmp[3].z = 20;
    vtx_tmp[0].y = vtx_tmp[1].y = vtx_tmp[2].y = vtx_tmp[3].y = 0.02f;
  ...
  for (i = 0; i < NUMDIV_ROADY; i++) {
    if (SUCCEEDED(d3ddev->CreateVertexBuffer(sizeof(MYVERTEX3D) * ((NUMDIV_ROADX + 1) * 2), 0, FVF_3DVERTEX,
                                             D3DPOOL_MANAGED, &vb_road[i])) &&
        SUCCEEDED(vb_road[i]->Lock(0, 0, (BYTE **)&vtx_tmp, 0))) {
      for (j = 0; j <= NUMDIV_ROADX; j++) {
        vtx_tmp[j * 2].color = vtx_tmp[j * 2 + 1].color = 0xFFFFFFFF;
        vtx_tmp[j * 2].tu = vtx_tmp[j * 2 + 1].tu = (1.0f / (float)NUMDIV_ROADX) * j;
  ...
  /*	vb_jct[1].color=vb_jct[2].color=
          ...
          vb_jct[4]=vtx_tmp;*/
  for (i = 0; i < NUMDIV_JCTY; i++) {
    if (SUCCEEDED(d3ddev->CreateVertexBuffer(sizeof(MYVERTEX3D) * ((NUMDIV_JCTX + 1) * 2), 0, FVF_3DVERTEX,
                                             D3DPOOL_MANAGED, &vb_jct[i])) &&
        SUCCEEDED(vb_jct[i]->Lock(0, 0, (BYTE **)&vtx_tmp, 0))) {
      for (j = 0; j <= NUMDIV_JCTX; j++) {
        vtx_tmp[j * 2].color = vtx_tmp[j * 2 + 1].color = 0xFFFFFFFF;
        vtx_tmp[j * 2].tu = vtx_tmp[j * 2 + 1].tu = (1.0f / (float)NUMDIV_JCTX) * j;
  ...
  //ガードレール
  if (SUCCEEDED(d3ddev->CreateVertexBuffer(sizeof(MYVERTEX3D) * ((NUM_GURD + 1) * 2), 0, FVF_3DVERTEX,
                                           D3DPOOL_MANAGED, &vb_gurd)) &&
  ...
  //看板支柱
  if (SUCCEEDED(
          d3ddev->CreateVertexBuffer(sizeof(MYVERTEX3D) * 4, 0, FVF_3DVERTEX, D3DPOOL_MANAGED, &vb_brd1a)) &&
    ...
    vtx_tmp[3].y = 0;
    vtx_tmp[0].z = vtx_tmp[1].z = vtx_tmp[2].z = vtx_tmp[3].z = 3;

    vb_brd1a->Unlock();
  }
  //看板表
  if (SUCCEEDED(
          d3ddev->CreateVertexBuffer(sizeof(MYVERTEX3D) * 4, 0, FVF_3DVERTEX, D3DPOOL_MANAGED, &vb_brd1b)) &&
      SUCCEEDED(vb_brd1b->Lock(0, 0, (BYTE **)&vtx_tmp, 0))) {
    vtx_tmp[0].color = vtx_tmp[1].color = vtx_tmp[2].color = vtx_tmp[3].color = 0xFF666666;
    vtx_tmp[0].tv = 0;
    vtx_tmp[0].tu = 0;
    vtx_tmp[1].tv = 0;
    vtx_tmp[1].tu = 1;
  ...
  //ガードレールの柱
  if (SUCCEEDED(d3ddev->CreateVertexBuffer(sizeof(MYVERTEX3D) * (6 * NUM_GURDPOLL), 0, FVF_3DVERTEX,
                                           D3DPOOL_MANAGED, &vb_gurdpoll)) &&
      SUCCEEDED(vb_gurdpoll->Lock(0, 0, (BYTE **)&vtx_tmp, 0))) {
    for (i = 0; i < NUM_GURDPOLL; i++) {
      alpha = (BYTE)((255 / (NUM_GURDPOLL * 1.5)) * (NUM_GURDPOLL - i));
      vtx_tmp[i * 6 + 0].color = vtx_tmp[i * 6 + 1].color = vtx_tmp[i * 6 + 2].color =
          vtx_tmp[i * 6 + 3].color = 0xFF000000 + ((alpha * 256 * 256) & 0x00FF0000) +
                                     ((alpha * 256) & 0x0000FF00) + ((alpha)&0x000000FF);
      vtx_tmp[i * 6 + 0].z = vtx_tmp[i * 6 + 1].z = vtx_tmp[i * 6 + 2].z = vtx_tmp[i * 6 + 3].z =
          4.0f + 0.65f * i;
      vtx_tmp[i * 6 + 0].tu = vtx_tmp[i * 6 + 0].tv = vtx_tmp[i * 6 + 1].tu = vtx_tmp[i * 6 + 1].tv =
          vtx_tmp[i * 6 + 2].tu = vtx_tmp[i * 6 + 2].tv = vtx_tmp[i * 6 + 3].tu = vtx_tmp[i * 6 + 3].tv = 0;
      vtx_tmp[i * 6 + 0].y = vtx_tmp[i * 6 + 3].y = -0.3f + 0.015f * i;
      vtx_tmp[i * 6 + 1].y = vtx_tmp[i * 6 + 2].y = 0;
      x0 = (1.0f / 3.0f - 5.0f) / 20.0f * vtx_tmp[i * 6 + 0].z + 5.0f;
      x0 *= 0.55f;
      vtx_tmp[i * 6 + 0].x = vtx_tmp[i * 6 + 1].x = x0 + 0.10f;
      vtx_tmp[i * 6 + 2].x = vtx_tmp[i * 6 + 3].x = x0 + 0.05f;
  ...
  //信号
  vb_tlight[0].color = vb_tlight[1].color = vb_tlight[2].color = vb_tlight[3].color = 0xFF666666;
  vb_tlight[0].tv = 0;
  vb_tlight[0].tu = 0;
  vb_tlight[1].tv = 0;
  vb_tlight[1].tu = 1;
  ...
  vb_tlight[0].z = vb_tlight[1].z = vb_tlight[2].z = vb_tlight[3].z = 3;

  //街灯
  for (i = 0; i < NUM_LIGHT; i++) {
    //	alpha= (BYTE)( (255/(NUM_GURDPOLL*1.5))*(NUM_GURDPOLL-i) );
    vb_light[i][0].color = vb_light[i][1].color = vb_light[i][2].color = vb_light[i][3].color =
        0xFFFFFFFF; // 0xFF000000 + ((alpha*256*256)&0x00FF0000) + ((alpha*256)&0x0000FF00) +
                    // ((alpha)&0x000000FF);
    vb_light[i][0].tu = 0;
    vb_light[i][0].tv = 0;
    vb_light[i][1].tu = 0;
    ...
    vb_light[i][3].tu = 1;
    vb_light[i][3].tv = 1;
    vb_light[i][0].z = vb_light[i][1].z = vb_light[i][2].z = vb_light[i][3].z = 4.0f + 1.5f * i;
  ...
  vb_brd2a[3].y = 0;
  vb_brd2a[0].z = vb_brd2a[1].z = vb_brd2a[2].z = vb_brd2a[3].z = 6;
  //看板裏
  vb_brd2b[0].color = vb_brd2b[1].color = vb_brd2b[2].color = vb_brd2b[3].color = 0xFF666666;
  vb_brd2b[0].tv = 0;
  vb_brd2b[0].tu = 0;
  vb_brd2b[1].tv = 0;
  vb_brd2b[1].tu = 1;
  ...
  vb_brd2b[3].y = -3 + 0.9f;
  vb_brd2b[0].z = vb_brd2b[1].z = vb_brd2b[2].z = vb_brd2b[3].z = 6;
  for (i = 0; i < 4; i++) {
    vb_brd2a[i].x *= -1;
    vb_brd2b[i].x *= -1;
    vb_brd2a[i].y *= 0.7f;
  ...
  //手前ガードレール
  vb_maegurd[0].color = vb_maegurd[1].color = vb_maegurd[2].color = vb_maegurd[3].color = 0xFFFFFFFF;
  ...
  vb_maegurd[0].z = vb_maegurd[1].z = vb_maegurd[2].z = vb_maegurd[3].z = 1.5f;
  vb_maegurd[0].tv = 0;
  vb_maegurd[0].tu = 0;
  vb_maegurd[1].tv = 0;
  vb_maegurd[1].tu = 1;
  ...
  //手前ガードレール支柱
  vb_maegurdpoll[0].color = vb_maegurdpoll[1].color = vb_maegurdpoll[2].color = vb_maegurdpoll[3].color =
      0xFFEEEEEE;
  ...
  vb_maegurdpoll[0].z = vb_maegurdpoll[1].z = vb_maegurdpoll[2].z = vb_maegurdpoll[3].z = 1.52f;
  ...
  //お空
  vb_sky[0].z = vb_sky[1].z = vb_sky[2].z = vb_sky[3].z = 35;
  vb_sky[0].tu = vb_sky[0].tv = vb_sky[1].tu = vb_sky[1].tv = vb_sky[2].tu = vb_sky[2].tv = vb_sky[3].tu =
      vb_sky[3].tv = 0;
  vb_sky[0].color = vb_sky[1].color = 0xFF000000;
  vb_sky[2].color = vb_sky[3].color = 0xFF333333;
  vb_sky[0].x = -20;
  vb_sky[0].y = -20;
  vb_sky[1].x = 20;
  vb_sky[1].y = -20;
...
void CStage::LoadTextures() //テクスチャ読込み
{
  ...
  D3DXCreateTextureFromFileExA(d3ddev, filename, 0, 0, 0, 0, D3DFMT_A1R5G5B5, D3DPOOL_MANAGED,
                               D3DX_FILTER_POINT, D3DX_FILTER_POINT, 0xFF000000, NULL, NULL, &ptex_moon);

  //街シルエット
  sprintf(filename, "%s\\tex_city.png", GetDLLPath());
  D3DXCreateTextureFromFileExA(d3ddev, filename, 0, 0, 0, 0, D3DFMT_A1R5G5B5, D3DPOOL_MANAGED,
                               D3DX_FILTER_POINT, D3DX_FILTER_POINT, 0xFF000000, NULL, NULL, &ptex_city);
  ...
  D3DXCreateTextureFromFileExA(d3ddev, filename, 0, 0, 0, 0, D3DFMT_X1R5G5B5, D3DPOOL_MANAGED,
                               D3DX_FILTER_POINT, D3DX_FILTER_POINT, 0, NULL, NULL, &ptex_jct);

  //道路
  sprintf(filename, "%s\\tex_road.bmp", GetDLLPath());
  D3DXCreateTextureFromFileA(d3ddev, filename, &ptex_road);

  //ガードレール
  sprintf(filename, "%s\\tex_gurd.bmp", GetDLLPath());
  D3DXCreateTextureFromFileA(d3ddev, filename, &ptex_gurd);

  //看板支柱
  sprintf(filename, "%s\\tex_sicyu.bmp", GetDLLPath());
  D3DXCreateTextureFromFileExA(d3ddev, filename, 0, 0, 0, 0, D3DFMT_A1R5G5B5, D3DPOOL_MANAGED,
                               D3DX_FILTER_POINT, D3DX_FILTER_POINT, 0xFF000000, NULL, NULL, &ptex_brd1a);
  //看板表
  sprintf(filename, "%s\\tex_bord.png", GetDLLPath());
  D3DXCreateTextureFromFileExA(d3ddev, filename, 0, 0, 0, 0, D3DFMT_X1R5G5B5, D3DPOOL_MANAGED,
                               D3DX_FILTER_POINT, D3DX_FILTER_POINT, 0, NULL, NULL, &ptex_brd1b);

  //信号
  sprintf(filename, "%s\\tex_tlight.bmp", GetDLLPath());
  D3DXCreateTextureFromFileExA(d3ddev, filename, 0, 0, 0, 0, D3DFMT_A1R5G5B5, D3DPOOL_MANAGED,
                               D3DX_FILTER_POINT, D3DX_FILTER_POINT, 0xFFFFFFFF, NULL, NULL, &ptex_tlight);

  //街灯
  sprintf(filename, "%s\\tex_light.bmp", GetDLLPath());
  D3DXCreateTextureFromFileExA(d3ddev, filename, 0, 0, 0, 0, D3DFMT_A1R5G5B5, D3DPOOL_MANAGED,
                               D3DX_FILTER_POINT, D3DX_FILTER_POINT, 0xFFFFFFFF, NULL, NULL, &ptex_light);

  //看板裏向き
  sprintf(filename, "%s\\tex_bord2.bmp", GetDLLPath());
  D3DXCreateTextureFromFileExA(d3ddev, filename, 0, 0, 0, 0, D3DFMT_A1R5G5B5, D3DPOOL_MANAGED,
                               D3DX_FILTER_POINT, D3DX_FILTER_POINT, 0xFFFFFFFF, NULL, NULL, &ptex_brd2);
...
void CStage::DrawBacks() {
  ...
  //★
  d3ddev->DrawPrimitiveUP(D3DPT_POINTLIST, NUM_STARS, vb_stars, sizeof(MYVERTEX3D));
}
...
void CStage::DrawJct() {
  ...
  d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vb_maegurd, sizeof(MYVERTEX3D));
  //手前ガードレール支柱
  d3ddev->SetTexture(0, NULL);
  d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vb_maegurdpoll, sizeof(MYVERTEX3D));
  ...
  //看板2
  d3ddev->SetTexture(0, ptex_brd1a);
  d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vb_brd2a, sizeof(MYVERTEX3D));
  d3ddev->SetTexture(0, ptex_brd2);
  d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vb_brd2b, sizeof(MYVERTEX3D));

  //信号
  d3ddev->SetTexture(0, ptex_tlight);
  d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vb_tlight, sizeof(MYVERTEX3D));
}
...
void CStage::DrawRoad() {
  ...
  for (int i = 0; i < NUM_LIGHT; i++) {
    d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vb_light[i], sizeof(MYVERTEX3D));
  }
