...
/*	pbul_rocket = new CRocket(this,aif[ATTACKINFO_ROCKET]);
	...
	if(m_opt_hadou)	pbul_hadou = new CHadouKenB(this);
	else			pbul_hadou = new CHadouKenA(this);
...
void CHadouKenB::act_run()
{
	...
	movex(pdat->vx);
	if(ccounter>60)ChangeAction(CBB_STATE_BOMB);
}

void CHadouKenB::act_bomb()
{
	if(ccounter<8)cell(CELL_HADOU_B7);
	else if(ccounter<16)cell(CELL_HADOU_B8);
	else ChangeAction(CBB_STATE_IDLE);

	float dur = 8.0f;
	float t = (ccounter/dur);

	if(t>1.0f)ChangeAction(CBB_STATE_IDLE);
	else{
...
void CGoodbye::act_base()
{
	...
	if(ccounter<6)	pdat->kougeki=TRUE;
	else			pdat->kougeki=FALSE;
}
