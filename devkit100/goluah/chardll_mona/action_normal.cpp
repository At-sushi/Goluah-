...
/*--------------------------------------------------------------
        ニュートラル
----------------------------------------------------------------*/
void CCharacter::act_neutral() { cell(CELL_NEUTRAL); }

/*--------------------------------------------------------------
        しゃがみ
----------------------------------------------------------------*/
void CCharacter::act_crouch() { cell(CELL_CROUCH); }
...
void CCharacter::act_tojyo() {
  ...
  else if (pdat->counter < 68)
    pdat->cnow = CELL_TOJYO4;
  else if (pdat->counter < 76)
    pdat->cnow = CELL_TOJYO5;
  else if (pdat->counter < 84)
    pdat->cnow = CELL_TOJYO6;
  else {
...
/*--------------------------------------------------------------
        タイムオーバー
----------------------------------------------------------------*/
void CCharacter::act_timeoverlose() { cell(CELL_TLOSE); }
...
void CCharacter::act_jampc() {
  if (pdat->counter == 0) {
    if (m_aerial && m_2nd_step_flag)
      pdat->vy = m_opt_AStep ? -27 : -15;
    else
      pdat->vy = m_opt_AStep ? /*-22 : -23*/ -25 : -27;
    pdat->vx = 0;
  }
