...
void CCharacter::act_att_smc() {
  ...
  if (pdat->counter == 1)
    StartAttack(ATTACKINFO_STAND_MC);
}
...
void CCharacter::act_tatumaki1() {
  ...
  if (pdat->vy > 0)
    pdat->aid = ACTID_TATUMAKIF;
  if (pdat->counter < 8)
    pdat->muteki = FALSE;

  pdat->cnow = CELL_TATUMAKI2 + (pdat->counter % 4);
  if (pdat->counter < 2)
    pdat->cnow = CELL_TATUMAKI1;

  if (pdat->cnow == CELL_TATUMAKI5)
    pdat->revx = TRUE;
  else
    pdat->revx = FALSE;

  if (pdat->cnow == CELL_TATUMAKI3)
    StartAttack(ATTACKINFO_TATUMAKI);
}
...
void CCharacter::act_reikucom() {
  if (pdat->counter < 13) {
    ...
    if (pdat->counter < 3 + 13)
      pdat->cnow = CELL_I2;
    else if (pdat->counter < 20 + 13) {
      pdat->cnow = CELL_I1;
      movex(10);
    } else if (pdat->counter < 25 + 13)
      pdat->cnow = CELL_WALK1;
    else
      pdat->aid = ACTID_NEUTRAL;
    ...
    if (pdat->counter < 3 + 13 + 13)
      pdat->cnow = CELL_KU2;
    else if (pdat->counter < 20 + 13 + 13) {
      pdat->cnow = CELL_KU1;
      movex(10);
    } else if (pdat->counter < 25 + 13 + 13)
      pdat->cnow = CELL_WALK1;
    else
      pdat->aid = ACTID_NEUTRAL;
