...
void CCharacter::act_att_sd() {
  if (ccounter == 0)
    PlayMySound(wave_body_ga);

  if (ccounter == 5)
    StartAttack(ATTACKINFO_STAND_D);
  pdat->z = ZZAHYO_CHARACTER_F;
...
void CCharacter::act_att_se() {
  if (ccounter == 0)
    PlayMySound(wave_kurae);

  if (ccounter == 5)
    StartAttack(ATTACKINFO_STAND_E);
...
void CCharacter::act_att_ja() {
  if (ccounter == 1)
    StartAttack(m_aerial ? ATTACKINFO_AERIAL_A : ATTACKINFO_JAMP_A);
...
void CCharacter::act_att_jb() {
  if (ccounter == 5)
    StartAttack(m_aerial ? ATTACKINFO_AERIAL_B : ATTACKINFO_JAMP_B);
...
void CCharacter::act_att_jc() {
  if (ccounter == 1)
    StartAttack(m_aerial ? ATTACKINFO_AERIAL_C : ATTACKINFO_JAMP_C);
...
void CCharacter::act_nage() {
  ...
  if (pdat->counter < 10 + 10)
    pdat->cnow = CELL_SB1;
  else if (pdat->counter < 14 + 10) {
    pdat->cnow = CELL_SB2;
  } else if (pdat->counter < 18 + 10) {
    pdat->cnow = CELL_SB4;
