...
void CCharacter::act_gcancel1j() { act_tatumaki1(ATTACKINFO_GCANCEL); }
...
void CCharacter::act_gcancel2j() { act_tatumaki2(ATTACKINFO_GCANCEL); }
...
void CCharacter::act_gcancel3j() { act_tatumaki3(ATTACKINFO_GCANCEL); }
...
void CCharacter::act_reiku1a() {
  if (ccounter == 0)
    PlayMySound(wave_reiku_re);

  if (pdat->counter < 20) {
    pdat->cnow = CELL_REIKU_RE1;
    movex(10);
  } else if (pdat->counter < 28)
    pdat->cnow = CELL_REIKU_RE2;
  else if (pdat->counter < 36)
    pdat->cnow = CELL_REIKU_RE3;
  else
...
void CCharacter::act_reiku1b() {
  if (ccounter == 0)
    PlayMySound(wave_reiku_i);

  if (pdat->counter < 20) {
    pdat->cnow = CELL_REIKU_I1;
    movex(10);
  } else if (pdat->counter < 28)
    pdat->cnow = CELL_REIKU_I2;
  else if (pdat->counter < 36)
    pdat->cnow = CELL_REIKU_I3;
  else
    pdat->aid = ACTID_NEUTRAL;
...
void CCharacter::act_reiku1c() {
  if (ccounter == 0)
    PlayMySound(wave_reiku_ku);

  if (pdat->counter < 20) {
    pdat->cnow = CELL_REIKU_KU1;
    movex(10);
  } else if (pdat->counter < 28)
    pdat->cnow = CELL_REIKU_KU2;
  else if (pdat->counter < 36)
    pdat->cnow = CELL_REIKU_KU3;
  else
    pdat->aid = ACTID_NEUTRAL;
...
void CCharacter::act_hadou1a() {
  if (ccounter == 0)
    PlayMySound(wave_hado1);

  UINT anmspd = 1;

  if (ccounter < anmspd * 2)
    cell(CELL_HADOU1);
    ...
    cell(CELL_HADOU6);
  else if (ccounter < anmspd * 10 + 20)
    cell(CELL_HADOU7);
  else
    ChangeAction(ACTID_NEUTRAL);

  if (ccounter == anmspd * 6) {
    pbul_hadou->GoA(pdat->muki, pdat->x + zurex(70), -50, 4, &aif[ATTACKINFO_HADOU1A]);
  }
}
...
void CCharacter::act_hadou2a() {
  ...
  if (ccounter < anmspd * 2)
    ...
    cell(CELL_HADOU6);
  else if (ccounter < anmspd * 10 + 30)
    cell(CELL_HADOU7);
  else
    ChangeAction(ACTID_NEUTRAL);

  if (ccounter == anmspd * 6) {
    pbul_hadou->GoA(pdat->muki, pdat->x + zurex(70), -50, 5, &aif[ATTACKINFO_HADOU2A]);
  }
}
...
void CCharacter::act_hadou3a() {
  ...
  if (ccounter < anmspd * 2)
    ...
    cell(CELL_HADOU6);
  else if (ccounter < anmspd * 10 + 40)
    cell(CELL_HADOU7);
  else
    ChangeAction(ACTID_NEUTRAL);

  if (ccounter == anmspd * 6) {
    pbul_hadou->GoA(pdat->muki, pdat->x + zurex(70), -50, 6, &aif[ATTACKINFO_HADOU3A]);
  }
}
...
void CCharacter::act_hadou1b() {
  ...
  if (ccounter == anmspd * 8) {
    pbul_hadou->GoB(pdat->muki, pdat->x + zurex(100), -50, 12, &aif[ATTACKINFO_HADOU1B]);
  }
}
...
void CCharacter::act_hadou2b() {
  ...
  if (ccounter == anmspd * 8) {
    pbul_hadou->GoB(pdat->muki, pdat->x + zurex(100), -50, 16, &aif[ATTACKINFO_HADOU2B]);
  }
}
...
void CCharacter::act_hadou3b() {
  ...
  if (ccounter == anmspd * 8) {
    pbul_hadou->GoB(pdat->muki, pdat->x + zurex(100), -50, 20, &aif[ATTACKINFO_HADOU3B]);
  }
}
...
void CCharacter::act_goodbye2() {
  ...
  if (ccounter == 9) {
    pbul_goodbye->GoA(pdat->muki, pdat->x + zurex(80), 0, 12, &aif[ATTACKINFO_GOODBYE2]);
