...
DWORD CCharacter::Message(DWORD msg, LPVOID dat, DWORD prm) {
  ...
  case GOBJMSG_HADOUHIT:
    AddPowerGauge(0.10);
    break;
  case GOBJMSG_HADOUGUARD:
    AddPowerGauge(0.05);
    break;
...
BOOL CCharacter::Command_Hissatu(DWORD key) {
  ...
  } else                         //地上必殺技のコマンド判定
  {
    ...
    if (TRUE) { // OptReiku()){
      if (com214(30)) {
        if (key & KEYSTA_BC2) {
          pdat->aid = ACTID_REIKU3A;
          return TRUE;
        } else if (key & KEYSTA_BB2) {
          pdat->aid = ACTID_REIKU2A;
          return TRUE;
        } else if (key & KEYSTA_BA2) {
          pdat->aid = ACTID_REIKU1A;
          return TRUE;
        }
      }
    }
    if (m_opt_hadou) {
      ...
      if (pbul_hadou->isReady() && com236(30)) {
        if (key & KEYSTA_BC2) {
          pdat->aid = ACTID_HADOU3B;
          return TRUE;
        } else if (key & KEYSTA_BB2) {
          pdat->aid = ACTID_HADOU2B;
          return TRUE;
        } else if (key & KEYSTA_BA2) {
          pdat->aid = ACTID_HADOU1B;
          return TRUE;
        }
      }
    } else {
      ...
      if (pbul_hadou->isReady() && com236(30)) {
        if (key & KEYSTA_BC2) {
          pdat->aid = ACTID_HADOU3A;
          return TRUE;
        } else if (key & KEYSTA_BB2) {
          pdat->aid = ACTID_HADOU2A;
          return TRUE;
        } else if (key & KEYSTA_BA2) {
          pdat->aid = ACTID_HADOU1A;
          return TRUE;
        }
      }
    }
...
BOOL CCharacter::Command_Normal(DWORD key) {
  ...
  if (key & KEYSTA_UP) {
    ...
    if (key & KEYSTA_BACK && pdat->aid == ACTID_DASHB)
      return TRUE; //ダッシュ継続
  ...
  } else //地上動作中
  {
    ...
    if (m_opt_exAttack) {
      ...
      if ((key & KEYSTA_BC2) && (key & KEYSTA_FOWORD)) {
        ChangeAction(ACTID_ATT_SE);
        return TRUE;
      }

      //レバー前入れ中
      if ((key & KEYSTA_BB2) && (key & KEYSTA_FOWORD)) {
        ChangeAction(ACTID_ATT_SD);
        return TRUE;
      }
    }
  }
...
BOOL CCharacter::Command_OnAttacking(DWORD key) {
  if (pdat->aid & ACTID_KUCYU) {
    if (m_opt_AAttack) {
      //空中チェーン
      if (key & KEYSTA_BC2) {
        if (ChainCombo(CHAIN_JC)) {
          pdat->aid = ACTID_ATT_JC;
          return TRUE;
        }
      } else if (key & KEYSTA_BB2) {
        if (ChainCombo(CHAIN_JB)) {
          pdat->aid = ACTID_ATT_JB;
          return TRUE;
        }
      } else if (key & KEYSTA_BA2) {
        if (ChainCombo(CHAIN_JA)) {
  ...
  if ((pdat->aid & ACTID_GUARD) && m_opt_gcancel) {
    if (!(pdat->aid & ACTID_KUCYU)) {
      ...
      if (com623(30) && pdat->gauge > NEEDGAUGE_GC) {
        if (key & KEYSTA_BC2) {
          pdat->aid = ACTID_GCANCEL3;
          return TRUE;
        } else if (key & KEYSTA_BB2) {
          pdat->aid = ACTID_GCANCEL2;
          return TRUE;
        } else if (key & KEYSTA_BA2) {
          pdat->aid = ACTID_GCANCEL1;
          return TRUE;
        }
      }
    }
...
DWORD CCharacter::CommandCOM(DWORD wid) {
  if (wid == ACTID_GOODBYE1 && pdat->gauge < 1.0f)
  ...
  //レイク
  if (wid == ACTID_REIKU1B)
    return (pdat->aid == ACTID_REIKU1A) ? TRUE : FALSE;
  if (wid == ACTID_REIKU1C)
    return (pdat->aid == ACTID_REIKU1B) ? TRUE : FALSE;
  if (wid == ACTID_REIKU2B)
    return (pdat->aid == ACTID_REIKU2A) ? TRUE : FALSE;
  if (wid == ACTID_REIKU2C)
    return (pdat->aid == ACTID_REIKU2B) ? TRUE : FALSE;
  if (wid == ACTID_REIKU3B)
    return (pdat->aid == ACTID_REIKU3A) ? TRUE : FALSE;
  if (wid == ACTID_REIKU3C)
    return (pdat->aid == ACTID_REIKU3B) ? TRUE : FALSE;
...
DWORD CCharacter::CmdCom_OnNormal(DWORD wid) {
  ...
  if (!(pdat->aid & ACTID_KUCYU) && (m_opt_exAttack)) {
    ...
    if (GetGObject()->aid & ACTID_ATTACK) {
      switch (wid) {
      case ACTID_ATT_SE:
        return ChainCombo(CHAIN_SE);
        break;
      case ACTID_ATT_SD:
        return ChainCombo(CHAIN_SD);
        break;
      }
    }
  }
...
DWORD CCharacter::TouchA(ATTACKINFO *info, DWORD ta_eid) {
  ...
  //竜巻旋風脚
  if ((aid == ACTID_TATUMAKI1) && (cntr < 10))
    return TOUCHA_MUSI2;
  if ((aid == ACTID_TATUMAKI2) && (cntr < 20))
    return TOUCHA_MUSI2;
  if ((aid == ACTID_TATUMAKI3) && (cntr < 30))
    return TOUCHA_MUSI2;

  //レバー入れ攻撃
  if (aid == ACTID_ATT_SD) {
    if (pdat->cnow == CELL_SD2 || pdat->cnow == CELL_SD3)
      return TOUCHA_MUSI2;
  }
  if (aid == ACTID_ATT_SE) {
    if (pdat->cnow == CELL_SE3)
      return TOUCHA_MUSI2;
  }
  ...
  if (IsLocalCom()) {
    ...
    case ACTID_ATT_SB: {
      if (ComLevelCk(5) && rand() % 2) {
        if (ChainCheck(CHAIN_SD))
          SetComAct(ACTID_ATT_SD, 5);
        else if (ChainCheck(CHAIN_SE))
          SetComAct(ACTID_ATT_SE, 5);
        else
    ...
    case ACTID_ATT_SC: {
      if (ComLevelCk(5) && pdat->gauge >= 1.0f) {
        ...
        if (ChainCheck(CHAIN_SD))
          SetComAct(ACTID_ATT_SD, 8);
        else if (ChainCheck(CHAIN_SE))
          SetComAct(ACTID_ATT_SE, 8);
        else
          SetComAct(ACTID_REIKU1C, 5);
      }
    } break;
    ...
    case ACTID_ATT_SD: {
      if (ComLevelCk(5) && pdat->gauge >= 1.0f) {
        ...
        if (ChainCheck(CHAIN_SE))
          SetComAct(ACTID_ATT_SE, 8);
        else
          SetComAct(ACTID_REIKU1C, 8);
      }
    } break;
    case ACTID_ATT_SE: {
      if (ComLevelCk(5) && pdat->gauge >= 1.0f) {
    ...
    case ACTID_ATT_CB: {
      if (ComLevelCk(4)) {
        if (ChainCheck(CHAIN_SD))
          SetComAct(ACTID_ATT_SD, 3);
        else if (ChainCheck(CHAIN_SE))
          SetComAct(ACTID_ATT_SE, 3);
        else
          SetComAct(ACTID_REIKU1C, 5);
      }
    } break;
...
DWORD CCharacter::Action() {
  ...
  case ACTID_ATT_SE:
    ...
    ChainCombo(CHAIN_SE);
    break;
  case ACTID_TATUMAKI1:
    act_tatumaki1(ATTACKINFO_TATUMAKI1);
    break; //竜巻(弱)
  case ACTID_TATUMAKI2:
    act_tatumaki2(ATTACKINFO_TATUMAKI2);
    break; //竜巻(中)
  case ACTID_TATUMAKI3:
    act_tatumaki3(ATTACKINFO_TATUMAKI3);
    break; //竜巻(強)
  case ACTID_TATUMAKIR:
    act_tatumakir();
    break; //竜巻落下
  case ACTID_LANDING:
    act_landing();
  ...
  case ACTID_GCANCEL3:
    act_gcancel3();
    break; //ガードキャンセル
  case ACTID_GCANCEL1J:
    act_gcancel1j();
    break; //ガードキャンセル
  case ACTID_GCANCEL2J:
    act_gcancel2j();
    break; //ガードキャンセル
  case ACTID_GCANCEL3J:
    act_gcancel3j();
    break; //ガードキャンセル
  case ACTID_2ND_STEP:
    act_2nd_step();
  ...
  case ACTID_STRIKER_IN:
    act_striker_in();
    break; //支援攻撃・画面内に入ってくる
  case ACTID_STRIKER_DASH:
    act_striker_dash();
    break; //支援攻撃・相手のほうにちょっと進む
...
BOOL CCharacter::EnemyAction(DWORD eid, DWORD hid) {
  ...
  switch (hid) {
  case USERHIT_AERIALPOP:
    eact_aerialpop(pedat);
    break;
  case USERHIT_AERIAL1:
    eact_aerialhit1(pedat);
    break;
  case USERHIT_AERIAL2:
    eact_aerialhit2(pedat);
    break;
  case USERHIT_AERIAL3:
    eact_aerialhit3(pedat);
    break;
  default:
...
void CCharacter::InitAttackInfo() {
  ...
  aif[i].hit = HITINFO_REACT3A | HITINFO_STOP | HITINFO_MARK4 | HITINFO_SNDHIT3 | HITINFO_EFCTSINDO |
               HITINFO_EFCTFLASH;                                                           //ヒット情報
  ...
  aif[i].hit =
      HITINFO_FUTTOBI2 | HITINFO_STOP | HITINFO_MARK4 | HITINFO_SNDHIT3 | HITINFO_EFCTSINDO; //ヒット情報
  aif[i].guard = GUARDINFO_REACT3 | GUARDINFO_SIV3 | GUARDINFO_XJAMP | GUARDINFO_EFCTSINDO;  //ガード情報
  ...
  aif[i].hit = HITINFO_EFCTBURN_B | HITINFO_EFCTSINDO | HITINFO_SIV1 | HITINFO_FUTTOBI | HITINFO_MARK4 |
               HITINFO_SNDHIT3;                     //ヒット情報
  aif[i].guard = GUARDINFO_REACT3 | GUARDINFO_SIV1; //ガード情報
  aif[i].id = 0;                                    //ゲージ増加時に使用するID
  aif[i].damage = 0;                                //ダメージ
...
void CCharacter::EndAttack() { GetGObject()->kougeki = FALSE; }
