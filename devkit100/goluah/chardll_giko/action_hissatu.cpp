...
void CCharacter::act_atemi1() {
  if (ccounter == 0)
    PlayMySound(wave_atemi);
  cell(CELL_ATEMI_A);
  if (ccounter > 30)
    ChangeAction(ACTID_ATEMI_SUKI);
}
...
void CCharacter::act_atemi_nage() {
  ...
  if (ccounter < 4)
    cell(CELL_ATEMI_NAGE1);
  else if (ccounter < 8)
    cell(CELL_ATEMI_NAGE2);
  else if (ccounter < 12)
    cell(CELL_ATEMI_NAGE3);
  else if (ccounter < 20)
    cell(CELL_ATEMI_NAGE4);
  else
    ChangeAction(ACTID_NEUTRAL);
}
...
DWORD CCharacter::eact_atemi_nage(GOBJECT *pedat) {
  ...
  if (pedat->counter < 20) {
    if (pedat->counter < 4)
      rot = 45;
    else if (pedat->counter < 8)
      rot = 90;
    else if (pedat->counter < 12)
      rot = 135;
    else if (pedat->counter < 20)
      rot = 180;
  ...
  /*	if(pedat->counter < 10){
     ...
     ); pedat->y=-60 - 80*sin( (pedat->counter-20)*12/180.0*3.1415  - 3.1415/4.0  ); pedat->rot =
     (pedat->counter-20)*9; return(0);
          }*/
  else {
