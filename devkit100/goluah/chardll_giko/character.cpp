...
CHARACTER_LOAD_OPTION option[] = {
	...
	{ OPTIONS_HEAVY_HIT			,OPTIONS_IRON_SKIN		,0, "Heavy Hit"			, 7 },
	{ OPTIONS_IRON_SKIN			,OPTIONS_HEAVY_HIT		,0,	"Iron Skin"			, 7 },
...
CCharacterInfo CharInfo("ギコ",		// キャラクターの名前（最大31バイト）
...
void CCharacter::InitParameters()
{
	...
	BOOL iron		= (option_flags&OPTIONS_IRON_SKIN		) ? TRUE : FALSE;
...
{
	if(wid==ACTID_CYOHI && pdat->gauge<1.0f)return FALSE;//ゲージチェック
...
void CCharacter::InitAttackInfo()
{
	...
	i=ATTACKINFO_CYOHI;
	aif[i].hit		= HITINFO_MARK4 | HITINFO_USER2(USERHIT_ROTFUTTOBI) | HITINFO_SNDHIT3 | HITINFO_EFCTSINDO;//ヒット情報
