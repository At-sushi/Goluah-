...
void CCharacter::act_att_cb() {
  ...
  if (ccounter < anmspd)
    ...
    cell(CELL_CB8);
  else if (ccounter < anmspd * 8 + anmspd2 * 1)
    cell(CELL_CB7);
  else if (ccounter < anmspd * 8 + anmspd2 * 2)
    cell(CELL_CB6);
  else if (ccounter < anmspd * 8 + anmspd2 * 3)
    cell(CELL_CB5);
  else if (ccounter < anmspd * 8 + anmspd2 * 4)
    cell(CELL_CB4);
  else if (ccounter < anmspd * 8 + anmspd2 * 5)
    cell(CELL_CB3);
  else if (ccounter < anmspd * 8 + anmspd2 * 6)
    cell(CELL_CB2);
  else if (ccounter < anmspd * 8 + anmspd2 * 7)
    cell(CELL_CB1);
  else
...
void CCharacter::act_att_cc() {
  ...
  if (ccounter < anmspd)
    ...
    cell(CELL_CC6);
  else if (ccounter < anmspd * 6 + anmspd2 * 1)
    cell(CELL_CC6);
  else if (ccounter < anmspd * 6 + anmspd2 * 2)
    cell(CELL_CC5);
  else if (ccounter < anmspd * 6 + anmspd2 * 3)
    cell(CELL_CC4);
  else if (ccounter < anmspd * 6 + anmspd2 * 4)
    cell(CELL_CC3);
  else if (ccounter < anmspd * 6 + anmspd2 * 5)
    cell(CELL_CC2);
  else if (ccounter < anmspd * 6 + anmspd2 * 6)
    cell(CELL_CC1);
  else
    ChangeAction(ACTID_CROUCH);
}
...
void CCharacter::act_nage() {
  ...
  if (pdat->counter < offset)
    pdat->cnow = CELL_NAGE1;
  else if (pdat->counter < offset + 25) {
    // if(pdat->muki)
    {
      if (ccounter < offset + 2)
        pdat->cnow = CELL_NAGE2;
      else if (ccounter < offset + 15)
        pdat->cnow = CELL_NAGE3;
      else if (ccounter < offset + 17)
        pdat->cnow = CELL_NAGE4;
      else if (ccounter < offset + 19)
        pdat->cnow = CELL_NAGE5;
      else if (ccounter < offset + 21)
        pdat->cnow = CELL_NAGE6;
      else if (ccounter < offset + 23)
        pdat->cnow = CELL_NAGE7;
      else if (ccounter < offset + 25)
        pdat->cnow = CELL_NAGE8;
    }
  } else
...
void CCharacter::act_kiriget() {
  if (ccounter == 0)
    PlayMySound(wave_kiriget);
  if (ccounter == 0)
    ComDelay(22);

  if (ccounter == 0) {
    if (efe_wind) {
      efe_wind->Go(pdat->muki, pdat->x, 0, 2);
    }
    pdat->vx = 15;
...
void CCharacter::act_sagena() {
  ...
  if (pdat->counter < 6)
    ...
    movex(5);
    if (pdat->counter < 9)
      pdat->cnow = CELL_SAGE1;
    else if (pdat->counter < 12) {
      pdat->cnow = CELL_SAGE2;
    } else if (pdat->counter < 15) {
      pdat->cnow = CELL_SAGE3;
    } else if (pdat->counter < 18) {
      pdat->cnow = CELL_SAGE4;
    } else if (pdat->counter < 21) {
      pdat->cnow = CELL_SAGE5;
    } else if (pdat->counter < 24) {
      pdat->cnow = CELL_SAGE6;
    } else {
      pdat->cnow = CELL_SAGE7;
    }
  } else
    pdat->aid = ACTID_SAGENASUKI;
}

void CCharacter::act_sagenasuki() {
  if (pdat->counter < 18) {
    pdat->cnow = CELL_SAGE8;
  } else
    pdat->aid = ACTID_NEUTRAL;
}
...
void CCharacter::act_striker_chainsaw() {
  if (ccounter < 3)
    ...
    cell(CELL_SC4 + ccounter % 4);
    if (ccounter % 5 == 0)
      StartAttack(ATTACKINFO_STRIKERCHAIN);
  } else if (ccounter < 25)
    cell(CELL_SC3);
  else if (ccounter < 30)
    cell(CELL_SC2);
    ...
    ChangeAction(ACTID_STRIKER_OUT);
}
