...
void CCharacter::act_att_jc() //飛強
{
  ...
  if (pdat->counter < 9)
    ...
    pdat->cnow = CELL_JC1 + pdat->counter % 8;
  else
    pdat->cnow = CELL_JAMP;
  JiyuuRakka(2, TRUE);
}
...
void CCharacter::act_staff() {
  ...
  if (pdat->counter < anmspd)
    pdat->cnow = CELL_STAFF7;
  else if (pdat->counter < anmspd * 2)
    pdat->cnow = CELL_STAFF6;
  else if (pdat->counter < anmspd * 3)
    pdat->cnow = CELL_STAFF5;
  else if (pdat->counter < anmspd * 4)
    pdat->cnow = CELL_STAFF4;
  else if (pdat->counter < anmspd * 5)
    pdat->cnow = CELL_STAFF3;
  else if (pdat->counter < anmspd * 6)
    pdat->cnow = CELL_STAFF2;
  else if (pdat->counter < anmspd * 7)
    pdat->cnow = CELL_STAFF1;
  else if (pdat->counter < anmspd * 8)
    pdat->cnow = CELL_STAFF2;
  else if (pdat->counter < anmspd * 9)
    pdat->cnow = CELL_STAFF3;
  else if (pdat->counter < anmspd * 10)
    pdat->cnow = CELL_STAFF4;
  else if (pdat->counter < anmspd * 11)
    pdat->cnow = CELL_STAFF5;
  else if (pdat->counter < anmspd * 12)
    pdat->cnow = CELL_STAFF6;
  else
    pdat->cnow = CELL_STAFF7;
}
...
void CCharacter::act_reversal() {
  ...
  if (pdat->counter % 3 == 0)
    StartAttack(ATTACKINFO_REVERSAL);
  ...
  if (pdat->counter < 6)
    ...
    pdat->cnow = CELL_REVERSAL1 + pdat->counter % 8;
  else
    pdat->cnow = CELL_JAMP;
