...
void CCharacter::InitBullets()
{
	...
	if(m_opt_quickB)	pbul_moyaB->Quick();
	if(m_opt_quickC)	pbul_moyaC->Quick();
	if(m_opt_crazy)
	{
		pbul_moyaA->Quick();
		pbul_moyaB->Quick();
...
void CCharacter::ArekoreRelease()
{
	...
	switch(type)
	{
	case Moya_Ocya:		ChangeAction( ACTID_POI_OCYA );			break;
	case Moya_Jien:		ChangeAction( ACTID_POI_JIEN );			break;
	case Moya_Aroe:		ChangeAction( ACTID_POI_AROE );			break;
	case Moya_MissileH:	ChangeAction( ACTID_POI_MISSILE_H );	break;
	case Moya_Moukoneeyo:ChangeAction( ACTID_POI_MOUKONEEYO );	break;
	case Moya_MissileSP:ChangeAction( ACTID_POI_MISSILE_SP );	break;
	case Moya_Handg:	ChangeAction( ACTID_POI_HANDG );		break;
	case Moya_Craymore:	ChangeAction( ACTID_POI_CRAYMORE );		break;
	case Moya_MissileV:	ChangeAction( ACTID_POI_MISSILE_V );	break;
	case Moya_FireThrow:ChangeAction( ACTID_POI_FIRETHROW );	break;
	case Moya_Shot:		ChangeAction( ACTID_POI_SHOT );			break;
	case Moya_Rifle:	ChangeAction( ACTID_POI_RIFLE );		break;
	case Moya_Rocket:	ChangeAction( ACTID_POI_ROCKET );		break;
	case Moya_Lantern:	ChangeAction( ACTID_POI_LANTERN );		break;
	case Moya_Hellfire:	ChangeAction( ACTID_POI_HELLFIRE );		break;
	case Moya_HttpLaser:ChangeAction( ACTID_POI_HTTPLASER );	break;
	case Moya_Eraser99:	ChangeAction( ACTID_POI_ERASER99 );		break;
	default:
		ChangeAction( ACTID_NEUTRAL );			break;
	}
}
...
void COcya::act_bomb()
{
	if(ccounter<3)cell( CELL_OCYAD1 );
	else if(ccounter<6)cell( CELL_OCYAD2 );
	else if(ccounter<9)cell( CELL_OCYAD3 );
	else Hide();
}
...
void CJien::act_bomb()
{
	if(ccounter<2)cell( CELL_JISAKUD1 );
	else if(ccounter<4)cell( CELL_JISAKUD2 );
	else if(ccounter<6)cell( CELL_JISAKUD3 );
	else if(ccounter<8)cell( CELL_JISAKUD4 );
	else if(ccounter<10)cell( CELL_JISAKUD5 );
	else if(ccounter<12)cell( CELL_JISAKUD6 );
	else Hide();
}
...
void CAroe::act_bomb()
{
	if(ccounter<3)cell( CELL_AROED1 );
	else if(ccounter<6)cell( CELL_AROED2 );
	else if(ccounter<9)cell( CELL_AROED3 );
	else Hide();
}
...
void CMissile::act_run()
{
	...
	if(vxc<0){pdat->muki = TRUE;vxc*=-1.0;}
...
void CMissile::act_bomb()
{
	...
	if(ccounter<1)cell( CELL_BOMB1 );
	else if(ccounter<2)cell( CELL_BOMB2 );
	else if(ccounter<8)	{cell( CELL_BOMB3 );pdat->y+=pdat->vy;}
	else if(ccounter<14){cell( CELL_BOMB4 );pdat->y+=pdat->vy;}
	else if(ccounter<20){cell( CELL_BOMB5 );pdat->y+=pdat->vy;}
	else if(ccounter<26){cell( CELL_BOMB6 );pdat->y+=pdat->vy;}
	else Hide();
}
...
void CHandg::act_run()
{
	...
	if(pdat->y > 0)
	{
		...
		if(pdat->vx <0)pdat->vx *= -1.0;
		if(rand()%3==0)pdat->vx *= -1.0;
	}
	if(pdat->x < -640.0f)
	{
		pdat->vx *= -1.0f;
...
void CCraymore::act_wait()
{
	...
	if(ccounter>CRAYMORE_TIMEOUT)ChangeAction( CRAYMORE_STATE_ATTACK );
}
...
void CCraymore::act_atk()
{
	...
	if(ccounter%2)pdat->kougeki = TRUE;
	...
	if(ccounter<3 )cell( CELL_CRAYMORED1 );
	else if(ccounter<6 )cell( CELL_CRAYMORED2 );
	else if(ccounter<9 )cell( CELL_CRAYMORED3 );
	else if(ccounter<12 )cell( CELL_CRAYMORED4 );
	else Bomb();
}
...
DWORD CCraymore::Action()
{
	switch( pdat->aid )
	{
	case CRAYMORE_STATE_WAIT:	act_wait();	break;
	case CRAYMORE_STATE_ATTACK:	act_atk();	break;
	default:return CBulletBase::Action();
	}
	return TRUE;
}
...
{
	if(pdat->aid!=CRAYMORE_STATE_ATTACK)ChangeAction( CRAYMORE_STATE_ATTACK );
	return TRUE;
}
...
DWORD CFireThrower::TouchC( ATTACKINFO *info,DWORD tc_eid )
{
	...
	else		info->hit &= ~HITINFO_EFCTBURN;
...
/*	const UINT rnum=3;
	...
	if(ccounter<10)pdat->magx=0.1f*ccounter;
	else pdat->magx = 1;
}
...
void CLantern::act_agi()
{
	...
	if(ccounter>60)ChangeAction( ACT_LANTERN_DISAPPEAR );
}
...
void CLantern::act_maha_agi()
{
	...
	else ChangeAction( ACT_LANTERN_DISAPPEAR );
}
...
DWORD CLantern::Action()
{
	switch(pdat->aid)
	{
	case ACT_LANTERN_AGI		:act_agi();			break;
	case ACT_LANTERN_MAHA_AGI	:act_maha_agi();	break;
	case ACT_LANTERN_APPEAR		:act_appear();		break;
	case ACT_LANTERN_DISAPPEAR	:act_disappear();	break;
	default:
...
CComeon::CComeon(CCharacterBase* parent) : CBulletBase(parent)//CEffectBase(parent)
{
	base_z = ZZAHYO_BULLET3;
	RemoveProperty( GOBJFLG_ATTACK );
}
...
DWORD CMoyamoya::Draw()
{
	if(pdat->aid!=CBB_STATE_IDLE)
	{
		...
		if(cellno!=0)
		{
			alpha = (glow<MOYA_CH_VAL) ? (DWORD)alpha_base : (DWORD)( alpha_base*(1.0f-(glow-MOYA_CH_VAL)/(1.0f-MOYA_CH_VAL)) );
			SetDWAlpha( pdat->color, alpha );
			pdat->cnow = cellno;
			CellDraw( pdat );
		}
...
DWORD CMoyamoya::Type2Cell(MoyaType type)
{
	switch(type)
	{
	case Moya_Ocya:		return CELL_FUKI_OCYA;
	case Moya_Jien:		return CELL_FUKI_JIEN;
	case Moya_Aroe:		return CELL_FUKI_AROE;
	case Moya_Moukoneeyo:return CELL_FUKI_MOUKONEEYO;
	case Moya_MissileH:	return CELL_FUKI_MISSILE_H;
	case Moya_MissileSP:return CELL_FUKI_MISSILE_SP;
	case Moya_Handg:	return CELL_FUKI_HANDG;
	case Moya_Craymore:	return CELL_FUKI_CRAYMORE;
	case Moya_MissileV:	return CELL_FUKI_MISSILE_V;
	case Moya_FireThrow:return CELL_FUKI_FIRETHROW;
	case Moya_Shot:		return CELL_FUKI_SHOT;
	case Moya_Rifle:	return CELL_FUKI_RIFLE;
	case Moya_Rocket:	return CELL_FUKI_ROCKET;
	case Moya_Lantern:	return CELL_FUKI_LANTERN;
	case Moya_Hellfire:	return CELL_FUKI_HELLFIRE;
	case Moya_HttpLaser:return CELL_FUKI_LASER;
	case Moya_Eraser99:	return CELL_FUKI_ERASER;
	default:			return 0;
	}
}
...
MoyaType CMoyamoyaA::GetNextType()
{
	switch(crnt_type)
	{
	case Moya_Ocya:			return Moya_Jien;
	case Moya_Jien:			return Moya_Aroe;
	case Moya_Aroe:			return Moya_Moukoneeyo;
	case Moya_Moukoneeyo:	return Moya_MissileH;
	case Moya_MissileH:
		{
			if(!m_2ndFlag)
			{
		...
		break;
	case Moya_MissileSP:	return GetFirstType();
	}
...
MoyaType CMoyamoyaB::GetNextType()
{
	switch(crnt_type)
	{
	case Moya_Handg:		return Moya_Craymore;
	case Moya_Craymore:		return Moya_Lantern;
	case Moya_Lantern:		return Moya_FireThrow;
	case Moya_FireThrow:
		{
			if(!m_2ndFlag)
			{
		...
		break;
	case Moya_Hellfire:	return GetFirstType();
	}
	return Moya_Invalid;
}
...
MoyaType CMoyamoyaC::GetNextType()
{
	switch(crnt_type)
	{
	case Moya_Shot:		return Moya_Rifle;
	case Moya_Rifle:	return Moya_Rocket;
	case Moya_Rocket:	return Moya_MissileV;
	case Moya_MissileV:	return Moya_HttpLaser;
	case Moya_HttpLaser:
		{
			if(!m_2ndFlag)
			{
		...
		break;
	case Moya_Eraser99:	return GetFirstType();
	}
	return Moya_Invalid;
}
...
void CBlood::act_run()
{
	if(ccounter < 5)cell( CELL_BLOOD1 );
	else if(ccounter < 10)cell( CELL_BLOOD2 );
	else if(ccounter < 15)cell( CELL_BLOOD3 );
	else if(ccounter < 20)cell( CELL_BLOOD4 );
	else if(ccounter < 25)cell( CELL_BLOOD5 );
	else if(ccounter < 30)cell( 0 );
	else Hide();
}

void CBlood::act_bomb()
{
	Hide();
}
