...
void CCharacter::act_tojyo() //登場
{
  ...
  DWORD spd = 3;
  DWORD cyc = spd * 8;
  if (pdat->counter < spd * 1)
    pdat->cnow = CELL_TOJYO1;
  else if (pdat->counter < spd * 2)
    pdat->cnow = CELL_TOJYO2;
  else if (pdat->counter < spd * 3)
    pdat->cnow = CELL_TOJYO3;
  else if (pdat->counter < spd * 4)
    pdat->cnow = CELL_TOJYO4;
  else if (pdat->counter < spd * 5) {
    pdat->cnow = CELL_TOJYO5;
  } else if (pdat->counter < spd * 6)
    pdat->cnow = CELL_TOJYO4;
  else if (pdat->counter < spd * 7)
    pdat->cnow = CELL_TOJYO3;
  else if (pdat->counter < spd * 8)
    pdat->cnow = CELL_TOJYO2;
  else if (pdat->counter < cyc + spd * 1)
    pdat->cnow = CELL_TOJYO1;
  else if (pdat->counter < cyc + spd * 2)
    pdat->cnow = CELL_TOJYO2;
  else if (pdat->counter < cyc + spd * 3)
    pdat->cnow = CELL_TOJYO3;
  else if (pdat->counter < cyc + spd * 4)
    pdat->cnow = CELL_TOJYO4;
  else if (pdat->counter < cyc + spd * 5) {
    pdat->cnow = CELL_TOJYO5;
  } else if (pdat->counter < cyc + spd * 6)
    pdat->cnow = CELL_TOJYO4;
  else if (pdat->counter < cyc + spd * 7)
    pdat->cnow = CELL_TOJYO3;
  else if (pdat->counter < cyc + spd * 8)
    pdat->cnow = CELL_TOJYO2;
  else if (pdat->counter < cyc * 2 + spd * 1)
    pdat->cnow = CELL_TOJYO1;
  else if (pdat->counter < cyc * 2 + spd * 2)
    pdat->cnow = CELL_TOJYO2;
  else if (pdat->counter < cyc * 2 + spd * 3)
    pdat->cnow = CELL_TOJYO3;
  else if (pdat->counter < cyc * 2 + spd * 4)
    pdat->cnow = CELL_TOJYO4;
  else if (pdat->counter < cyc * 2 + spd * 5)
    pdat->cnow = CELL_TOJYO5;
  else if (pdat->counter < cyc * 2 + spd * 6)
    pdat->cnow = CELL_TOJYO4;
  else if (pdat->counter < cyc * 2 + spd * 7)
    pdat->cnow = CELL_TOJYO3;
  else if (pdat->counter < cyc * 2 + spd * 8 + 20)
    pdat->cnow = CELL_TOJYO2;
  else {
  ...
  if (pdat->counter == spd * 5)
    AddEffect(EFCTID_MARK2, (int)pdat->x + zurex(70), -70, 0);
  if (pdat->counter == spd * 5 + cyc)
    AddEffect(EFCTID_MARK2, (int)pdat->x + zurex(70), -70, 0);
  if (pdat->counter == spd * 5 + cyc * 2)
    AddEffect(EFCTID_MARK2, (int)pdat->x + zurex(70), -70, 0);
}
...
{
  if (pdat->counter < 1)
    pdat->cnow = CELL_JAMPS1;
  else if (pdat->counter < 2)
    pdat->cnow = CELL_JAMPS2;
  else
