...
BOOL CCharacter::Command_Hissatu(DWORD key_now) {
  ...
  } else                         //地上必殺技のコマンド判定
  {
    ...
    if (com214(30)) {
      if (key_now & KEYSTA_BC2) {
        pdat->aid = ACTID_TETU3;
        return TRUE;
      } else if (key_now & KEYSTA_BB2) {
        pdat->aid = ACTID_TETU2;
        return TRUE;
      } else if (key_now & KEYSTA_BA2) {
        pdat->aid = ACTID_TETU1;
        return TRUE;
      }
    }
    if (com426(45)) {
...
BOOL CCharacter::Command_OnNormal(DWORD keyinfo) {
  ...
  if (pdat->aid & ACTID_KUCYU) { ////ジャンプ動作中
    ...
    if (keyinfo & KEYSTA_FOWORD && keyinfo & KEYSTA_BC2 && NageHantei(MAAI_NAGE) &&
        ObjCatch(pdat->eid, GOBJMSG_NAGE1)) {
...
DWORD CCharacter::CommandCOM(DWORD wid) { return CCharacterBase::CommandCOM(wid); }

DWORD CCharacter::CmdCom_OnSystem(DWORD wid) { return CCharacterBase::CmdCom_OnSystem(wid); }
