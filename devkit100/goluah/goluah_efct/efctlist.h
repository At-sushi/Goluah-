...
#ifdef GOLUAH_EFCT_EXPORTS
#define GEFCT_EXPORT __declspec(dllexport)
#else
#define GEFCT_EXPORT __declspec(dllimport)
#endif
 ...
 *	@ingroup Battle
 */
class GEFCT_EXPORT CEffectList {
public:
  CEffectList();
  ~CEffectList() { Destroy(); }
  ...
  static DWORD
  EfctObjectMessage(DWORD msg, LPVOID dat, DWORD prm1); //!< エフェクト描画オブジェクト共通メッセ処理関数
