...
void CFxCyohiLight::Update() {
  if (pdat->counter >= CYOHILIGHT_LIFETIME)
    End();
}
...
BOOL CFxCyohiLight::Draw() {
  ...
  if (counter < CYOHILIGHT_LIFETIME) {
    ...
    g_draw.d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLELIST, CYOHILIGHT_LINES, p_vbcl[counter], sizeof(MYVERTEX3D));
    g_draw.d3ddev->SetRenderState(D3DRS_ZENABLE, TRUE);
  }
  if (counter < 20)
    g_draw.DrawCircle((int)pdat->x, (int)pdat->y, //ひろがる
                      counter * 10, (20 - counter) * 2, 0.02f, 0xFF0055FF, TRUE);
...
CFxPSprite::CFxPSprite() { m_part = aki3d.CreateParticleObject(); }

CFxPSprite::~CFxPSprite() { aki3d.DestroyParticleObject(m_part); }

BOOL CFxPSprite::Create(int prm1, int prm2, int prm3) {
  if (SetParticleDat(prm1, prm2, (DWORD)prm3)) {
    if (num > 16) {
      g_system.Log("CFxPSprite::Create numが16を超えています", SYSLOG_WARNING);
      num = 16;
    }
...
void CFxPSprite::Update() {
  ...
  for (tmp2 = 0; tmp2 < 16; tmp2++) {
    ...
    if (pdat->counter < life[tmp2])
      tmp3 = FALSE;
    else
      vb[tmp2].color = 0x00000000;
  }
  if (tmp3)
    End();
}
...
BOOL CFxBurn::Create(int prm1, int prm2, int prm3) {
  ...
  for (i = 0; i < NUM_FIRE; i++) {
    ...
    vb[i][0].z = vb[i][1].z = vb[i][2].z = vb[i][3].z = 0.2f - 0.01f * (rand() % 40);
...
void CFxBurn::Update() {
  ...
  for (i = 0; i < NUM_FIRE; i++) {
    ...
    vb[i][0].color = vb[i][1].color = vb[i][2].color = vb[i][3].color = col;
  }

  if (pdat->aid == ACTID_EFCTBURN_DETOUCHED) {
    if (pdat->counter > EFCTBURN_MAXLIFE)
      End();
  }
}
...
BOOL CFxBurn::Draw() {
  ...
  for (int i = 0; i < NUM_FIRE; i++) {
    if (life[i] < lifemax[i]) {
      g_draw.d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vb[i], sizeof(MYVERTEX3D));
    }
  }
  g_draw.SetAlphaMode(0);
  g_draw.EnableZ();
...
CFxGuardCancel::CFxGuardCancel() { m_part = aki3d.CreateParticleObject(); }

CFxGuardCancel::~CFxGuardCancel() { aki3d.DestroyParticleObject(m_part); }
...
BOOL CFxGuardCancel::Draw() {
  ...
  if (pdat->counter < 15) { //円
    g_draw.DrawCircle((int)pdat->x, (int)pdat->y, (pdat->counter + 4) * 5, 10, 0,
                      GetColor() + (((255 - pdat->counter * (255 / 15)) * 255 * 255 * 255) & 0xFF000000),
                      TRUE);
  }
