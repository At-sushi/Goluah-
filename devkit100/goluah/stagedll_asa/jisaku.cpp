...
void InitializeJien() {
  ...
  if (pdat != NULL)
    prev_hp[ouen] = pdat->hp;
  pdat = (GOBJECT *)funco->getactivechar(teki);
  if (pdat != NULL)
    prev_hp[teki] = pdat->hp;
...
DWORD gMsgJien(DWORD msg, LPVOID pd, DWORD prm) {
  ...
  case GOBJMSG_DRAWBACK:
    ...
    funcd->celldraw((void **)pimg, cdat, rdat, pdat->cnow, (int)pdat->x, (int)pdat->y, (float)pdat->z, 0,
                    pdat->revx, 0, 0xFFFFFFFF, 1.0f, 1.0f);
    return (TRUE);
  case GOBJMSG_DELETE:
    CleanupJien();
    break;
