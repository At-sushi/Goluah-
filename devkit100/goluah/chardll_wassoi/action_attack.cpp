...
{
  if (pdat->counter < 5)
    pdat->cnow = CELL_SUWA1;
  else if (pdat->counter < 20)
    pdat->cnow = CELL_SUWA2;
  else if (pdat->counter < 25)
    pdat->cnow = CELL_SUWA3;
  else if (pdat->counter < 30)
    pdat->cnow = CELL_SUWA4;
  else {
    pdat->aid = ACTID_NEUTRAL;
  }
...
void CCharacter::act_nage() {
  ...
  } else if (pdat->counter < 22 + 10) {
    pdat->cnow = CELL_NAGE4;
  } else if (pdat->counter < 26 + 10) {
...
DWORD CCharacter::act_nagee(GOBJECT *pedat) {
  if (pedat->counter < 10 + 10) {
    ...
    pedat->x =
        pdat->x - zurex(20) + zurex(80) * cos((pedat->counter - 20) * 12 / 180.0 * 3.1415 - 3.1415 / 4.0);
...
void CCharacter::act_striker2() //攻撃(1)
{
  ...
  if (pdat->counter < 60) {
    ...
    pdat->aid = ACTID_STRIKER3;

  GOBJECT *pmdat = GetActiveCharacter(pdat->tid);
  if (pmdat != NULL) {
    if (pmdat->hp > 0) {
      pmdat->hp++;
      if (pmdat->hp > (int)pmdat->hpmax)
        pmdat->hp = pmdat->hpmax;
    }
    /*	if(pdat->counter%4==0){
