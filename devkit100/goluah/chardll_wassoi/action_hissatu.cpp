...
void CCharacter::act_hadou1() {
  if (pdat->counter < 4)
    pdat->cnow = CELL_CUTTER1;
  else if (pdat->counter < 8)
    pdat->cnow = CELL_CUTTER2;
  else if (pdat->counter < 52)
    pdat->cnow = CELL_CUTTER3;
  else if (pdat->counter < 56)
    pdat->cnow = CELL_CUTTER4;
  else if (pdat->counter < 60)
    pdat->cnow = CELL_CUTTER5;
  else if (pdat->counter < 64)
    pdat->cnow = CELL_CUTTER6;
  else
    pdat->aid = ACTID_NEUTRAL;
...
void CCharacter::act_syara1() {
  ...
  if (pdat->counter < 3)
    pdat->cnow = CELL_SYARA1;
  else if (pdat->counter < 6)
    pdat->cnow = CELL_SYARA2;
  else if (pdat->counter < 9)
    pdat->cnow = CELL_SYARA3;
  else
    pdat->cnow = CELL_SYARA4;
}
...
void CCharacter::act_festival() //海苔を投げる
{
  ...
  if (pdat->counter < 5)
    pdat->cnow = CELL_FESTIVAL1;
  else if (pdat->counter < 8)
    pdat->cnow = CELL_FESTIVAL2;
  else if (pdat->counter < 60)
    pdat->cnow = CELL_FESTIVAL3;
  else if (pdat->counter < 70)
    pdat->cnow = CELL_CUTTER4;
...
void CCharacter::act_festivaldash() {
  ...
  if (pdat->counter < 40)
    pdat->cnow = CELL_FESTIVALDASH;
  else
    pdat->aid = ACTID_FESTIVALSUKI;
}
