...
BOOL CCharacter::Command_OnSystem(DWORD key) {
  if (pdat->aid == ACTID_OKIAGARI && pdat->counter == 2 && pdat->gauge >= NEEDGAUGE_REV && OptKoroKoro()) {
...
BOOL CCharacter::Command_Hissatu(DWORD key_now) {
  ...
  if (pdat->aid & ACTID_KUCYU) { //空中必殺技のコマンド判定
    ...
    if (com214(30)) { //しゃら～
      if (key_now & KEYSTA_BC2) {
        pdat->aid = ACTID_SYARA3;
        return TRUE;
      } else if (key_now & KEYSTA_BB2) {
        pdat->aid = ACTID_SYARA2;
        return TRUE;
      } else if (key_now & KEYSTA_BA2) {
        pdat->aid = ACTID_SYARA1;
        return TRUE;
      }
    }
    if (b_nori && b_nori->isReady() && com236(30)) { //海苔
      if (key_now & KEYSTA_BC2) {
        pdat->aid = ACTID_HADOU3;
        return TRUE;
      } else if (key_now & KEYSTA_BB2) {
        pdat->aid = ACTID_HADOU2;
        return TRUE;
      } else if (key_now & KEYSTA_BA2) {
        pdat->aid = ACTID_HADOU1;
        return TRUE;
      }
    }
  }
...
BOOL CCharacter::Command_OnNormal(DWORD keyinfo) {
  ...
  if (pdat->aid & ACTID_KUCYU) { //ジャンプ動作中
    ...
    //投げ
    if (keyinfo & KEYSTA_FOWORD && keyinfo & KEYSTA_BC2 && NageHantei(MAAI_NAGE) &&
...
DWORD CCharacter::CommandCOM(DWORD wid) {
  //ゲージチェック
  if (wid == ACTID_FESTIVAL && pdat->gauge < 1.0f)
    return FALSE;
...
DWORD CCharacter::CmdCom_OnSystem(DWORD wid) {
  //ガーキャン
  if (wid == ACTID_GCANCEL1 || wid == ACTID_GCANCEL2 || wid == ACTID_GCANCEL3) {
    if ((pdat->aid & ACTID_GUARD) && !(pdat->aid & ACTID_KUCYU) && pdat->gauge >= NEEDGAUGE_GC &&
        OptGuardCancel()) {
      return TRUE;
    }
    return FALSE;
...
DWORD CCharacter::Action() {
  ...
  case ACTID_ATT_SMC:
    act_att_smc();
    ChainCombo(CHAIN_SMC);
    break;
  case ACTID_WASSYORYUKEN1:
    act_wassoi1();
    break;
  case ACTID_WASSYORYUKEN2:
    act_wassoi2();
    break;
  case ACTID_WASSYORYUKEN3:
    act_wassoi3();
    break;
  case ACTID_WASSYORYUKENF:
    act_wassoif();
    break;
  case ACTID_HADOU1:
    act_hadou1();
  ...
  case ACTID_FESTIVAL:
    act_festival();
    break;
  case ACTID_FESTIVALDASH:
    act_festivaldash();
    break;
  case ACTID_FESTIVALSUKI:
    act_festivalsuki();
    break;
  case ACTID_FESTIVALRUSH:
    act_festivalrush();
    break;
  case ACTID_FESTIVALFINISH:
    act_festivalfinish();
    break;
  case ACTID_GCANCEL1:
    act_gcancel1();
  ...
  case ACTID_NAGE1:
    act_nage();
    break;
  case ACTID_OKIAGARI_FOWORD:
    act_okiagari_foword();
    break;
  case ACTID_OKIAGARI_BACK:
    act_okiagari_back();
    break;
  case ACTID_STRIKER1:
    act_striker1();
...
DWORD CCharacter::TouchB(ATTACKINFO *info, BOOL hit) {
  ...
  if (info->id == ATTACK_SCDASH) {
    if (hit)
      ChangeAction(ACTID_FESTIVALRUSH);
    else
      ChangeAction(ACTID_FESTIVALSUKI);
  }
...
DWORD CCharacter::TouchA(ATTACKINFO *info, DWORD ta_eid) {
  //超必中はスーパーアーマー
  if (pdat->aid == ACTID_FESTIVALDASH || pdat->aid == ACTID_FESTIVALRUSH ||
