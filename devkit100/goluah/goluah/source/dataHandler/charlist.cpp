...
BOOL CCharacterList::VerifyCharacterDir(char *dir, DWORD ringindex) {
  ...
  if ((info.ver > 800 && info.ver < 1000) || info.ver < 400 /*680*/) { //バージョンのチェック（古いほう）
    strcpy(dameinfo.dir, dir);
...
int CCharacterList::GetCharacterCount() { return infolist.size(); }
...
int CCharacterList::GetDameCharCount() { return damelist.size(); }

char *CCharacterList::GetDameCharDir(UINT index) { return damelist.at(index).dir; }
char *CCharacterList::GetDameCharName(UINT index) { return (damelist[index].name); }
DWORD CCharacterList::GetDameCharReas(UINT index) { return (damelist[index].dame); }
DWORD CCharacterList::GetDameCharVer(UINT index) { return (damelist[index].ver); }
...
int CCharacterList::FindCharacter(char *name) {
  ...
  for (DWORD i = 0; i < infolist.size(); i++) {
    if (strcmp(infolist[i].name, name) == 0)
      return (i);
  }
  return (-1);
}
...
CCOptionSelecter *CCharacterList::GetOptionSelecter(DWORD cindex) {
  if (cindex > infolist.size())
    return (NULL);
  return new CCOptionSelecter(&infolist[cindex], infolist[cindex].maxpoint);
}
...
CCSimpleOptionSelecter *CCharacterList::GetSimpleOptionSelecter(DWORD cindex) {
  if (cindex > infolist.size())
    return (NULL);
  return new CCSimpleOptionSelecter(&infolist[cindex], infolist[cindex].maxpoint);
}

DWORD CCharacterList::GetRandomOption(DWORD index) {
  if (index >= infolist.size())
    return 0;
  if (infolist[index].options.size() == 0)
    return 0;
...
char *CCharacterList::GetRingName(UINT index) {
  if (ringlist.size() - 1 < index)
    return NULL;
  return ringlist.at(index).name;
}

int CCharacterList::GetRingNum() { return ringlist.size(); }
...
CCOptionSelecter::CCOptionSelecter(CCL_CHARACTERINFO *cinfo, DWORD maxp) {
  ...
  DWORD ini_opt =
      m_current_favorite == 0 ? cinfo->previous_option : cinfo->fav_opts[m_current_favorite - 1].opt;
...
BOOL CCOptionSelecter::HandlePad(DWORD inputIndex) {
  ...
  BOOL leftON =
      (input & KEYSTA_ALEFT) && (key1 & KEYSTA_ALEFT) && (key2 & KEYSTA_ALEFT) && (key3 & KEYSTA_ALEFT2);
  BOOL rightON =
      (input & KEYSTA_ARIGHT) && (key1 & KEYSTA_ARIGHT) && (key2 & KEYSTA_ARIGHT) && (key3 & KEYSTA_ARIGHT2);
