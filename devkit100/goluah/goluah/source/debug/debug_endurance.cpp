...
void CTEnduranceTest::Setting_Stage() { g_battleinfo.SetStage(rand() % g_stagelist.GetStageCount()); }

void CTEnduranceTest::Setting_Other() { g_battleinfo.SetLimitTime(rand() % 60 + 30); }
...
CTTaimanTest::CTTaimanTest() { select_flag = FALSE; }
...
BOOL CTTaimanTest::Execute(DWORD time) {
  ...
  if (first_exec) {
    ...
    if (key & KEYSTA_RIGHT2) {
      if (select_flag) {
        if (char1 + 1 < (UINT)g_charlist.GetCharacterCount())
          char1++;
      } else {
        if (char2 + 1 < (UINT)g_charlist.GetCharacterCount())
          char2++;
      }
    }
...
void CTTaimanTest::Setting_BattleType() { g_battleinfo.SetBattleType(TAISENKEISIKI_GOCYAMAZE); }
...
void CTTaimanTest::Setting_Characters() {
  if (!first_exec) {
    ...
    sprintf(str, "\t%s(%d)\t\t%s(%d)", g_charlist.GetCharacterName(winner), winner,
            g_charlist.GetCharacterName(loser), loser);
  ...
  g_battleinfo.AddCharacter(0, char1, 1, CASSIGN_COM, g_charlist.GetRandomOption(char1));

  g_battleinfo.AddCharacter(1, char2, 1, CASSIGN_COM, g_charlist.GetRandomOption(char2));
}

void CTTaimanTest::Setting_Other() { g_battleinfo.SetLimitTime(60); }

void CTTaimanTest::Setting_CreateBattleTask() { CTEnduranceTest::Setting_CreateBattleTask(); }
