...
public:
  ...
  static void CellDraw(LPVOID *ddsa,
                       LPVOID cdat,
                       LPVOID rdat,
                       DWORD cn,
                       int x,
  ...
  static DWORD
  CreateCellDat(char *filename,
                LPVOID *cdat,
                LPVOID *rdat,
                LPVOID *hdat); //!< セルデータ読み込み。ver0.90以降ではLoadCellDatからこちらに変更するべき
  static void
  DestroyCellDat(LPVOID *cdat, LPVOID *rdat, LPVOID *hdat); //!< CreateCellDatで生成されたバッファをクリア
  static LPVOID LoadImage(char *filename, char *palname); //!< 画像読み込み関数、PNG対応に伴い追加。
  static DWORD CreateCellDat2(char *filename,
                              LPVOID *cdat,
                              LPVOID *rdat,
                              LPVOID *hdat); //!< セルデータ読み込み（２）、圧縮形式対応に伴い追加。
};
