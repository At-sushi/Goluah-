 ...
 */
void CBattleLoadInfo::SetBattleType(BYTE type) { m_type = type; }

/*!
 *	制限時間設定
 *	@param val 設定値。マイナスは制限なしを表す。
 */
void CBattleLoadInfo::SetLimitTime(int val) { m_limittime = val; }
 ...
 */
void CBattleLoadInfo::AddCharacter(DWORD team, DWORD cindex, DWORD color, BYTE key, DWORD option) {
  if (team > 1)
    return;
  if (m_num_team[team] >= 3)
    return;
...
//!ステージを設定
void CBattleLoadInfo::SetStage(DWORD stage_index) { m_stage = stage_index; }
...
//対戦形式を取得
DWORD CBattleLoadInfo::GetBattleType() { return m_type; }

//制限時間取得
int CBattleLoadInfo::GetLimitTime() { return m_limittime; }

// 1チームの人数を取得
DWORD CBattleLoadInfo::GetNumTeam(DWORD team) { return m_num_team[team]; }

//キャラクタを取得
DWORD CBattleLoadInfo::GetCharacter(DWORD team, DWORD member_index) { return m_char[team][member_index]; }
...
//カラーを取得
DWORD CBattleLoadInfo::GetColor(DWORD team, DWORD member_index) { return m_color[team][member_index]; }
...
//ステージを取得
DWORD CBattleLoadInfo::GetStage() { return m_stage; }

//ストーリーで指定されたBGM取得（設定されていない場合NULL）
char *CBattleLoadInfo::GetStoryBGM() {
  if (strlen(m_storybgm) == 0)
    return NULL;
  else
    return m_storybgm;
}
...
void CBattleResultInfo::SetKatiSerif(DWORD team, char *str) { sprintf(m_serif[team], str); }

//*******************************************************************
//	Get～
//*******************************************************************
BOOL CBattleResultInfo::GetWinner() { return m_winner == 0 ? FALSE : TRUE; }

char *CBattleResultInfo::GetKatiSerif(DWORD team) { return m_serif[m_winner]; }

DWORD CBattleResultInfo::GetCharacter(DWORD idx) { return m_character[idx]; }

DWORD CBattleResultInfo::GetColor(UINT idx) { return m_color[idx]; }
