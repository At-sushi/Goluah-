...
char *CExport::GetEnemyName(DWORD tid) {
  ...
  if (tid == TEAM_PLAYER1)
    eid = TEAM_PLAYER2;
  char *ret = g_charlist.GetCharacterName(
      g_battleinfo.GetCharacter(eid, GetCurrentBattleTask()->GetActiveCharacterID(eid)));
  FUNC_OUT;
  return ret;
}
char *CExport::GetCharacterName(DWORD oid) {
  ...
  char *ret = NULL;
  if (pobj)
    ret = g_charlist.GetCharacterName(
        g_battleinfo.GetCharacter((pobj->dll_id - 1) / MAXNUM_TEAM, (pobj->dll_id - 1) % MAXNUM_TEAM));
  FUNC_OUT;
  return ret;
}
DWORD CExport::GetKeyInput(DWORD oid) {
  ...
  DWORD ret = 0;
  if (pobj)
    ret = g_battleinfo.GetKeyAssign((pobj->dll_id - 1) / MAXNUM_TEAM, (pobj->dll_id - 1) % MAXNUM_TEAM);
  FUNC_OUT;
  return ret;
}
...
BYTE CExport::GetEnemyFace(DWORD tid) {
  ...
  if (tid == TEAM_PLAYER1)
    eid = TEAM_PLAYER2;
  BYTE ret = (BYTE)(OPT2ALT(
      g_battleinfo.GetCharacterOption(eid, GetCurrentBattleTask()->GetActiveCharacterID(eid))));
  FUNC_OUT;
  return ret;
}
...
LPVOID CExport::LoadBmp(char *filename, char *palname) {
  ...
  gbl.ods("CExport::LoadBmp : %s - %s , %s", ret ? "success" : "failed", filename ? filename : "NULL",
          palname ? palname : "NULL");
  if (ret) {
    // NowLoadingを進める
    CTNowLoading *task = dynamic_cast<CTNowLoading *>(g_system.FindTask('LOAD'));
    if (task)
      task->Proceed(NowLoading_Image);
  }
  FUNC_OUT;
  return ret;
}
...
void CExport::CellDraw(LPVOID *ddsa,
                       LPVOID cdat,
                       LPVOID rdat,
                       DWORD cn,
                       int x,
  ...
  if (g_draw.devtypenow != D3DDEVTYPE_HAL) {
    ...
    if (battleTask) {
      ...
      if (y < 10 && y > -500 && abs(x) <= 640.0)
        shadow = TRUE;
      g_draw.CellDraw((MYSURFACE **)ddsa, cdat, rdat, cn,
                      x - ((int)battleTask->GetDisplayCenterX() - g_DISPLAYWIDTH / 2), y + 440, z, rot, revx,
                      revy, color, magx, magy, shadow);
      FUNC_OUT;
      return;
    }
  }
...
void CExport::Blt(
    void *dds, int x, int y, RECT r, double magx, double magy, BOOL revx, BOOL revy, float z, DWORD color) {
  FUNC_IN;
  ...
  if (g_draw.devtypenow != D3DDEVTYPE_HAL) {
    ...
    if (battleTask) {
      g_draw.CheckBlt2((MYSURFACE *)dds, x - ((int)battleTask->GetDisplayCenterX() - g_DISPLAYWIDTH / 2),
                       y + 440, r, magx, magy, revx, revy, 0, z, color);
      FUNC_OUT;
      return;
    }
  }
...
DWORD CExport::CreateCellDat(char *filename, LPVOID *cdat, LPVOID *rdat, LPVOID *hdat) {
  ...
  *hdat = new GCD_HANTEI[GCDMAX_CELLS];
  if (0 != CGCDHandler::GCDLoad(filename, (GCD_CELL2 *)(*cdat), (GCD_RECT *)(*rdat), (GCD_HANTEI *)(*hdat),
                                NULL)) {
    DELETE_ARRAY(*cdat);
    DELETE_ARRAY(*rdat);
    DELETE_ARRAY(*hdat);
...
LPVOID CExport::LoadImage(char *filename, char *palname) {
  ...
  gbl.ods("CExport::LoadImage : %s - %s , %s", ret ? "success" : "failed", filename ? filename : "NULL",
          palname ? palname : "NULL");
  if (ret) {
    // NowLoadingを進める
    CTNowLoading *task = dynamic_cast<CTNowLoading *>(g_system.FindTask('LOAD'));
...
DWORD CExport::CreateCellDat2(char *filename, LPVOID *cdat, LPVOID *rdat, LPVOID *hdat) {
  ...
  *hdat = new GCD_HANTEI[GCDMAX_CELLS];
  ...
  if (0 != CGCDHandler::GCDLoadCompressed(Buffer, (GCD_CELL2 *)(*cdat), (GCD_RECT *)(*rdat),
                                          (GCD_HANTEI *)(*hdat), NULL) &&
      0 != CGCDHandler::GCDLoad(Buffer, (GCD_CELL2 *)(*cdat), (GCD_RECT *)(*rdat), (GCD_HANTEI *)(*hdat),
                                NULL)) {
    ...
    if (0 != CGCDHandler::GCDLoadCompressed(Buffer, (GCD_CELL2 *)(*cdat), (GCD_RECT *)(*rdat),
      ...
      *rdat = new GCD_RECT_070[GCDMAX_RECTANGLES];
      *hdat = new GCD_HANTEI_070[GCDMAX_CELLS];
      if (0 != CGCDHandler::GCDLoad(Buffer, (GCD_CELL2 *)(*cdat), (GCD_RECT *)(*rdat), (GCD_HANTEI *)(*hdat),
                                    NULL, 700)) {
        DELETE_ARRAY(*cdat);
        DELETE_ARRAY(*rdat);
        DELETE_ARRAY(*hdat);
...
void CExport::BGMPause(void) { g_sound.BGMPause(); }
void CExport::BGMResume(void) { g_sound.BGMResume(); }
