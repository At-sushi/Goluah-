...
protected:
	virtual DWORD TxtCol(int idx){
		if(1< idx && idx <= 1+MAXNUM_TEAM)
			...
			(m_state == CTCoS_Selected ? 0xFF00AA00 : 0xFF00AAFF) : 0xFF5237FF;
		else if(1+MAXNUM_TEAM < idx && idx <= 1+MAXNUM_TEAM*2)
			return idx==m_selected ?
			(m_state == CTCoS_Selected ? 0xFF00AA00 : 0xFFFF8200) : 0xFFFF3752;
		else
			return idx==m_selected ?
			(m_state == CTCoS_Selected ? 0xFF00AA00 : 0xFF000000) : 0xFF777777;
	}
