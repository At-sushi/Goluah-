...
void CTIconRingBase::Exec_Ready() {
  ...
  if (m_keyindex >= 0 && m_sleepCount == 0) {
    ...
    //ボタン押された
    if (keystate & KEYSTA_BA2) {
      OnButtonDown(KEYSTA_BA2);
    } else if (keystate & KEYSTA_BD2) {
      OnButtonDown(KEYSTA_BD2);
    } else if (keystate & KEYSTA_BB2) {
      OnButtonDown(KEYSTA_BB2);
    } else if (keystate & KEYSTA_BC2) {
      OnButtonDown(KEYSTA_BC2);
    }
    //←・→
    ...
    //↑・↓(リング変更)
      ...
      m_previous_index = m_selected_index;
      m_selected_ring++;
      if ((int)m_selected_ring > GetMaxRing() - 1)
        m_selected_ring = 0;
      while (GetMaxIndex(m_selected_ring) == 0) {
        m_selected_ring++;
        if ((int)m_selected_ring > GetMaxRing() - 1)
...
--------------------------------------------------------------------*/
void CTWindowBase::DrawText(
    float x, float y, float z, char *str, DWORD col, float scaX, float scaY, DWORD flags) {
  //スケールする
  x *= m_ratio;
  y *= m_ratio;
  scaX *= m_ratio;
...
void CTBeltBase::Draw() {
  ...
  if (m_show_text) {
    ...
    if (m_txtR2L)
      txtflg += SYSBMPTXT_R2L;
    int txtZure = 3; //影つけずらし量
    if (m_txtTop) {
      g_system.DrawBMPTextEx(m_txtLeft + txtZure, m_top + txtZure, 0, m_disp_str, m_txtCol1, 1.0f, m_ratio,
                             txtflg);
      g_system.DrawBMPTextEx(m_txtLeft + txtZure, m_top + 33 + txtZure, 0, m_disp_str2, m_txtCol1, 1.0f,
                             m_ratio, txtflg);
      ...
      g_system.DrawBMPTextEx(640.0f - m_txtLeft + txtZure, m_bottom - 33 + txtZure, 0, m_disp_str, m_txtCol1,
                             1.0f, m_ratio, txtflg);
      g_system.DrawBMPTextEx(640.0f - m_txtLeft + txtZure, m_bottom - 66 + txtZure, 0, m_disp_str2, m_txtCol1,
                             1.0f, m_ratio, txtflg);
      if (m_txtCol2 & 0xFF000000)
        g_system.DrawBMPTextEx(640.0f - m_txtLeft, m_bottom - 33, 0, m_disp_str, m_txtCol2, 1.0f, m_ratio,
                               txtflg);
      g_system.DrawBMPTextEx(640.0f - m_txtLeft, m_bottom - 66, 0, m_disp_str2, m_txtCol2, 1.0f, m_ratio,
                             txtflg);
    }
  }
  g_draw.EnableZ();
