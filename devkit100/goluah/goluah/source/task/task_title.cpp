...
CTitle::~CTitle() { RELSURFACE(dds_back); }
...
#define CELLDRAW1(x, y, id)                                                                                  \
  g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),                       \
                  g_system.GetSystemGraphicRect(), id, x, y, -0.02f, 0, FALSE, FALSE, 0xFF8888FF);
#define CELLDRAW2(x, y, id)                                                                                  \
  g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),                       \
                  g_system.GetSystemGraphicRect(), id, x, y, -0.02f, 0, FALSE, FALSE, 0xFFFFFFFF);
...
void CTitle::Draw() {
  ...
  if (!setting_now) {
    g_draw.CheckBlt(dds_back, 0, 0, r, FALSE, FALSE, 0, 0,
                    /*g_system.IsErrorTitle() ? 0xFFFF8888 : */ 0xFFFFFFFF); // logo
    ...
    // storymode
    if (selectedgamemode == TITLE_STORY)
      text_color = text1_color;
    else
    ...
    // practice
    if (selectedgamemode == TITLE_PRACTICE)
      text_color = text1_color;
    else
      text_color = text2_color;
    ...
    // settings
    if (selectedgamemode == TITLE_SETTINGS)
      text_color = text1_color;
    else
      text_color = text2_color;
    ...
    // op movie
    if (selectedgamemode == TITLE_OPMOVIE)
      text_color = text1_color;
    else
      text_color = text2_color;
    ...
    case DIFF_VERYEASY:
      strcpy(&msg[strlen(msg)], "牛鮭定食");
      break;
    case DIFF_EASY:
      strcpy(&msg[strlen(msg)], "150円引き");
      break;
    case DIFF_NORMAL:
      strcpy(&msg[strlen(msg)], "標準");
      break;
    case DIFF_HARD:
      strcpy(&msg[strlen(msg)], "パパ特盛り");
      break;
    case DIFF_VERYHARD:
      strcpy(&msg[strlen(msg)], "素人にはお勧めできない");
      break;
    case DIFF_SUPERHARD:
      strcpy(&msg[strlen(msg)], "殺伐");
      break;
    case DIFF_ULTRAHARD:
      strcpy(&msg[strlen(msg)], "刺すか刺されるか");
      break;
    case DIFF_LIMITERCUT:
      strcpy(&msg[strlen(msg)], "ぶち切れ");
      break;
    default:
      strcpy(&msg[strlen(msg)], "未知の難易度？");
    }
    ...
    case GSPEED_SLOW:
      strcpy(&msg[strlen(msg)], "遅い(40fps)");
      break;
    case GSPEED_OLD:
      strcpy(&msg[strlen(msg)], "標準(50fps)");
      break;
    case GSPEED_NEW:
      strcpy(&msg[strlen(msg)], "ちょっと速い(60fps)");
      break;
    case GSPEED_30x2:
      strcpy(&msg[strlen(msg)], "コマ飛ばし(30fps、2倍)");
      break;
      //		case GSPEED_25x2
      //:strcpy(&msg[strlen(msg)],"コマ飛ばし－標準(25fps、2倍)");break;
    default:
      strcpy(&msg[strlen(msg)], "ﾜｶﾝﾈ（とりあえず50fpsで動作します）");
    }
...
void CTitle::MainMenu() {
  ...
  if (keystate & KEYSTA_UP) {
    if (selectedgamemode == 0)
      selectedgamemode = TITLE_OPMOVIE;
    else
      selectedgamemode--;
    g_system.PlaySystemSound(SYSTEMSOUND_HIT1);
    counter = 0;
