...
void CCharacterSelectPractice::CreateNextTask() { g_system.AddTask(new CBattlePractice); }
...
BOOL CTConditionSelecterPractice::Execute(DWORD time) {
  /*	static BYTE m_type;						//gobject.h , TAISENKEISIKI_～参照
     BYTEで足りる static BYTE m_assign[2][MAXNUM_TEAM];	//割り当て static int m_limit_time_index;
     //↓のインデックス static int m_limit_time[5];				//45,60,99,120,∞  ,  const
...
void CTConditionSelecterPractice::Draw() {
  ...
  g_system.DrawBMPTextEx(x + 300, y, 0.0f, tstr, TxtCol(1), txt_xr, txt_yr, SYSBMPTXT_PROP);
  y += ystep;
  y += ystep;
