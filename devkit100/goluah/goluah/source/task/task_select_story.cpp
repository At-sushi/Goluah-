...
void CTStorySelect::SelectChangeColor(UINT idx, int col) { m_sinfo->color[idx] = col; }
...
-------------------------------------------------------------------------*/
UINT CTStorySelecterRing::GetSelected() { return g_storylist.Ring2Index(m_selected_ring, m_selected_index); }
...
-------------------------------------------------------------------------*/
DWORD CTStorySelecterRing::GetColor(UINT ridx, UINT idx) { return 0x00FFFFFF; }
...
/*-----------------------------------------------------------------------
        リングの総数を取得する
-------------------------------------------------------------------------*/
UINT CTStorySelecterRing::GetMaxRing() { return g_storylist.GetRingNum(); }
...
BOOL CTStoryBigFace::Execute(DWORD time) {
  if (m_appear_counter) {
    ...
    if (m_appear_counter % CTStoryBigFace_AppearInterval == 0) {
      //定着
      CTCharacterBigFace::Set(m_cindex[m_selected_num], m_color[m_selected_num], m_alt[m_selected_num]);
      //テンポラリーに出現
      m_appear_cnt++;
      if (m_appear_cnt <= m_num) {
        SetTemporary(m_characters[m_num - m_appear_cnt], m_colors[m_num - m_appear_cnt],
                     m_alt[m_num - m_appear_cnt]);
      }
    }
  }
...
/*-----------------------------------------------------------------------
        既存キャラ色変更
-------------------------------------------------------------------------*/
void CTStoryBigFace::ChangeCharacter(int cindex, int idx) { m_cindex[idx] = cindex; }

/*-----------------------------------------------------------------------
        既存キャラ変更
-------------------------------------------------------------------------*/
void CTStoryBigFace::ChangeColor(int color, int idx) { m_color[idx] = color > 0 ? color : 1; }

/*-----------------------------------------------------------------------
        既存キャラFaceAlt変更
-------------------------------------------------------------------------*/
void CTStoryBigFace::ChangeAlt(int alt, int idx) { m_alt[idx] = alt; }

/*-----------------------------------------------------------------------
        dark=TRUE で 選択キャラ以外を暗く表示
-------------------------------------------------------------------------*/
void CTStoryBigFace::SetDisplayMode(BOOL dark) { dark = TRUE; }

/*-----------------------------------------------------------------------
        dark=TRUE 時に明るく表示するキャラを指定
-------------------------------------------------------------------------*/
void CTStoryBigFace::SetLightIndex(int idx) { m_lighting = idx; }
...
DWORD CTStoryBigFace::GetDispColor(UINT idx) {
  if (!m_dark)
    ...
    return idx == m_lighting ? 0xFFFFFFFF : 0xFF999999;
}
...
/*-----------------------------------------------------------------------
        ...
        破棄
-------------------------------------------------------------------------*/
void CTStorySelectPreview::CTStoryBrief::Terminate() { DELETEARRAY(disp_txt); }
...
void CTStorySelectPreview::CTStoryBrief::Draw() {
  ...
  if (disp_txt) {
    ...
    g_draw.DrawBlueText(r, disp_txt, disp_len, DT_LEFT | DT_WORDBREAK, 2);
  }

  //テキスト
  char *title = g_storylist.GetStoryName(m_sindex);
  if (title) {
    g_system.DrawBMPTextEx(620 + m_hideMove, 225, 0.0f, title, 0xFFFFFFFF, 1.0f, 1.0f,
...
void CTStoryParamWindow::Exec_Exec() {
  ...
  if (keysta & KEYSTA_BA2) {
    ...
    case 1: //カラー選択へ
    {
          ...
          (*m_sinfo)->characters[m_crnt_cindex] >= 0) //キャラクタが選択されていない場合は選択させない
      {
        ...
        pcosel->SetPos(115.0f + 75.0f + 150.0f * m_crnt_cindex, m_wintop + 55 + 30 * m_crnt_param);
  ...
  if (keysta & KEYSTA_ALEFT2) { //←
    if (m_crnt_cindex == 0)
      m_crnt_cindex = m_num - 1;
    else
      m_crnt_cindex--;
  } else if (keysta & KEYSTA_ARIGHT2) { //→
    m_crnt_cindex++;
    if (m_crnt_cindex == m_num)
      m_crnt_cindex = 0;
...
//消失時処理
BOOL CTStoryParamWindow::Exec_Hide() { return (GetDrawPriority() < 0) ? FALSE : TRUE; }

void CTStoryParamWindow::ToWaitMode() { m_state = state_wait; }
...
void CTStoryParamWindow::Draw() {
  ...
  // OK
  DrawText(600, y[3], 0.0f, m_ok ? "OK" : "Cannot Start", GetDrawColor(m_crnt_cindex, 3), m_ok ? 1.5f : 1.0f,
...
MYSURFACE *CTStoryParamWindow::CCharSelecter::GetIcon(UINT ridx, UINT idx) {
  ...
  if (sidx > g_charlist.GetCharacterCount())
    return NULL;
...
BOOL CTStoryParamWindow::CColorSelecter::Execute(DWORD time) {
  ...
  if (m_ratio == 1.0f) {
    ...
    if (keysta & KEYSTA_UP2) {
      color--;
      if (color == 0)
        color = MAXNUM_CHARACTERCOLOR;
      Notify();
    }
    if (keysta & KEYSTA_DOWN2) {
      color++;
      if (color > MAXNUM_CHARACTERCOLOR)
        color = 1;
      Notify();
    }
  }
