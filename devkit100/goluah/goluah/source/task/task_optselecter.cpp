...
void CTOptionSelecterBase::Draw() {
  ...
  // OPTIONS 描画
  DWORD okcolor;
  double tmpx = g_system.DrawBMPTextEx(offset_x, y, z, "--OPTIONS--", color_option | alpha, 1.0f,
                                       text_yscale * 1.2f, SYSBMPTXT_PROP);
  sprintf(tmp_str, "POINT:%d", m_selecter->current_point);
  if (m_selecter->current_point > 0)
    okcolor = color_ok;
  else
    okcolor = color_ng;
  ...
  //設定名表示
  sprintf(tmp_str, "PRESET:%s", m_selecter->GetCurrentSetName());
  ...
  for (ite = m_selecter->list->begin(); ite != m_selecter->list->end(); ite++) {
    ...
    if (ite->flag & ex_flag) { //競合フラグアリ
      if (k != m_selecter->current_selected)
        color = color_collis | alpha;
      else
        color = color_collis_sel | alpha;
      not_available = TRUE;
    } else if (m_selecter->enabled[k]) { //有効状態
      if (k != m_selecter->current_selected)
        color = color_enable | alpha;
      else
        color = color_enable_sel | alpha;
    } else {                                              //無効状態
      if ((ite->depends & setting_now) != ite->depends) { // dependsフラグが足りない
        if (k != m_selecter->current_selected)
          color = color_na2 | alpha;
        else
          color = color_na2_sel | alpha;
        not_available = TRUE;
      } else if (m_selecter->current_point < 0 ||
                 m_selecter->current_point < ite->point) { // ONにするとポイント足りない
        if (k != m_selecter->current_selected)
          color = color_pointb | alpha;
        else
          color = color_pointb_sel | alpha;
      } else { // ONにしてもポイントは足りる
        if (k != m_selecter->current_selected)
          color = color_disable | alpha;
        else
          color = color_disable_sel | alpha;
      }
    }
    //描画
    text_flag = SYSBMPTXT_PROP;
    if (k == m_selecter->current_selected)
      text_flag |= SYSBMPTXT_SHADE_B;
    g_system.DrawBMPTextEx(offset_x, y, z, ite->name, color, text_xscale, text_yscale, text_flag);
    if (not_available)
      g_system.DrawBMPTextEx(offset_x + 320, y, z, "N/A", color, text_xscale, text_yscale, text_flag);
    else if (m_selecter->enabled[k])
      g_system.DrawBMPTextEx(offset_x + 320, y, z, "ON", color, text_xscale, text_yscale, text_flag);
    else
      g_system.DrawBMPTextEx(offset_x + 320, y, z, "OFF", color, text_xscale, text_yscale, text_flag);
    y += text_spacing;
    k++;
  }
  ...
  if (m_selecter->commit_enabled) {
    if (m_selecter->current_selected != m_selecter->list->size())
      g_system.DrawBMPTextEx(offset_x + 150, y, z, "--OK--", color_ok | alpha, text_xscale, text_yscale,
                             SYSBMPTXT_PROP);
    else
      g_system.DrawBMPTextEx(offset_x + 150, y, z, "--OK--", color_ok_sel | alpha, text_xscale, text_yscale,
                             SYSBMPTXT_PROP | SYSBMPTXT_SHADE_B);
  } else {
    if (g_config.IsLimiterCut()) {
      if (m_selecter->current_selected != m_selecter->list->size())
        g_system.DrawBMPTextEx(offset_x + 150, y, z, "-LIMITER CUT-", color_ng | alpha, text_xscale,
                               text_yscale, SYSBMPTXT_PROP);
      else
        g_system.DrawBMPTextEx(offset_x + 150, y, z, "-LIMITER CUT-", color_ng_sel | alpha, text_xscale,
                               text_yscale, SYSBMPTXT_PROP | SYSBMPTXT_SHADE_B);
    } else {
      if (m_selecter->current_selected != m_selecter->list->size())
        g_system.DrawBMPTextEx(offset_x + 150, y, z, "--OVER--", color_ng | alpha, text_xscale, text_yscale,
                               SYSBMPTXT_PROP);
      else
        g_system.DrawBMPTextEx(offset_x + 150, y, z, "--OVER--", color_ng_sel | alpha, text_xscale,
                               text_yscale, SYSBMPTXT_PROP | SYSBMPTXT_SHADE_B);
    }
  }
...
int CTOptionSelecterBase::SetAndShow(DWORD cindex, DWORD keyindex) {
  ...
  if (m_selecter) {
    ...
    if (optnum < 2)
      ...
      m_fontheight = 20.0f - (20.0f - 8.0f) * ((float)optnum - 2) / 18.0f;
    else
      m_fontheight = 8.0f;
  } else
    return -1;
