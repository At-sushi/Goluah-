...
/*----------------------------------------------------------
        破棄
------------------------------------------------------------*/
void CCharacterSelect::Terminate() { g_sound.BGMStop(); }
...
BOOL CCharacterSelect::Execute(DWORD time) {
  //■step1
  if (!m_condition_sel_ok)
    return TRUE; //試合設定がまだ終わっていない

  //■step2
  if (!charsel_ok[0] || !charsel_ok[1]) {
    //デカface更新
    m_bface[0]->SetTemporary(m_ring[0]->GetSelected(), selected_color[0][num_selected[0]],
                             OPT2ALT(selected_option[0][num_selected[0]]));
    m_bface[1]->SetTemporary(m_ring[1]->GetSelected(), selected_color[1][num_selected[1]],
                             OPT2ALT(selected_option[1][num_selected[1]]));
    //オビ表示更新
...
void CCharacterSelect::CreateNextTask() { g_system.AddTask(new CBattleTask); }
...
void CCharacterSelect::OnConditionDecided(CTConditionSelecter *pcsel) {
  ...
  g_battleinfo.SetLimitTime(pcsel->m_limit_time[pcsel->m_limit_time_index]); //制限時間を設定
...
void CCharacterSelect::AssignKeys(int team) {
  ...
  //次に選択するのはコンピュータ
  if (m_assign[team][num_selected[team]] == CASSIGN_COM ||
      m_assign[team][num_selected[team]] == CASSIGN_STABLE) {
...
void CCharacterSelect::OnOptionSelect(CTOptionSelecter *pselecter, DWORD option) {
  ...
  // team？
  DWORD team = 3;
  if (pselecter == m_selecter[0] || pselecter == m_selecter[0]->GetCustomSelecter())
    team = 0;
  if (pselecter == m_selecter[1] || pselecter == m_selecter[1]->GetCustomSelecter())
    team = 1;
  if (team > 1)
    return;

  selected_option[team][num_selected[team]] = option;
  m_bface[team]->Set(selected_char[team][num_selected[team]], selected_color[team][num_selected[team]],
                     OPT2ALT(option)); //デカface更新
  num_selected[team]++;
...
void CCharacterSelect::OnChangeColor(CTCharacterRing *pring) {
  ...
  if (selected_color[team][num_selected[team]] > 3)
    selected_color[team][num_selected[team]] = 1;
}
...
/*-----------------------------------------------------------
        描画
-------------------------------------------------------------*/
void CCharacterSelect::Draw() { g_draw.SetTransform(FALSE); }
...
void CCharacterSelect::SetupBattleInfo() {
  ...
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < wanted_char_num[i]; j++) {
      g_battleinfo.AddCharacter(i, selected_char[i][j], selected_color[i][j], m_assign[i][j],
                                selected_option[i][j]);
    }
  }
  ...
  for (int i = 0; i < num_selected[team2]; i++) {
    if (selected_char[team][index] == selected_char[team2][i]) //キャラクターが同じ
    {
      if (selected_color[team][index] == selected_color[team2][i]) //色が同じ
      {
        ...
        case 1:
          selected_color[team][index] = 2;
          break;
        case 2:
          selected_color[team][index] = 1;
          break;
        case 3:
          selected_color[team][index] = 1;
          break;
...
MYSURFACE *CTCharacterRing::GetIcon(UINT ridx, UINT idx) {
  ...
  UINT sindex = g_charlist.RingIndex2SerialIndex(ridx, idx);
  if ((int)sindex > g_charlist.GetCharacterCount())
    return NULL;
...
/*--------------------------------------------------------------------------------------
        リングの総数を取得する
----------------------------------------------------------------------------------------*/
UINT CTCharacterRing::GetMaxRing() { return g_charlist.GetRingNum(); }

/*--------------------------------------------------------------------------------------
        指定リングが持っているアイコンの数を取得
----------------------------------------------------------------------------------------*/
UINT CTCharacterRing::GetMaxIndex(UINT ridx) { return g_charlist.GetCharacterCountRing(ridx); }
...
void CTCharacterRing::Draw() {
  ...
  // 選択されてるﾔｼの名前を表示
  // g_system.DrawBMPText(m_radius_base, m_height, 0, g_charlist.GetCharacterDir( GetSelected() ),
  // 0xFF4455AA);
}
...
void CTCharacterBigFace::Draw() {
  ...
  int kmax;
  if (m_draw_temporary)
    kmax = m_selected_num == m_max_num ? m_max_num : m_selected_num + 1;
  else
    kmax = m_max_num;

  for (int k = 0; k < kmax; k++) {
    l = m_selected_num - k; //奥のほうに押しやられる数
    if (m_selected_num == m_max_num)
      l--;
    if (l == 1)
      l = (int)(80 * (m_counter / 100.0));
    if (l == 2)
      l = (int)(80 + 80 * (m_counter / 100.0));
    ...
    if (pddstmp) {
      ...
      if (k == m_selected_num) {
        if (m_counter2 > pddstmp->wg)
          face2_maxx = (DWORD)pddstmp->wg;
        else
          face2_maxx = m_counter2;
      } else
        face2_maxx = (DWORD)pddstmp->wg;

      if (m_left) { // player1
        g_draw.CheckBlt(pddstmp, face2_maxx - (DWORD)pddstmp->wg - (m_max_num - 1) * 50 + l,
                        240 - (DWORD)pddstmp->hg / 2, r_face, FALSE, FALSE, 0, 0.02f + l * 0.008f, face2_col);
      } else { // player2
        g_draw.CheckBlt(pddstmp, 640 - face2_maxx + (m_max_num - 1) * 80 - l, 240 - (DWORD)pddstmp->hg / 2,
                        r_face, TRUE, FALSE, 0, 0.02f + l * 0.008f, face2_col);
      }
    }
  }
...
DWORD CTCharacterBigFace::GetDispColor(UINT idx) { return idx == m_selected_num ? 0xFFFFFFFF : 0xFFAAAAAA; }
...
void CTCharacterSelectBG::Initialize() {
  //テクスチャロード
  D3DXCreateTextureFromFileEx(g_draw.d3ddev, "system\\texture\\tex_cs1.png", 0, 0, 1, 0, g_draw.texformat,
                              D3DPOOL_MANAGED, D3DX_FILTER_POINT, D3DX_DEFAULT, 0x000000, NULL, NULL,
                              &ptex_cs1);
...
void CTCharacterSelectBG::Draw() {
  ...
  g_draw.d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vbg, sizeof(MYVERTEX3D));
...
void CTCharacterSelectBelt::SetCom(BOOL com) { m_com = com; }
...
void CTCharacterSelectBelt::Restore() { m_state = CTCSBS_Restore; }
...
void CTStageSelecter::UpdateText() {
  sprintf(m_disp_str, "%s", g_stagelist.GetStageDir(GetSelection()));
...
void CTStageSelecter::HandlePad() {
  ...
  //↑・↓(リング変更)
    ...
    if (m_selected_ring == 0)
      m_selected_ring = g_stagelist.GetRingNum() - 1;
    else
      m_selected_ring--;
    ...
    m_selected_ring++;
    if ((int)m_selected_ring > g_stagelist.GetRingNum() - 1)
      m_selected_ring = 0;
    while (g_stagelist.GetStageCountRing(m_selected_ring) == 0) {
      m_selected_ring++;
      if ((int)m_selected_ring > g_stagelist.GetRingNum() - 1)
...
int CTStageSelecter::GetSelection() { return g_stagelist.Ring2Serial(m_selected_ring, m_selected_index); }
...
BYTE CTConditionSelecter::m_assign[2][MAXNUM_TEAM] = {{0, CASSIGN_NONE, CASSIGN_NONE},
...
/*---------------------------------------------------------------------------------------
        終了処理
-----------------------------------------------------------------------------------------*/
void CTConditionSelecter::Terminate() { CTBeltBase::Terminate(); }
...
void CTConditionSelecter::Draw() {
  ...
  //対戦形式
  char *typenames[] = {"Cooperation", "Changeable", "Unchangeable"};
  sprintf(tstr, "Mode : %s", typenames[m_type - 1]);
  g_system.DrawBMPTextEx(x, y, 0.0f, tstr, TxtCol(0), txt_xr, txt_yr, SYSBMPTXT_PROP);
  ...
  g_system.DrawBMPTextEx(x + 300.0f, y, 0.0f, tstr, TxtCol(1), txt_xr, txt_yr, SYSBMPTXT_PROP);
  y += ystep;
  y += ystep;
  ...
  for (j = 0; j < 2; j++) {
    for (i = 0; i < MAXNUM_TEAM; i++) {
      ...
      if (m_assign[j][i] & CASSIGN_SPECIFIC) //特別
      {
        switch (m_assign[j][i]) {
        case CASSIGN_COM:
          sprintf(&tstr[strlen(tstr)], "Computer");
          break;
        case CASSIGN_NONE:
          sprintf(&tstr[strlen(tstr)], "--None--");
          break;
        default:
          sprintf(&tstr[strlen(tstr)], "Error?");
        }
      } else {
      ...
      g_system.DrawBMPTextEx(x + shiftX, y, 0.0f, tstr, TxtCol(j * MAXNUM_TEAM + i + 2), txt_xr, txt_yr,
                             SYSBMPTXT_PROP);
      y += ystep;
    }
    y += ystep;
  ...
  //"OK"描画
  g_system.DrawBMPTextEx(640.0f - x, y, 0.0f, m_ok ? "OK" : m_str_error, TxtCol(2 * MAXNUM_TEAM + 2), txt_xr,
                         txt_yr, SYSBMPTXT_PROP | SYSBMPTXT_R2L);
  y += ystep;
  //"return title"
  g_system.DrawBMPTextEx(640.0f - x, y, 0.0f, "Return Title", TxtCol(2 * MAXNUM_TEAM + 3), txt_xr, txt_yr,
  ...
  if (m_type == TAISENKEISIKI_GOCYAMAZE) {
    ...
    for (int k = 0; k < MAXNUM_KEYI; k++) {
      ...
      if (belong[0] && belong[1]) {
        sprintf(m_str_error, "Error , Player%d is Betrayer", k + 1); //裏切り者ハケーン
        m_ok = FALSE;
        return;
      }
    }
