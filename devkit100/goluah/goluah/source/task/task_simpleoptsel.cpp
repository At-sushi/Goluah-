...
/*-----------------------------------------------------------
        構築
-------------------------------------------------------------*/
CTSimpleOptionSelecter::CTSimpleOptionSelecter() { custom_mode = FALSE; }
...
void CTSimpleOptionSelecter::Draw() {
  ...
  // OPTIONS 描画
  DWORD okcolor;
  double tmpx = g_system.DrawBMPTextEx(offset_x, y, z, "--PRESETS--", color_option | alpha, 1.0f,
  ...
  k = 0;
  DWORD color;
  for (itef = m_selecter->m_ref_cinfo->fav_opts.begin(); itef != m_selecter->m_ref_cinfo->fav_opts.end();
       itef++) {
    if (k == m_selecter->m_current_favorite - 1)
      color = color_ok_sel | alpha;
    else
      color = color_ok | alpha;
...
void CTSimpleOptionSelecter::EndSelect() {
  ...
  if (m_selecter->m_current_favorite == 0 && m_selecter->state != 0xFFFFFFFF) {
