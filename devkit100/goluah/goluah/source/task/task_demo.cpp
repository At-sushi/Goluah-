...
BOOL CTDemo::Execute(DWORD time) {
  ...
  r_all.left += yurex;
  r_all.top += yurey;
  if (r_all.bottom - r_all.top > DEMODISPY)
    r_all.bottom = r_all.top + DEMODISPY;
  if (r_all.right - r_all.left > DEMODISPX)
    r_all.right = r_all.left + DEMODISPX;
  ...
  if (counter > 1 && g_input.GetKey(m_keyindex, 0) & KEYSTA_BD &&
      demodat[playingdemodat].num_select <= 0) { //選択肢でなければDで早送り
    if (txtlen < strlen(demodat[playingdemodat].msg)) {
      txtlen = strlen(demodat[playingdemodat].msg);
    } else
      demodat[playingdemodat].dur = 0;
...
void CTDemo::Draw() {
  ...
  //背景を描画
  if (ms != NULL)
    g_draw.CheckBlt(ms, 70, 90, r_all);
  for (int i = 0; i < 3; i++)
    if (chr[i] != NULL)
      g_draw.CheckBlt(chr[i], 70 + 100 + 200 * (i - 1), 90 /*DEMODISPY - (int)(chr[i]->hg / 2)*/, r_all);
...
DWORD CTDemo::GetGyoDemo(char *strin, char *strout, DWORD *susumu) {
  ...
  while (!ext) {
    if (strin[i] == 0x20)
      i++;
    else if (strin[i] == 0x81 && strin[i + 1] == 0x40)
      i += 2;
    else
      ext = TRUE;
  }
