...
/*----------------------------------------------------------------
        構築
------------------------------------------------------------------*/
CTaskStoryMode::CTaskStoryMode() { m_setting = NULL; }
...
void CTaskStoryMode::Initialize() {
  ...
  BOOL ret =
      CGoluahStoryScript::CreateScriptElementList(scr_list,
      ...
      for (int t = 0; t < 2; t++) {
        for (int j = 0; j < ele_vs->num[t]; j++) {
          if (ele_vs->characters[t][j] >= 0 && !ele_vs->is_random_opt[t][j]) {
            g_charlist.CorrectOption(ele_vs->characters[t][j], &ele_vs->option[t][j]);
          }
        }
      }
    }
  ...
  // settingブロックを飛ばす
  while (dynamic_cast<CStoryElement_Settings *>(*scr_i))
    scr_i++;
...
------------------------------------------------------------------*/
void CTaskStoryMode::Activate(DWORD prvTaskID) { m_prev_task_id = prvTaskID; }
...
void CTaskStoryMode::StartNextTask() {
  ...
  while (scr_i != scr_list.end() &&
         (*scr_i)->GetType() == CStoryScriptElement::CStoryElement_Types::GSE_ST_SELECT) {
...
void CTaskStoryMode::StartVSTask() {
  ...
  for (t = 0; t < 2; t++) {
    ...
    for (j = 0; j < ele_vs->num[t]; j++) {
      ...
      g_battleinfo.AddCharacter(t, ele_vs->characters[t][j], ele_vs->color[t][j], key, ele_vs->option[t][j]);
      // comレベルを設定
...
void CTaskStoryMode::CRandomCharacterGetter::Initialize(SScriptElementList &scr_list) {
  ...
  while (n != 0) {
    ...
    for (int k = 0; k < MAXNUM_TEAM; k++) {
      if (chars[k] == (n + rseed) % nn)
        in = FALSE;
    }
    if (in)
      all_list.push_back((n + rseed) % nn);
  }
...
void CTStoryContinue::Terminate() { RELEASE(m_texfb); }
...
void CTStoryContinue::DrawFBC(float t) {
  ...
  vb[0].color = vb[1].color = vb[2].color = vb[3].color = 0xFF000000 | (c << 16) | (c << 8) | c;
