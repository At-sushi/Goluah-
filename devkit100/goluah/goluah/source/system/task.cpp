...
CTaskManager::CTaskManager() { exNext = NULL; }
...
void CTaskManager::AddTask(CTaskBase *newTask) {
  ...
  if (exNext) {
    gbl.ods("■ALERT■ 排他タスクが2つ以上Addされた : %s / %s", typeid(*exNext).name(),
            typeid(*newTask).name());
  }
  exNext = pext;
...
void CTaskManager::Execute(DWORD time) {
  ...
  for (; i != ied; i++) {
    ...
    try {
      ...
      if (*i == NULL)
        g_system.Log("catch while execute1 : NULL", SYSLOG_ERROR);
      else
        g_system.LogErr("catch while execute1 : %X , %s", *i, typeid(**i).name());
  ...
  for (; i != ied; i++) {
    ...
    try {
      ...
      if (*i == NULL)
        g_system.Log("catch while execute2 : NULL", SYSLOG_ERROR);
      else
        g_system.LogErr("catch while execute2 : %X %s", *i, typeid(**i).name());
      g_system.NotifyExcption();
    }
#endif
  }
  ...
  if (ex_stack.size() != 0) {
    ...
    try {
      ...
      if (ex_stack.top() == NULL)
        g_system.Log("catch while execute3 : NULL", SYSLOG_ERROR);
      else
        g_system.LogErr("catch while execute3 : %X %s", ex_stack.top(), typeid(*ex_stack.top()).name());
      g_system.NotifyExcption();
    }
#endif
    ...
    if (!ex_ret) {
      ...
      if (!exNext) {
        ...
        try {
          ...
          if ((*i) == NULL)
            g_system.Log("catch while terminate1 : NULL", SYSLOG_ERROR);
          else
            g_system.LogErr("catch while terminate1 : %X %s", (*i), typeid(*(*i)).name());
          g_system.NotifyExcption();
        }
#endif
        ...
        try {
          ...
          if (exTsk == NULL)
            g_system.Log("catch while terminate2 : NULL", SYSLOG_ERROR);
          else
            g_system.LogErr("catch while terminate : %X %s", exTsk, typeid(*exTsk).name());
          g_system.NotifyExcption();
        }
#endif
        ...
        try {
          ...
          if (exTsk == NULL)
            g_system.Log("catch while activate : NULL", SYSLOG_ERROR);
          else
            g_system.LogErr("catch while activate : %X %s", exTsk, typeid(*exTsk).name());
          g_system.NotifyExcption();
        }
#endif
...
//全部なくなっちゃったら、やばいっしょ
BOOL CTaskManager::ExEmpty() { return (ex_stack.size() == 0) ? TRUE : FALSE; }
