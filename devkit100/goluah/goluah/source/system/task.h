...
public:
  ...
  virtual BOOL Execute(DWORD time) { return (TRUE); } //!< 毎フレームコールされる
  ...
  virtual DWORD GetID() {
    return 0;
  } //!< 0以外を返すようにした場合、マネージャに同じIDを持つタスクがAddされたとき破棄される
  virtual int GetDrawPriority() {
    return -1;
  } //!< 描画プライオリティ。低いほど手前に（後に）描画。マイナスならば表示しない

  static bool CompByDrawPriority(CTaskBase *arg1,
                                 CTaskBase *arg2) //!< 描画プライオリティでソートするための比較演算
  {
