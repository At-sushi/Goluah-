...
#else
  if (/*g_config.IsDebugMode() && */ GetKeyState(VK_SHIFT) & 0x8000)
    m_debug_menu_enabled = TRUE;

  //ゲーム開始
  //初期タスクの挿入
  if (m_debug_menu_enabled)
    AddTask(new CDebugMenu);
  else
    AddTask(new CTitle);
#endif
  return (TRUE);
...
void CSystem::OnWMKeydown(WPARAM wp, LPARAM lp) {
  ...
  //カメラ移動
  case 'O':
    g_draw.camera_z += 0.2f;
    break;
  case 'P':
    g_draw.camera_z -= 0.2f;
    break;
  case 'U':
    g_draw.camera_zurax += 0.2f;
    break;
  case 'I':
    g_draw.camera_zurax -= 0.2f;
    break;
  case 'T':
    g_draw.camera_zuray += 0.2f;
    break;
  case 'Y':
    g_draw.camera_zuray -= 0.2f;
    break;
...
BOOL CSystem::ShowInformation() {
  if (DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_DIALOG1), hwnd, DialogProc) != 0)
    return (FALSE);
  return (TRUE);
}
...
BOOL CALLBACK DialogProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
  ...
  if (uMsg == WM_INITDIALOG) {
    ...
    for (i = 0; i < g_charlist.GetCharacterCount(); i++) {
      sprintf(str, "%s : %s - ver%4.3f", g_charlist.GetCharacterDir(i), g_charlist.GetCharacterName(i),
    ...
    for (i = 0; i < g_charlist.GetDameCharCount(); i++) {
      ...
      case CCL_DAME_OLDDLL: //バージョンチェックに失敗
        sprintf(&str[strlen(str)], "ver.%4.3f(古) - %s", (double)g_charlist.GetDameCharVer(i) / 1000.0,
                g_charlist.GetDameCharName(i));
        break;
      case CCL_DAME_NEWDLL: //バージョンチェックに失敗(2)
        sprintf(&str[strlen(str)], "ver.%4.3f(新) - %s", (double)g_charlist.GetDameCharVer(i) / 1000.0,
                g_charlist.GetDameCharName(i));
        break;
      default:
  ...
  case WM_COMMAND:
    ...
    case IDCANCEL:
      ...
      break;
    case IDOK:
      /*	if(SendMessage(GetDlgItem(hwndDlg,IDC_RADIO1),BM_GETCHECK,0,0)==BST_CHECKED)network =
         NETWORK_NONE; else
         if(SendMessage(GetDlgItem(hwndDlg,IDC_RADIO2),BM_GETCHECK,0,0)==BST_CHECKED)network = NETWORK_HOST;
              else if(SendMessage(GetDlgItem(hwndDlg,IDC_RADIO3),BM_GETCHECK,0,0)==BST_CHECKED)network =
         NETWORK_CLIENT;*/
      EndDialog(hwndDlg, 0);
      break;
    }
 ...
 */
double CSystem::DrawBMPTextEx(
    double x, double y, float z, char *str, DWORD col, float scaX, float scaY, DWORD flags) {
  if (scaX < 0.01f)
    return x;
      ...
      if (cno > 0) {
        ...
        if (dr) {
          if (flags & SYSBMPTXT_SHADEMASK) {
            g_draw.CellDraw(sdds, scdat, srdat, cno, (int)x - step + 1, (int)y + 1, 0, 0, FALSE, FALSE,
                            shadecol.dwcol, scaX, scaY);
          }
          g_draw.CellDraw(sdds, scdat, srdat, cno, (int)x - step, (int)y, 0, 0, FALSE, FALSE, col, scaX,
                          scaY);
        }
      } else
        step = (int)(10 * scaX);
  ...
  while (*str != '\0') {
    ...
    if (cno > 0) {
      ...
      if (dr) {
        if (flags & SYSBMPTXT_SHADEMASK) {
          g_draw.CellDraw(sdds, scdat, srdat, cno, (int)x + 1, (int)y + 1, 0, 0, FALSE, FALSE, shadecol.dwcol,
                          scaX, scaY);
        }
...
/*------------------------------------------------------------------
        現在のメインタスククラスを取得
--------------------------------------------------------------------*/
CExclusiveTaskBase *CSystem::GetCurrentMainTask() { return taskManager.GetTopExclusiveTask(); }

/*------------------------------------------------------------------
        指定IDのバックグラウンドタスクを取得
--------------------------------------------------------------------*/
CBackgroundTaskBase *CSystem::FindBGTask(DWORD id) { return taskManager.FindBGTask(id); }

/*------------------------------------------------------------------
        指定IDの通常タスクを取得
--------------------------------------------------------------------*/
CTaskBase *CSystem::FindTask(DWORD id) { return taskManager.FindTask(id); }
...
void CSystem::SaveScreenShot() {
  ...
  sprintf(filename, "%s\\%d%s%d%s%d%s%d%s%d%s%d.%s", "system\\sshot\\",
  ...
  if (g_config.GetSShotImageFormat() == SSHOT_BMP) {
    ...
    head2.biClrUsed =
        0; // the bitmap uses the maximum number of colors corresponding to the value of the biBitCount member
...
void CSystem::PushSysTag(const char *tag_str) { m_systag.push(tag_str); }
...
void CSystem::PushDLLTag(const char *tag_str) { m_dlltag.push(tag_str); }
...
void CSystem::DumpTags() {
  while (m_dlltag.size() != 0) {
    LogErr("\t<dll(%d)> %s", m_dlltag.size(), (m_dlltag.top()) ? (m_dlltag.top()) : "(null)");
    m_dlltag.pop();
  }

  while (m_systag.size() != 0) {
    LogErr("\t<sys(%d)> %s", m_systag.size(), m_systag.top() ? m_systag.top() : "(null)");
    m_systag.pop();
  }
}
