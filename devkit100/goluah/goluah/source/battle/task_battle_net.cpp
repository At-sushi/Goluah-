...
void CBattleTaskNet::Initialize() {
  ...
  g_battleinfo.AddCharacter(TEAM_PLAYER1, g_charlist.FindCharacter("net1"), 1,
                            g_play.IsHost() ? 0 : CASSIGN_NONE,
                            /*g_charlist.GetRandomOption( g_charlist.FindCharacter("おにぎり") )*/ 0);
  g_battleinfo.AddCharacter(TEAM_PLAYER2, g_charlist.FindCharacter("net2"), 2, g_play.IsHost() ? 1 : 0,
                            /*g_charlist.GetRandomOption( g_charlist.FindCharacter("おにぎり") )*/ 0);
...
BOOL CBattleTaskNet::Execute(DWORD time) {
  ...
  */
  ...
  // 2秒おきにHP情報を送信
  if (GetGObjectInfo(0)->counter % 100 == 0 && g_play.IsHost()) {
    for (int i = 0; i <= 1; i++) {
      struct Sync2Mes s2m;
...
void CBattleTaskNet::T_Action(BOOL stop) {
  ...
  for (i = 0; i < (int)p_objects.size(); i++) {
    if (p_objects[i] != NULL &&
      ...
      else if (p_objects[i]->data.objtype & GOBJFLG_DONOTSTOP || p_objects[i]->data.nonstop)
        p_objects[i]->Message(GOBJMSG_ACTION);
      else
      ...
      if (BATTLETASK_ISNOTFXOBJ((&p_objects[i]->data)) && IsLocal(p_objects[i]->dll_id) &&
        ...
        if (p_objects[i]->data.counter % 15 == 1) {
          ...
          g_play.SendMsg(DPNID_ALL_PLAYERS_GROUP, tsm, sizeof(tsm), 300,
  ...
  if (g_play.IsHost() && GetGObjectInfo(0)->counter % 2 != 1) { // アクションだけ（１ＰＣの全キャラ分を一括）
    ...
    am.msgid =
        (GetGObjectInfo(0)->counter % 15 != 1 ? GNETMSG_ACTION2 : GNETMSG_ACTION); // testsyncとのダブり対策
                                ...
                                if (!pdat2->muteki) {               //無敵状態OFF
                                  if (pdat2->phdat != NULL) {
                                    if (pdat2->pcdat != NULL) {
                                      ...
                                      /*if ( ((GCD_CELL2_070*)pdat2->pcdat)[0].cell[0].flag==700 )
                                      {
                                              CGCDHandler::GCDConvCell_070_090( &(
                                      ((GCD_CELL2_070*)pdat1->pcdat)[ pdat1->cnow ]), &c_a);
                                              CGCDHandler::GCDConvCell_070_090( &(
                                      ((GCD_CELL2_070*)pdat2->pcdat)[ pdat2->cnow ]), &c_k);
                                      }
                                      else
...
DWORD CBattleTaskNet::MessageFromObject(DWORD oid, DWORD msg, DWORD prm) { return (0); }
