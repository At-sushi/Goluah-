...
CGObject::~CGObject() {
  ...
  if ((data.id & 0x7FFFFFFF) != (object_id_original & 0x7FFFFFFF)) {
    g_system.LogErr("object id changed %08X > %08X , dll_id = %d", object_id_original, data.id, dll_id);
  }
}
...
DWORD CGObject::Message(DWORD type, DWORD prm) {
  ...
  case GOBJMSG_ACTION:
    ...
    if (data.aid == ACTID_OKIAGARI)
      nage_muteki_cnt = 25;
    else if (data.aid & ACTID_GUARD)
      nage_muteki_cnt = 25;
    else if (nage_muteki_cnt)
      nage_muteki_cnt--;
    break;
  case GOBJMSG_DRAW:
    if (siver > 0) {
...
void CGObject::Defmsg_Draw() //描画。cellを描画するのみ
{
  ...
  //描画の設定
  if (data.alphamode != 0)
    g_draw.SetAlphaMode(data.alphamode);
  if (data.objtype & GOBJFLG_DISPZAHYO)
  ...
  g_draw.CellDraw(data.pmsarr, data.pcdat, data.prdat, data.cnow,
                  (g_draw.devtypenow == D3DDEVTYPE_HAL || data.objtype & GOBJFLG_DISPZAHYO)
                      ? (int)data.x
                      : (int)data.x - ((int)battleTask->GetDisplayCenterX() - g_DISPLAYWIDTH / 2),
                  (g_draw.devtypenow == D3DDEVTYPE_HAL || data.objtype & GOBJFLG_DISPZAHYO)
                      ? (int)data.y
                      : (int)data.y + 440,
                  data.z, data.rot, rev_x, data.revy, data.color, data.magx, data.magy, shadow);
...
DWORD CGObject::Defmsg_TouchA() //相手の攻撃に当たったときの処理
{
  ...
  DWORD keyinput = CExport::GetKeyInput(
      data.id); //オブジェクトidをgetkeyinputに代入して指定キャラのキー入力インデックス値を取得
  DWORD key_now = g_input.GetKeyEx(keyinput, 0); //直った・・・？

  // 待機/退避中は無視しちゃう
  if (data.aid & ACTID_INOUT)
    return (TOUCHA_AVOID);
  //投げ中は無視しちゃう
  if (data.aid & ACTID_NAGE)
    return (TOUCHA_AVOID);

  // AQ防止?
  if (data.aid == ACTID_DOWN2)
    return (TOUCHA_AVOID);
  //喰らい、またはガード中
...
void CGObject::Defmsg_Action() {
  ...
  case ACTID_OKIAGARI:
    dact_okiagari(&data);
    break; //おきあがり
  case ACTID_FINALBOUND:
    dact_finalbound(&data);
    break; //氏にバウンド
  case ACTID_FINALDOWN2:
    dact_finaldown2(&data);
    break;
  case ACTID_FUTTOBI2:
    dact_futtobi2(&data);
    break; //ぶっとび2
  case ACTID_DAMAGE1A:
    dact_damages1a(&data);
    break; //立ち喰らい（小）、のけぞりなし
  case ACTID_DAMAGE2A:
    dact_damages2a(&data);
    break; //立ち喰らい（中）、のけぞりなし
  case ACTID_DAMAGE3A:
    dact_damages3a(&data);
    break; //立ち喰らい（大）、のけぞりなし
  case ACTID_DAMAGEC1A:
    dact_damagec1a(&data);
    break; //しゃがみ喰らい（小）、のけぞりなし
  case ACTID_DAMAGEC2A:
    dact_damagec2a(&data);
    break; //しゃがみ喰らい（中）、のけぞりなし
  case ACTID_DAMAGEC3A:
    dact_damagec3a(&data);
    break; //しゃがみ喰らい（大）、のけぞりなし
  case ACTID_TATAKITUKE1A:
    dact_tatakituke1a(&data);
    break; //真下叩きつけ、バウンドあり
  case ACTID_TATAKITUKE2A:
    dact_tatakituke2a(&data);
    break; //　　〃　　　、〃　　　なし
  case ACTID_TATAKITUKE1B:
    dact_tatakituke1b(&data);
    break; //斜め叩きつけ、バウンドあり
  case ACTID_TATAKITUKE2B:
    dact_tatakituke2b(&data);
    break; //　　〃　　　、〃　　　なし
  case ACTID_KAITENFINISH:
    dact_kaitenfinish(&data);
    break; //回転フィニッシュ
  case ACTID_USERHIT1:
    dact_userhit1();
    break; //ユーザー定義喰らい(地上)
  ...
  case ACTID_GUARDJ3:
    dact_guardj3(&data);
    break; //空中ガード（大）
  case ACTID_USERGUARD1:
    dact_userguard1();
    break; //ユーザー定義ガード(地上)
  case ACTID_USERGUARD2:
    dact_userguard2();
    break; //ユーザー定義ガード(空中)
  default:
    return;
...
void CGObject::dact_down(GOBJECT *pdat) //転倒
{
  ...
  if (pdat->counter < 8) {
    ...
    pdat->cnow = DCELL_RAKKA1;
  } else if (pdat->counter < 16) {
    pdat->cnow = DCELL_RAKKA2;
  }
...
void CGObject::dact_okiagari(GOBJECT *pdat) //おきあがり
{
  ...
  if (pdat->counter < 4)
    pdat->cnow = DCELL_OKIAGARI1;
  else if (pdat->counter < 8)
    pdat->cnow = DCELL_OKIAGARI2;
  else if (pdat->counter < 12)
    pdat->cnow = DCELL_OKIAGARI3;
  else if (pdat->counter < 16)
    pdat->cnow = DCELL_OKIAGARI4;
  else if (pdat->counter < 20)
    pdat->cnow = DCELL_OKIAGARI5;
  else {
    Furimuki(pdat);
    pdat->aid = ACTID_NEUTRAL;
  }
...
void CGObject::dact_guards1(GOBJECT *pdat) //立ちガード(弱)
{
  if (pdat->counter < 2) {
    ...
    pdat->cnow = DCELL_GUARDS2;
    DmgMovex(pdat, -1);
  } else if (pdat->counter < 6) {
    pdat->cnow = DCELL_GUARDS3;
    DmgMovex(pdat, -1);
  } else if (pdat->counter < 8) {
    pdat->cnow = DCELL_GUARDS4;
    DmgMovex(pdat, 0);
  } else
    pdat->aid = ACTID_NEUTRAL;
}
...
void CGObject::dact_guardc1(GOBJECT *pdat) //しゃがみガード(弱)
{
  if (pdat->counter < 2) {
    ...
    pdat->cnow = DCELL_GUARDC2;
    DmgMovex(pdat, -1);
  } else if (pdat->counter < 6) {
    pdat->cnow = DCELL_GUARDC3;
    DmgMovex(pdat, -1);
  } else if (pdat->counter < 8) {
    pdat->cnow = DCELL_GUARDC4;
    DmgMovex(pdat, 0);
  } else
    pdat->aid = ACTID_CROUCH;
}
...
void CGObject::dact_guardj1(GOBJECT *pdat) //空中ガード(弱)
{
  if (pdat->counter < 2) {
    ...
    pdat->cnow = DCELL_GUARDJ2;
    DmgMovex(pdat, -1);
  } else if (pdat->counter < 8) {
    pdat->cnow = DCELL_GUARDJ3;
    DmgMovex(pdat, 0);
  } else {
    if (pdat->atk2.flags & ATKINFO2_RIGHTBACK)
      pdat->muki = TRUE;
...
void CGObject::ComCommand() {
  ...
  if (data.winfo == NULL)
    return;
  if (battleTask->GetState() != BFSTATE_FIGHTING && battleTask->GetState() != BFSTATE_FIGHTING)
    return;
  if (data.msghandler == NULL)
    return;
  ...
  if (data.aid & ACTID_KUCYU) { //ジャンプ中の場合
    if ((maai <= comreach[1] &&
         abs(data.y - pedat->y) <= comreach[1]) /* && rand()%5!=0*/) { // 80%の確率で攻撃を出す
