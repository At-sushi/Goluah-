...
/*-------------------------------------------------------------------
        生成破棄
---------------------------------------------------------------------*/
CGauge::CGauge(DI_FUNCTIONS_S *pfs, DI_FUNCTIONS_O *pfo, DI_FUNCTIONS_D *pfd) {
  for (int i = 0; i < 2; i++) {
    for (int j = 0; j < 3; j++) {
      dds_face[i][j] = NULL;
    }
...
void CGauge::Initialize() {
  ...
  GOBJECT *pdat = NULL;
  for (i = 0; i < 2; i++) {
    for (j = 0; j < (*funcs->getteamnum2)(i); j++) {
      pdat = (GOBJECT *)(*funco->getinfo)((*funco->getcharid)(i, j));
      if (pdat != NULL) {
  ...
  for (i = 0; i < 2; i++) {
    for (j = 0; j < g_battleinfo.GetNumTeam(i); j++) {
      ...
      dds_face[i][j] = (*funcd->loadimage)(filename, palname);
    }
  }
...
void CGauge::Action() {
  ...
  for (j = 0; j < 2; j++) {
    for (i = 0; i < (int)(*funcs->getteamnum2)(j); i++) {
      // prv の更新
      pdat = (GOBJECT *)(*funco->getinfo)((*funco->getcharid)(j, i));
      if (pdat != NULL) {
      ...
      //パワーゲージの移動
      if (funcs->gettaisenkeisiki() != TAISENKEISIKI_GOCYAMAZE) {
        if ((int)funco->getactivechar(j) == i) {
          pg_dx[j][i] += 5;
          if (pg_dx[j][i] > 0)
            pg_dx[j][i] = 0;
        } else {
...
void CGauge::Draw() {
  //全体の枠(?)
  (*funcd->settrans)(FALSE);
  funcd->celldraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
...
void CGauge::DrawHPGauge1() //体力ゲージの描画（TAISENKEISIKI_GOCYAMAZE）
{
  ...
  ((LPDIRECT3DDEVICE8)funcd->getd3dd())->SetTexture(0, NULL);
  ((LPDIRECT3DDEVICE8)funcd->getd3dd())->SetTransform(D3DTS_WORLD, &mati);
  ...
  ((LPDIRECT3DDEVICE8)funcd->getd3dd())->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, vb, sizeof(MYVERTEX3D));
  //黒っぽい背景(2p)
  vb[0].x = (float)(320.0 / 240.0 + 50.0 / 240.0);
  ...
  for (DWORD j = 0; j < funcs->getteamnum2(0); j++) {
    switch (funcs->getteamnum2(0)) {
    case 2:
      turiage_y = -0.025;
      break;
    case 3:
    ...
    // 1p側体力棒
                pdat = (GOBJECT*)(*funco->getinfo)( (*funco->getcharid(0,j) );
		if(pdat!=NULL){
      //体力
      if (pdat->hpmax != 0) {
        gauge_prv2[0][j] -= 20;
        ...
        if (pdat->hp < (int)pdat->hpmax) {
          ...
          if (pdat->hp < 0) {
            ...
            if (gauge_prv[0][j] < pdat->hp)
              gauge_prv[0][j] = pdat->hp;
          }
  ...
  for (DWORD j = 0; j < g_battleinfo.GetNumTeam(1); j++) {
    ...
    // 2p側体力棒
                pdat = (GOBJECT*)(GOBJECT*)(*funco->getinfo)( (*funco->getcharid(1,j) );
		if(pdat!=NULL){
      //体力
      if (pdat->hpmax != 0) {
        gauge_prv2[1][j] -= 20;
        ...
        if (pdat->hp < (int)pdat->hpmax) {
          ...
          if (pdat->hp < 0) {
            ...
            if (gauge_prv[1][j] < pdat->hp)
              gauge_prv[1][j] = pdat->hp;
          }
  ...
  for (i = 0; i < (int)g_config.GetMaxPoint(); i++) {
    if ((int)wincount > i)
      g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
                      g_system.GetSystemGraphicRect(), CELL_WINMARK, 320 - 65 - 30 * i, 65, -0.05f, 0);
    else
      g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
                      g_system.GetSystemGraphicRect(), CELL_WINMARK2, 320 - 65 - 30 * i, 65, -0.05f, 0);
  }
  // 2p
...
void CGauge::DrawHPGauge2() //体力ゲージの描画（2vs2）
{
  ...
  for (DWORD j = 0; j < g_battleinfo.GetNumTeam(0); j++) {
    ...
    // 1p側体力棒
                pdat = (GOBJECT*)(GOBJECT*)(*funco->getinfo)( (*funco->getcharid(0,j) );
		if(pdat!=NULL){
      //体力
      if (pdat->hpmax != 0) {
        gauge_prv2[0][j] -= 20;
      ...
      if (j == battleTask->GetActiveCharacterID(0)) {
        ...
        if (pdat != NULL) {
          if (pdat->hp <= 0)
            ...
            g_draw.MyBlt3D(dds_face[0][j], rsrc, rdst, 0, 0xFF777777);
        } else
          g_draw.MyBlt3D(dds_face[0][j], rsrc, rdst, 0, 0xFF777777);
      }
    }
      ...
      if (j == battleTask->GetActiveCharacterID(1)) {
        ...
        if (pdat != NULL) {
          if (pdat->hp <= 0)
            ...
            g_draw.MyBlt3D(dds_face[1][j], rsrc, rdst, 0, 0xFF777777);
        } else
          g_draw.MyBlt3D(dds_face[1][j], rsrc, rdst, 0, 0xFF777777);
      }
    }
  ...
  for (i = 0; i < (int)g_config.GetStrikerCount(); i++) {
    if ((int)wincount > i)
      g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
                      g_system.GetSystemGraphicRect(), CELL_STRIKERMARK1, 320 - 65 - 20 * i, 65, -0.05f, 0);
    else
      g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
                      g_system.GetSystemGraphicRect(), CELL_STRIKERMARK2, 320 - 65 - 20 * i, 65, -0.05f, 0);
  }
  // 2p
  ...
  for (j = 0; j < 2; j++) {
    ...
    case 1:
      if (j == 0)
        ...
        player = TRUE;
                        pdat = (GOBJECT*)(GOBJECT*)(*funco->getinfo)( (*funco->getcharid(j,0) );
			if(pdat!=NULL){
        ...
        if (pdat->gaugetype == GOBJFLGA_POWGAUGE1)
          ...
          DrawPowerGaugeType2(player, 0, 0, 0, FALSE);
			...
			pdat = (GOBJECT*)(GOBJECT*)(*funco->getinfo)( (*funco->getcharid(j,1) );
			if(pdat!=NULL){
        if (pdat->gaugetype == GOBJFLGA_POWGAUGE1)
        ...
        if (pdat->gaugetype == GOBJFLGA_POWGAUGE1)
          ...
          DrawPowerGaugeType2(player, 1, 0, 10, FALSE);
			...
			pdat = (GOBJECT*)(GOBJECT*)(*funco->getinfo)( (*funco->getcharid(j,2));
			if(pdat!=NULL){
        if (pdat->gaugetype == GOBJFLGA_POWGAUGE1)
    ...
    for (int i = 0; i < (int)g_battleinfo.GetNumTeam(j); i++) {
      if (pg_draw[j][i]) {
        if (j == 0)
          ...
          player = TRUE;
                                pdat = (GOBJECT*)(GOBJECT*)(*funco->getinfo)( (*funco->getcharid(j,i));
				if(pdat!=NULL){
          if (pdat->gaugetype == GOBJFLGA_POWGAUGE1)
...
void CGauge::DrawPowerGaugeType1(BOOL player, int pindex, int x, int y) {
  ...
  if (!player) {
    ...
    g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
                    ...
                    gcolor); //ワク
                pdat = (GOBJECT*)(GOBJECT*)(*funco->getinfo)( (*funco->getcharid( j,pindex ) );
		if(pdat!=NULL){
      ...
      if (pdat->gauge == pdat->gaugemax) { //ゲージマックス!!
        g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
                        g_system.GetSystemGraphicRect(), CELL_PG2_MAX, x + 65, y + 448, 0.0f, 0, FALSE, FALSE,
                        gcolor);
      } else { //棒を描く
    ...
    g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
      ...
      if (pdat->gauge == pdat->gaugemax) { //ゲージマックス!!
        g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
                        g_system.GetSystemGraphicRect(), CELL_PG2_MAX, x + 487, y + 448, 0.0f, 0, FALSE,
                        FALSE, gcolor);
      } else { //棒を描く
        rpg.right = rpg.left + (int)((pgauge_prv[j][pindex] - num) * 87);
        g_draw.CheckBlt(g_system.GetSystemGraphicSurface()[1], x + 575 - (rpg.right - rpg.left), y + 448, rpg,
                        FALSE, FALSE, 0, 0.0f, gcolor);
      }
		}
  }
}
  ...
  if (!player) {
    ...
    if (pow)
      ...
      if (pdat->gauge == pdat->gaugemax) { //ゲージマックス
        ...
        g_draw.CheckBlt(g_system.GetSystemGraphicSurface()[1], x + 48, y + 452, rpg, FALSE, FALSE, 0, 0.0f,
                        gcolor);                                               //棒
    ...
    if (pow)
      g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
                      g_system.GetSystemGraphicRect(), CELL_PG1_POW, x + 612, y + 455, 0.0f, 0, FALSE, FALSE,
                      gcolor); //「パウッ」
    else
      x += 40;
                pdat = (GOBJECT*)(GOBJECT*)(*funco->getinfo)( (*funco->getcharid( j,pindex ));
      ...
      if (pdat->gauge == pdat->gaugemax) { //ゲージマックス
        ...
        g_draw.CellDraw(g_system.GetSystemGraphicSurface(), g_system.GetSystemGraphicCell(),
                        g_system.GetSystemGraphicRect(), CELL_PG1_BLANK, x + 452, y + 455, 0.0f, 0, FALSE,
                        FALSE, gcolor); //ワク
        rpg.right = rpg.left + (int)((pgauge_prv[j][pindex]) * 132);
        g_draw.CheckBlt(g_system.GetSystemGraphicSurface()[1], x + 588 - (rpg.right - rpg.left), y + 452, rpg,
                        FALSE, FALSE, 0, 0.0f, gcolor);                         //棒
