...
BOOL CFxKO::DrawF() {
  ...
  for (i = 0; i < 2; i++) {
    ...
    //描画
    g_draw.d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 2, v, sizeof(MYVERTEX3D));
  }
  //'K'(加算)
  g_draw.SetAlphaMode(GBLEND_KASAN);
  v = v2;
...
void CFxFight::Divide(VertList &vlist, LineList &llist, TriList &tlist) {
  ...
  if (l1->v[0] == l2->v[0])
    v0 = l2->v[0];
  else if (l1->v[0] == l2->v[1])
    v0 = l2->v[1];
  else if (l1->v[1] == l2->v[0])
    v0 = l2->v[0];
  else if (l1->v[1] == l2->v[1])
    v0 = l2->v[1];
  else {
  ...
  if (t_old->l[0] == l0)
    t_old->l[0] = l51;
  else if (t_old->l[1] == l0)
    t_old->l[1] = l51;
  else if (t_old->l[2] == l0)
    t_old->l[2] = l51;
  else {
...
BOOL CFxFight::DrawF() {
  ...
  for (; i != ie; i++) {
    ...
    //描画
    g_draw.d3ddev->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP, 1, t->pos, sizeof(MYVERTEX3D));
  }
