...
void CEffectList::LoadTextures() {
  ...
  for (int i = 0; i < MAXNUM_SYSFXTEXTURES; i++) {
    ...
    if (D3D_OK != D3DXCreateTextureFromFileA(g_draw.d3ddev, filename, &p))
      p = NULL;
...
void CEffectList::AddEffect(DWORD type, int prm1, int prm2, int prm3) {
  if (!g_config.UsePointSprite()) {
    if (type == EFCTID_TUBUTUBU || type == EFCTID_TUBUTUBU2)
      return;
  }
  ...
  case EFCTID_MARK4:
    ...
    break;
  case EFCTID_MARKG2:
    fx = new CFxGuardmark1;
    break;
  case EFCTID_MARKG:
    fx = new CFxGuardmark2;
    break;
  case EFCTID_HIKARI:
    fx = new CFxCircle;
    break;
  ...
  case EFCTID_GNDCIR:
    fx = new CFxGndCircle;
    break;
  case EFCTID_CYOHILIGHT:
    fx = new CFxCyohiLight;
    break;
  case EFCTID_ROUND1:
    fx = new CFxRound1;
    break;
  ...
  case EFCTID_FIGHT:
    fx = new CFxFight;
    break;
  case EFCTID_TUBUTUBU:
    fx = new CFxPSprite;
    break;
  case EFCTID_TUBUTUBU2:
    fx = new CFxPSprite2;
    break;
  case EFCTID_BURN:
    fx = new CFxBurn;
  ...
  case EFCTID_COMEON:
    ...
    break;
  case EFCTID_GCANCEL:
    fx = new CFxGuardCancel;
    break;
  case EFCTID_SUPERARMER:
    fx = new CFxSuperArmer;
    break;
  case EFCTID_HATTEN:
    fx = new CFxHatten;
    break;
  case EFCTID_TIMEOVER:
    fx = new CFxTimeOver;
    break;
...
CSystemFxBase::~CSystemFxBase() { Destroy(); }
