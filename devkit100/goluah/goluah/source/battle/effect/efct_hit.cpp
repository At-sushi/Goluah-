...
void CFxHitmark1::Update() {
  ...
  if (pdat->counter < 6)
    pdat->cnow = CELL_HITMARK1_4;
  else if (pdat->counter < 8)
    pdat->cnow = CELL_HITMARK1_3;
  else if (pdat->counter < 10)
    pdat->cnow = CELL_HITMARK1_2;
  else if (pdat->counter < 12)
    pdat->cnow = CELL_HITMARK1_1;
  else
    End();
...
static BOOL DrawHitmark(const GOBJECT *pdat) {
  ...
  if (counter < 15) { //円
    ...
    g_draw.DrawCircle((int)pdat->x, (int)pdat->y, (counter + 4) * 5, 30, 0, 0xAACC33FF, TRUE);
...
void CFxHitmark2::Update() {
  ...
  if (pdat->counter < 8)
    pdat->cnow = CELL_HITMARK2_1;
  else if (pdat->counter < 11)
    pdat->cnow = CELL_HITMARK2_2;
  else if (pdat->counter < 14)
    pdat->cnow = CELL_HITMARK2_3;
  else if (pdat->counter < 17)
    pdat->cnow = CELL_HITMARK2_4;
  else
    End();
...
void CFxHitmark3::Update() {
  ...
  if (pdat->counter < 10)
    pdat->cnow = CELL_HITMARK3_1;
  else if (pdat->counter < 14)
    pdat->cnow = CELL_HITMARK3_2;
  else if (pdat->counter < 18)
    pdat->cnow = CELL_HITMARK3_3;
  else if (pdat->counter < 22)
    pdat->cnow = CELL_HITMARK3_4;
  else
    End();
...
void CFxHitmark4::Update() {
  ...
  if (pdat->counter < 8)
    pdat->cnow = CELL_HITMARK4_1;
  else if (pdat->counter < 11)
    pdat->cnow = CELL_HITMARK4_2;
  else if (pdat->counter < 14)
    pdat->cnow = CELL_HITMARK4_3;
  else if (pdat->counter < 17)
    pdat->cnow = CELL_HITMARK4_4;
  else if (pdat->counter < 20)
    pdat->cnow = CELL_HITMARK4_5;
  else
    End();
  if (pdat->counter == 0)
    pdat->color = 0xFF000000;
...
BOOL CFxHitmark2::Draw() { return DrawHitmark(pdat); }

BOOL CFxHitmark3::Draw() { return DrawHitmark(pdat); }

BOOL CFxHitmark4::Draw() { return DrawHitmark(pdat); }
...
BOOL CFxGuardmark1::Draw() {
  ...
  if (counter < 12) {
    if (pdat->muki) {
      g_draw.DrawCircle((int)pdat->x, (int)pdat->y, (counter + 3) * 5, (15 - counter / 3), 0, 0xFFFFAA00,
                        TRUE, -80, TRUE);
    } else {
      g_draw.DrawCircle((int)pdat->x, (int)pdat->y, (counter + 3) * 5, (15 - counter / 3), 0, 0xFFFFAA00,
                        TRUE, 80, TRUE);
...
BOOL CFxGuardmark2::Draw() {
  ...
  if (counter < 12) {
    if (pdat->muki) {
      g_draw.DrawCircle((int)pdat->x, (int)pdat->y, (counter + 3) * 5, (15 - counter / 3), 0, 0xFF5555FF,
                        TRUE, -80, TRUE);
    } else {
      g_draw.DrawCircle((int)pdat->x, (int)pdat->y, (counter + 3) * 5, (15 - counter / 3), 0, 0xFF5555FF,
                        TRUE, 80, TRUE);
...
void CFxCatchmark::Update() {
  if (pdat->counter < 2)
    pdat->cnow = CELL_CATCH1;
  else if (pdat->counter < 3)
    pdat->cnow = CELL_CATCH2;
  else if (pdat->counter < 4)
    pdat->cnow = CELL_CATCH3;
  else if (pdat->counter < 5)
    pdat->cnow = CELL_CATCH4;
  else if (pdat->counter < 10)
    pdat->cnow = CELL_CATCH5;
  else if (pdat->counter < 15)
    pdat->cnow = CELL_CATCH6;
  else
    End();
}
...
void CFxGndCircle::Update() {
  if (pdat->counter < 3)
    pdat->cnow = CELL_GNDCIR1;
  else if (pdat->counter < 6)
    pdat->cnow = CELL_GNDCIR2;
  else if (pdat->counter < 9)
    pdat->cnow = CELL_GNDCIR3;
  else if (pdat->counter < 12)
    pdat->cnow = CELL_GNDCIR4;
  else
    End();
