...
public:
  ...
  virtual CGObject *GetCharacterObject(DWORD j,
                                       DWORD i) = 0; //!< キャラクターのオブジェクト取得（ゲージ等で必要）
  ...
  void
  DrawNumber9(DWORD num, int x, int y, BOOL hits, float z, float magx = 1.0f, float magy = 1.0f); //時間制限
  ...
  virtual void SetCurrentDLLID(BYTE dllID) {
    m_crnt_dllid = dllID;
  } //!< CGObjectの生成時に呼ばれ、DLLのIDを返す
  virtual void SetCurrentDLLIDSystem() {
    m_crnt_dllid = 0;
  } //!< CGObjectがDLLの関数を呼び出す前後に呼ばれ、現在のDLLIDを設定する
