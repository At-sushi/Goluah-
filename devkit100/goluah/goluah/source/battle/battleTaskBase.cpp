...
void CBattleTaskBase::InitializeLoadDLLs() {
  ...
  for (j = 0; j < 2; j++) {
    ...
    case 3: // 3人目をロードする
      ...
      if (g_charlist.GetCharacterVer(g_battleinfo.GetCharacter(j, i)) < 680) {
        ...
        sprintf(filename, "%s\\action.dll", g_charlist.GetCharacterDir(g_battleinfo.GetCharacter(j, i)));
      ...
      if (now_loading)
        now_loading->Proceed(NowLoading_DLL);
    case 2: // 2人目をロードする
      i = 1;
      m_crnt_dllid = j * MAXNUM_TEAM + i + 1;
      m_current_com_level = g_battleinfo.GetComLevel(j, i);
  ...
  sprintf(filename, "%s\\stage.dll", g_stagelist.GetStageDir(g_battleinfo.GetStage()));
...
void CBattleTaskBase::AddDamage(DWORD oid, DWORD eoid, int x, int y) {
  ...
  if (TRUE) {      //常に喰らう
    ...
    case HITINFO_SNDSHK2:
      ...
      break;
    default:
      CSystem::Log("実装されていないヒット効果音が指定された", SYSLOG_WARNING);
    }
    ...
    case HITINFO_STOP:
      ...
      break;
    default:
      CSystem::Log("実装されていないヒットストップID", SYSLOG_WARNING);
    }
...
DWORD CBattleTaskBase::GetMaai(DWORD oid, DWORD eoid, BOOL h) {
  ...
  if (!h) { //水平間合い
    ...
    if (max_p1 < min_p1)
      koukan(&min_p1, &max_p1);
    if (max_p2 < min_p2)
      koukan(&min_p2, &max_p2);
    min_p1 += (int)pdat->x;
    max_p1 += (int)pdat->x;
    min_p2 += (int)pedat->x;
    max_p2 += (int)pedat->x;
...
DWORD CBattleTaskBase::GetWinCount(DWORD tid) { return (wincount[tid]); }

DWORD CBattleTaskBase::GetStrikerCount(DWORD tid) { return (strikercount[tid]); }

//勝利台詞を設定
void CBattleTaskBase::SetKatiSerif(DWORD tid, char *serif) { g_battleresult.SetKatiSerif(tid, serif); }

GOBJECT *CBattleTaskBase::GetActiveCharacter(DWORD tid) {
  if (!(tid == TEAM_PLAYER1 || tid == TEAM_PLAYER2))
    return (NULL);
  if (active_character[tid] >= MAXNUM_TEAM) {
    g_system.Log("CBattleTaskBase::GetActiveCharacter , active_characterにヘンな値がはいっています",
                 SYSLOG_WARNING);
    return (NULL);
  }
...
DWORD CBattleTaskBase::GetActiveCharacterID(DWORD team) {
  ...
  if (active_character[team] >= MAXNUM_TEAM) {
    g_system.Log("CBattleTaskBase::GetActiveCharacterID , active_characterにヘンな値がはいっています",
                 SYSLOG_WARNING);
    return 0;
  }
...
#define OLDCELLDRAW_NISE(a, b, c, d, e, f, g, h, i, j, k)                                                    \
  g_draw.CellDraw(d, f, e, c, a, b, i, 0, g, h, -1, j, k)

void CBattleTaskBase::DrawNumber(DWORD num, int x, int y, BOOL hits, float z, float magx, float magy) {
  switch ((num / 10) % 10) {
  case 1:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HR1, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 2:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HR2, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 3:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HR3, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 4:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HR4, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 5:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HR5, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 6:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HR6, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 7:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HR7, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 8:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HR8, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 9:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HR9, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 0:
    if (!hits)
      OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HR0, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  }
  switch (num % 10) {
...
void CBattleTaskBase::DrawNumber2(DWORD num, int x, int y, float z) {
  ...
  switch ((num / 1000) % 10) {
  case 1:
    OLDCELLDRAW(x, y, CELL_NUMBER_R1, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 2:
    OLDCELLDRAW(x, y, CELL_NUMBER_R2, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 3:
    OLDCELLDRAW(x, y, CELL_NUMBER_R3, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 4:
    OLDCELLDRAW(x, y, CELL_NUMBER_R4, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 5:
    OLDCELLDRAW(x, y, CELL_NUMBER_R5, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 6:
    OLDCELLDRAW(x, y, CELL_NUMBER_R6, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 7:
    OLDCELLDRAW(x, y, CELL_NUMBER_R7, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 8:
    OLDCELLDRAW(x, y, CELL_NUMBER_R8, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 9:
    OLDCELLDRAW(x, y, CELL_NUMBER_R9, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  }
  ...
  default:
    if (uketa)
      OLDCELLDRAW(x, y, CELL_NUMBER_R0, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
  }
  switch ((num / 10) % 10) {
  case 1:
...
void CBattleTaskBase::DrawNumber3(double numd, int x, int y, float z) //ゲージ1用
{
  ...
  switch (num % 10) {
  case 1:
    OLDCELLDRAW(x, y, CELL_PG2_NUM1, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 2:
    OLDCELLDRAW(x, y, CELL_PG2_NUM2, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 3:
    OLDCELLDRAW(x, y, CELL_PG2_NUM3, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 4:
    OLDCELLDRAW(x, y, CELL_PG2_NUM4, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 5:
    OLDCELLDRAW(x, y, CELL_PG2_NUM5, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 6:
    OLDCELLDRAW(x, y, CELL_PG2_NUM6, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 7:
    OLDCELLDRAW(x, y, CELL_PG2_NUM7, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 8:
    OLDCELLDRAW(x, y, CELL_PG2_NUM8, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 9:
    OLDCELLDRAW(x, y, CELL_PG2_NUM9, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 0:
    OLDCELLDRAW(x, y, CELL_PG2_NUM0, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  }
}
...
void CBattleTaskBase::DrawNumber4(double numd, int x, int y, float z) //ゲージ2用
{
  ...
  switch ((num / 10) % 10) {
  case 1:
    OLDCELLDRAW(x, y, CELL_PG1_NUM1, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 2:
    OLDCELLDRAW(x, y, CELL_PG1_NUM2, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 3:
    OLDCELLDRAW(x, y, CELL_PG1_NUM3, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 4:
    OLDCELLDRAW(x, y, CELL_PG1_NUM4, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 5:
    OLDCELLDRAW(x, y, CELL_PG1_NUM5, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 6:
    OLDCELLDRAW(x, y, CELL_PG1_NUM6, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 7:
    OLDCELLDRAW(x, y, CELL_PG1_NUM7, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 8:
    OLDCELLDRAW(x, y, CELL_PG1_NUM8, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 9:
    OLDCELLDRAW(x, y, CELL_PG1_NUM9, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 0:
    OLDCELLDRAW(x, y, CELL_PG1_NUM0, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  }
...
void CBattleTaskBase::DrawNumber5(DWORD num, int x, int y, BOOL hits, float z, float magx, float magy) {
  switch ((num / 10) % 10) {
  case 1:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HL1, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 2:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HL2, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 3:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HL3, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 4:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HL4, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 5:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HL5, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 6:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HL6, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 7:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HL7, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 8:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HL8, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 9:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HL9, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 0:
    if (!hits)
      OLDCELLDRAW_NISE(x, y, CELL_NUMBER_HL0, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  }
  switch (num % 10) {
...
void CBattleTaskBase::DrawNumber6(DWORD num, int x, int y, float z) {
  ...
  switch ((num / 1000) % 10) {
  case 1:
    OLDCELLDRAW(x, y, CELL_NUMBER_L1, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 2:
    OLDCELLDRAW(x, y, CELL_NUMBER_L2, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 3:
    OLDCELLDRAW(x, y, CELL_NUMBER_L3, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 4:
    OLDCELLDRAW(x, y, CELL_NUMBER_L4, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 5:
    OLDCELLDRAW(x, y, CELL_NUMBER_L5, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 6:
    OLDCELLDRAW(x, y, CELL_NUMBER_L6, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 7:
    OLDCELLDRAW(x, y, CELL_NUMBER_L7, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 8:
    OLDCELLDRAW(x, y, CELL_NUMBER_L8, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  case 9:
    OLDCELLDRAW(x, y, CELL_NUMBER_L9, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
    uketa = TRUE;
    break;
  }
  ...
  default:
    if (uketa)
      OLDCELLDRAW(x, y, CELL_NUMBER_L0, sdds, srdat, scdat, FALSE, FALSE, z);
    x += zure1;
  }
  switch ((num / 10) % 10) {
  case 1:
...
void CBattleTaskBase::DrawNumber7(double numd, int x, int y, float z) //ゲージ1用 TEAM2
{
  ...
  switch (num % 10) {
  case 1:
    OLDCELLDRAW(x, y, CELL_PG2_NUM_R1, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 2:
    OLDCELLDRAW(x, y, CELL_PG2_NUM_R2, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 3:
    OLDCELLDRAW(x, y, CELL_PG2_NUM_R3, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 4:
    OLDCELLDRAW(x, y, CELL_PG2_NUM_R4, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 5:
    OLDCELLDRAW(x, y, CELL_PG2_NUM_R5, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 6:
    OLDCELLDRAW(x, y, CELL_PG2_NUM_R6, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 7:
    OLDCELLDRAW(x, y, CELL_PG2_NUM_R7, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 8:
    OLDCELLDRAW(x, y, CELL_PG2_NUM_R8, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 9:
    OLDCELLDRAW(x, y, CELL_PG2_NUM_R9, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 0:
    OLDCELLDRAW(x, y, CELL_PG2_NUM_R0, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  }
}
...
void CBattleTaskBase::DrawNumber8(double numd, int x, int y, float z) //ゲージ2用 TEAM2
{
  ...
  switch ((num / 10) % 10) {
  case 1:
    OLDCELLDRAW(x, y, CELL_PG1_NUM_R1, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 2:
    OLDCELLDRAW(x, y, CELL_PG1_NUM_R2, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 3:
    OLDCELLDRAW(x, y, CELL_PG1_NUM_R3, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 4:
    OLDCELLDRAW(x, y, CELL_PG1_NUM_R4, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 5:
    OLDCELLDRAW(x, y, CELL_PG1_NUM_R5, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 6:
    OLDCELLDRAW(x, y, CELL_PG1_NUM_R6, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 7:
    OLDCELLDRAW(x, y, CELL_PG1_NUM_R7, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 8:
    OLDCELLDRAW(x, y, CELL_PG1_NUM_R8, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 9:
    OLDCELLDRAW(x, y, CELL_PG1_NUM_R9, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  case 0:
    OLDCELLDRAW(x, y, CELL_PG1_NUM_R0, sdds, srdat, scdat, FALSE, FALSE, z);
    break;
  }
...
void CBattleTaskBase::DrawNumber9(DWORD num, int x, int y, BOOL hits, float z, float magx, float magy) {
  switch ((num / 10) % 10) {
  case 1:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER1, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 2:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER2, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 3:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER3, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 4:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER4, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 5:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER5, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 6:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER6, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 7:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER7, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 8:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER8, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 9:
    OLDCELLDRAW_NISE(x, y, CELL_NUMBER9, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  case 0:
    if (!hits)
      OLDCELLDRAW_NISE(x, y, CELL_NUMBER0, sdds, srdat, scdat, FALSE, FALSE, z, magx, magy);
    x += (int)(20 * magx);
    break;
  }
  switch (num % 10) {
...
void CBattleTaskBase::DrawGObject(GOBJECT *pdat) {
  ...
  //描画の設定
  if (pdat->alphamode != 0)
    g_draw.SetAlphaMode(pdat->alphamode);
  if (pdat->objtype & GOBJFLG_DISPZAHYO)
    SetTransform(FALSE);

  g_draw.CellDraw(pdat->pmsarr, pdat->pcdat, pdat->prdat, pdat->cnow, (int)pdat->x, (int)pdat->y, pdat->z,
...
const char *CBattleTaskBase::MessageID2String(DWORD id) {
  switch (id) {
  case GOBJMSG_DELETE:
    return "GOBJMSG_DELETE";
  case GOBJMSG_ACTION:
    return "GOBJMSG_ACTION";
  case GOBJMSG_COMMAND:
    return "GOBJMSG_COMMAND";
  case GOBJMSG_COMMANDCOM:
    return "GOBJMSG_COMMANDCOM";
  case GOBJMSG_TOUCHA:
    return "GOBJMSG_TOUCHA";
  case GOBJMSG_TOUCHB:
    return "GOBJMSG_TOUCHB";
  case GOBJMSG_TOUCHC:
    return "GOBJMSG_TOUCHC";
  case GOBJMSG_CNGAID:
    return "GOBJMSG_CNGAID";
  //描画系メッセージ
  case GOBJMSG_DRAW:
    return "GOBJMSG_DRAW";
  case GOBJMSG_DRAWBACK:
    return "GOBJMSG_DRAWBACK";
  case GOBJMSG_DRAWFRONT:
    return "GOBJMSG_DRAWFRONT";
  //登場・交代などメッセージ
  case GOBJMSG_DOTOJYO:
    return "GOBJMSG_DOTOJYO";
  case GOBJMSG_DOTIMEOVERLOSE:
    return "GOBJMSG_DOTIMEOVER";
  case GOBJMSG_DOYOUWIN:
    return "GOBJMSG_DOYOUWIN";
  case GOBJMSG_TAIKI:
    return "GOBJMSG_TAIKI";
  case GOBJMSG_KOUTAI:
    return "GOBJMSG_KOUTAI";
  case GOBJMSG_KOUTAI2:
    return "GOBJMSG_KOUTAI2";
  case GOBJMSG_STRIKER:
    return "GOBJMSG_STRIKER";
  case GOBJMSG_DOYOUWIN2:
    return "GOBJMSG_DOYOUWIN2";
  case GOBJMSG_STRIKERREADY:
    return "GOBJMSG_STRIKERREADY";
  //座標操作系メッセージ
  case GOBJMSG_KNOCKBACK:
    return "GOBJMSG_KNOCKBACK";
  case GOBJMSG_CLIPX:
    return "GOBJMSG_CLIPX";
  //オブジェクト間相互作用・その他
  case GOBJMSG_CNGTARGET:
    return "GOBJMSG_CNGTARGET";
  case GOBJMSG_SOUSAI:
    return "GOBJMSG_SOUSAI";
  //ゲーム進行
  case GOBJMSG_CNGROUND:
    return "GOBJMSG_CNGGROUND";
  //ネットワーク
  case GOBJMSG_SYNC:
    return "GOBJMSG_SYNC";
  }
...
void CBattleTaskBase::Notify_Exception(CGObject *obj, DWORD msgid, DWORD prm) {
  ...
  if (team == 3) {
    g_system.LogErr("system/unknown object , msg:%s prm:0x%08X act:0x%08X cnt:%d last_func:%s(%s)",
                    MessageID2String(msgid), prm, obj->data.aid, obj->data.counter, CExport::last_funcname,
    ...
    g_system.LogErr("stage object %s(%s) , msg:%s prm:%08X act:0x%08X cnt:%d last_func:%s(%s)",
                    g_stagelist.GetStageName(sindex), (stgobjid == obj->data.id) ? "main" : "sub",
                    MessageID2String(msgid), prm, obj->data.aid, obj->data.counter, CExport::last_funcname,
                    CExport::func_in ? "in" : "out");
  } else {
    ...
    g_system.LogErr("character object %s(%s) , msg:%s prm:0x%08X act:0x%08X cnt:%d last_func:%s(%s)",
                    ...
                    (charobjid[team][index] == obj->data.id) ? "main" : "sub", MessageID2String(msgid), prm,
                    obj->data.aid, obj->data.counter, CExport::last_funcname,
                    CExport::func_in ? "in" : "out");
  }
}
...
const char *CBattleTaskBase::GetBattleStateString() {
  ...
  case BFSTATE_WAITFORENDPOSE:
    return "BFSTATE_WAITFORENDPOSE";
  case BFSTATE_ROUNDCALL:
    return "BFSTATE_ROUNDCALL";
  case BFSTATE_FIGHTING:
    return "BFSTATE_FIGHTING";
  case BFSTATE_FINISHED:
    return "BFSTATE_FINISHED";
  case BFSTATE_WAITFORENDWIN:
    return "BFSTATE_WAITFORENDWIN";
  case BFSTATE_DOUBLEKO:
    return "BFSTATE_DOUBLEKO";
  case BFSTATE_TIMEOVER:
    return "BFSTATE_TIMEOVER";
  }
