...
void CCharAdaptor::InitAdaptor(CDI_CHARACTERINFO2 *info) {
  ...
  pCreate = (BOOL(*)(DWORD, PVOID))GetProcAddress(hAction2, "CreateCharacter");
  if (pCreate == NULL) { // 失敗
...
DWORD CCharAdaptor::Message(DWORD msg, LPVOID dat, DWORD prm) {
  ...
  case MYGOBJMSG_BULLETHIT:
    ...
    return (BulletHit(&((STRUCT_BULLETA *)pbulainfo->pbul->pobjdat_u)->pbinfo_a->atk, pbulainfo->hit));
  default:
    return (0); //あとは、知らん
  }
  return (TRUE);
                  ...
                  if(pdat->aid & ACTID_KUCYU){////ジャンプ動作中
                          ...
                          if(key_now & 0x22220000){
                                  if(key_now & KEYSTA_DOWN){
                                          if(key_now & KEYSTA_BC2){if(ChainCombo(CHAIN_CC)){pdat->aid =
     ACTID_ATT_CC;return;}} else if(key_now & KEYSTA_BB2){if(ChainCombo(CHAIN_CB)){pdat->aid =
     ACTID_ATT_CB;return;}} else if(key_now & KEYSTA_BA2){if(ChainCombo(CHAIN_CA)){pdat->aid =
     ACTID_ATT_CA;return;}}
                                  }
                                  else{
                                          if(key_now & KEYSTA_BC2){if(ChainCombo(CHAIN_SC)){pdat->aid =
     ACTID_ATT_SC;return;}} else if(key_now & KEYSTA_BB2){if(ChainCombo(CHAIN_SB)){pdat->aid =
     ACTID_ATT_SB;return;}} else if(key_now & KEYSTA_BA2){if(ChainCombo(CHAIN_SA)){pdat->aid =
     ACTID_ATT_SA;return;}}
                                  }
                          }
                  }
...
DWORD CCharAdaptor::TouchB(ATTACKINFO *info, BOOL hit) {
  ...
  //ゲージの増加
  if (hit)
    AddPowerGauge(pdat, pg_hit);
  else
    AddPowerGauge(pdat, pg_guard);
...
void CCharAdaptor::ActionIDChanged() { pdat->z = ZZAHYO_CHARACTER; }
...
DWORD CCharAdaptor::BulletHit(MYATTACKINFO *info, BOOL hit) {
  ...
  if (hit)
    AddPowerGauge(pdat, info->pg_hit);
  else
    AddPowerGauge(pdat, info->pg_guard);
