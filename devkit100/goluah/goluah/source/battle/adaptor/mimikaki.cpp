...
void CCharAdaptor::InitializeGCDandBMP(GOBJECT *pdat, DWORD color) {
  ...
  if (hFile == INVALID_HANDLE_VALUE)
    return;
  if (!ReadFile(hFile, &cfh, sizeof(cfh), &br, NULL)) {
    CloseHandle(hFile);
    return;
  }
  if (br < sizeof(cfh)) {
  ...
  if (cfh.size == sizeof(GCD_FILEHEADER)) { // 最新バージョン
    CloseHandle(hFile);
    (*funcd->create_celldat)(filename, (LPVOID *)&pdat->pcdat, (LPVOID *)&pdat->prdat,
                             (LPVOID *)&pdat->phdat); // cell.gcd
  } else {
    ...
    if (!ReadFile(hFile, crect, sizeof(CHARDAT_RECT) * cfh.num_rects, &br, NULL)) {
      CloseHandle(hFile);
      return;
    }
    ...
    if (!ReadFile(hFile, ccell, sizeof(CHARDAT_CELL2) * cfh.num_cells, &br, NULL)) {
      CloseHandle(hFile);
      return;
    }
...
BOOL GetWinSerif(char *getmsg, char *enename, char *filepath) {
  ...
  if (numser == 0 && numserr == 0)
    return (FALSE); //一つも台詞が見つからなかった

  //見つかった台詞のうち、ランダムに1つの台詞を選択
  if (numser != 0)
    pnow = sstartpos[GetRandNum(numser)];
  else
    pnow = sstartposr[GetRandNum(numserr)]; //ランダム台詞
...
void LoadAndSetKatiSerif(DWORD tid) //テキストから勝利時のセリフを読み込んで設定する
{
  ...
  //	else funcs->setkatiserif(tid,"セリフの取得に失敗");
}
...
ATTACKINFO *ResisterAttackInfo(DWORD oid, MYATTACKINFO *mainfo) {
  ...
  mai_index++;
  if (mai_index >= MAXNUM_MYATTACKINFO)
    mai_index = 0;
  ...
  if (pdat->muki != my_ainfo[index].muki)
    my_ainfo[index].muki = TRUE;
  else
    my_ainfo[index].muki = FALSE;
...
// ATTACKINFO構造体を旧式に変換
void GetMyAttackInfo(DWORD oid, const ATTACKINFO *ainfo, MYATTACKINFO *mainfo, BOOL IsMyObject) {
  GOBJECT *pdat;
...
DWORD BulletACallBack(DWORD msg, LPVOID dat, DWORD prm) {
  ...
  case GOBJMSG_ACTION: //アクショソ
    ...
    if (pbdat->flags & BULLETA_DRAWBACK)
      pdat->z = ZZAHYO_BULLET2;
    if (pbdat->flags & BULLETA_DRAWMIDDLE)
      pdat->z = ZZAHYO_BULLET3;
    break;
