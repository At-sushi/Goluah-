...
#define GETOBJ(id)                                                                                           \
  (HIWORD(id) ? ((GOBJECT *)funco->getinfo(pnagerare[LOWORD(id)]))                                           \
...
void DoStartAttack(DWORD id, LPVOID atk) { StartAttack(GETOBJ(LOWORD(id))->id, (MYATTACKINFO *)atk); }
int SeekKey(DWORD id, int nanka, int hen, DWORD dane) {
  if (HIWORD(id))
    return -1;
  return funcs->seekkey(GETCLASS(id)->keynumber, nanka, hen, dane);
}
void SetKatiSerif(DWORD id, char *serif) { funcs->setkatiserif(GETOBJ(id)->tid, serif); }
void PlayMy(DWORD id, DWORD num) {
  if (!HIWORD(id))
    GETCLASS(id)->PlayMySound(num);
}
void DoGoBullet(LPVOID bul) { GoBulletA(GETOBJ(((BULLETINFO_A *)bul)->cid), (BULLETINFO_A *)bul); }
...
void DoDrawFront(DWORD id, BOOL isfront) { DrawFront(GETOBJ(id), isfront); }
BOOL DoCatchEnemy(DWORD id, LPVOID ufocatcher) {
  CATCHYOU cy;
  cy.oid = GETOBJ(id)->id;
  cy.actmsg = MYGOBJMSG_NAGE;
  if (HIWORD(id))
    return FALSE;
...
void AddDamage(DWORD eid, LPVOID atk, int x, int y) {
  GETOBJ(LOWORD(eid))->atk = ResisterAttackInfo(GETOBJ(LOWORD(eid))->id, (MYATTACKINFO *)atk);
  funco->adddamage(GETOBJ(LOWORD(eid))->id, GETOBJ(eid)->id, x, y);
  if (!HIWORD(eid))
    GETCLASS(eid)->olddat.hp -= ((MYATTACKINFO *)atk)->damage;
}
void ExtWin(DWORD ext) { extwin = ext; }
BOOL DoAddPowGauge(DWORD id, double addgauge) {
  if (GETOBJ(id)->gauge + addgauge < 0)
    return FALSE;
  AddPowerGauge(GETOBJ(id), addgauge);
  if (!HIWORD(id))
    GETCLASS(id)->olddat.gauge = GETOBJ(id)->gauge;
  return TRUE;
}
void AddEffect(DWORD efid, int prm1, int prm2, DWORD prm3) {
  if (efid == OLDEFCTID_CYOHI)
    ADDEFFECT(EFCTID_CYOHI, prm1, prm2, prm3);
  else
    ADDEFFECT(efid, prm1, prm2, prm3);
}
