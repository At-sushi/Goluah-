      ...
      if (*linestr == '#') //ブロック定義開始
      {
        ...
        if (list.size() == 0) {
          if (strcmp(tmpstr, "settings") != 0) {
            list.push_back(
                new CStoryElement_Error("settingsブロックの前に他ブロックを定義することはできません"));
            throw CStoryScriptException();
          }
        } else {
...
/*---------------------------------------------------------------
        ストーリー ブロック 基礎定義
-----------------------------------------------------------------*/
CStoryScriptElement::CStoryScriptElement() { m_error = NULL; }

CStoryScriptElement::~CStoryScriptElement() { DELETEARRAY(m_error); }
...
/*---------------------------------------------------------------
        ストーリー ブロック外エラー要素
-----------------------------------------------------------------*/
CStoryElement_Error::CStoryElement_Error(char *err) { Error(err); }
...
void CStoryElement_Settings::FeedLine(char *str) {
  //複数ラインにわたる定義
  if (multiline_flag) {
    if (gbl.strcheck(str, "brief_end"))
      multiline_flag = FALSE;
    else
      brief = gbl.MergeString(brief, str);
    return;
  }
...
void CStoryElement_VS::FeedLine(char *str) {
  ...
  if (multiline_flag) {
    if (multiline_flag == 1) { //勝利テキスト
      if (gbl.strcheck(str, "wintext_end"))
        multiline_flag = 0;
      else
        text_win = gbl.MergeString(text_win, str);
    } else if (multiline_flag == 2) { //負けテキスト
      if (gbl.strcheck(str, "losetext_end"))
        multiline_flag = 0;
      else
        text_lose = gbl.MergeString(text_lose, str);
    }
    return;
  }
...
void CStoryElement_VS::Read_stage(char *str) {
  ...
  sscanf(str, "%s %s", DMY_STR, tmp_str[0]);
...
CStoryElement_Demo::CStoryElement_Demo() { m_filename = NULL; }

CStoryElement_Demo::~CStoryElement_Demo() { DELETEARRAY(m_filename); }

void CStoryElement_Demo::FeedLine(char *str) {
  char **tmp = gbl.CreateTemporaryStrBuff(2);
  sscanf(str, "%s %s", tmp[0], tmp[1]);
...
/*---------------------------------------------------------------
        ストーリー #staff エレメント
-----------------------------------------------------------------*/
CStoryElement_Staff::CStoryElement_Staff() { m_filename = NULL; }

CStoryElement_Staff::~CStoryElement_Staff() { DELETEARRAY(m_filename); }
