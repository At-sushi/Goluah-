...
void CGoluahPlusBase::Destroy(CGoluahPlusBase *ptr) {
  if (ptr)
    delete ptr;
}
...
/*---------------------------------------------------------------
        インスタンスをゲトー
-----------------------------------------------------------------*/
CScriptLoader *CScriptLoader::Create() { return new CScriptLoaderInstance; }
...
BOOL CScriptLoaderInstance::LoadFile(char *base_dir, char *filename, UINT include_count) {
  if (STRINGHANDLER_MAXNESTCOUNT < include_count)
    return FALSE;
  ...
  // include処理
  if (!IncludeCheck(include_count + 1))
    return FALSE;
