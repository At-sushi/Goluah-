...
CCharacterBase *CreateMorarael(CDI_CHARACTERINFO2 *info) { return new CCharacterEL(info); }
...
BOOL CCharacterEL::Command_Hissatu(DWORD key) {
  ...
  if (!m_opt_tetra || pdat->gauge < NEEDGAUGE_TETRA)
    earrow_ok = FALSE;
  ...
  if (pdat->aid & ACTID_KUCYU) //空中必殺技のコマンド判定
  {
    ...
    if (earrow_ok) {
      if (com236(30)) {
        if (key & KEYSTA_BC2) {
          pdat->aid = ACTID_EARROW3J;
          return TRUE;
        } else if (key & KEYSTA_BB2) {
          pdat->aid = ACTID_EARROW2J;
          return TRUE;
        } else if (key & KEYSTA_BA2) {
          pdat->aid = ACTID_EARROW1J;
          return TRUE;
        }
      }
    }
  } else //地上必殺技のコマンド判定
  {
    ...
    if (com214(30) && m_opt_sword && pdat->gauge >= NEEDGAUGE_SOJ) {
      if (key & KEYSTA_BC2) {
        pdat->aid = ACTID_SWORD3R;
        return TRUE;
      } else if (key & KEYSTA_BB2) {
        pdat->aid = ACTID_SWORD2R;
        return TRUE;
      } else if (key & KEYSTA_BA2) {
        pdat->aid = ACTID_SWORD1R;
        return TRUE;
      }
    }
    ...
    if (earrow_ok) {
      if (com236(30)) {
        if (key & KEYSTA_BC2) {
          pdat->aid = ACTID_EARROW3;
          return TRUE;
        } else if (key & KEYSTA_BB2) {
          pdat->aid = ACTID_EARROW2;
          return TRUE;
        } else if (key & KEYSTA_BA2) {
          pdat->aid = ACTID_EARROW1;
          return TRUE;
        }
      }
    }
...
BOOL CCharacterEL::Command_OnNormal(DWORD key) {
  ...
  if (pdat->aid & ACTID_KUCYU) //ジャンプ動作中
  {
    ...
    //投げ（地上）
    if (key & KEYSTA_FOWORD && key & KEYSTA_BC2 && NageHantei(40) && ObjCatch(pdat->eid, GOBJMSG_NAGE)) {
      pdat->aid = ACTID_NAGE1;
      return TRUE;
    }
  }

  if (CCharacterBase::Command_OnNormal(key))
    return TRUE; //通常攻撃の判定
  ...
  if (pdat->aid & ACTID_KUCYU) {
    ...
    if (pdat->aid == ACTID_DASHFJ) {
      if (!(key & KEYSTA_FOWORD) && pdat->counter > 5)
        ChangeAction(ACTID_RAKKA2);
      return TRUE;
    }
    if (pdat->aid == ACTID_DASHBJ) {
      if (!(key & KEYSTA_BACK) && pdat->counter > 5)
        ChangeAction(ACTID_RAKKA2);
      return TRUE;
    }
  }
...
DWORD CCharacterEL::CommandCOM(DWORD wid) {
  if (wid == ACTID_JUDGEMENT && pdat->gauge < 1.0f)
  ...
  //ソードオブジャスティスキャンセル
  {
    if (wid == ACTID_SWORD3A && ChainCombo(CHAIN_SWORD3)) {
      if (pdat->aid == ACTID_SWORD2D || pdat->aid == ACTID_SWORD1D) {
        if (pdat->gauge < NEEDGAUGE_SOJ || !m_opt_sword)
          return FALSE;
        AddPowerGauge(-1 * NEEDGAUGE_SOJ);
        return TRUE;
      }
...
DWORD CCharacterEL::CmdCom_OnNormal(DWORD wid) {
  ...
  if (!m_opt_tetra || pdat->gauge < NEEDGAUGE_TETRA)
    earrow_ok = FALSE;
  if (wid == ACTID_EARROW1)
    if (pdat->gauge < NEEDGAUGE_TETRA || !earrow_ok)
      return FALSE;
...
DWORD CCharacterEL::Action() {
  ...
  case ACTID_SWORD1A:
    act_sword1a();
    ChainCombo(CHAIN_SWORD1);
    break;
  case ACTID_SWORD2A:
    act_sword2a();
    ChainCombo(CHAIN_SWORD2);
    break;
  case ACTID_SWORD3A:
    act_sword3a();
    ChainCombo(CHAIN_SWORD3);
    break;
  case ACTID_SWORD1D:
    act_sword1d();
    break;
...
void CCharacterEL::PreAction() {
  ...
  if (m_opt_regene && pdat->hp > 5) {
    if (!(pdat->aid & ACTID_ATTACK) && !(pdat->aid & ACTID_SYSTEM) && !(pdat->aid & ACTID_KUCYU) &&
        (pdat->aid != ACTID_TOJYO) && (pdat->aid != ACTID_SYORI)) {
      if ((pdat->counter % m_regene_rate) == 0)
        pdat->hp += 1;
      if (pdat->hp > (int)pdat->hpmax)
        pdat->hp = pdat->hpmax;
    } else {
      if ((pdat->counter % (m_regene_rate * 3)) == 0)
        pdat->hp += 1;
      if (pdat->hp > (int)pdat->hpmax)
        pdat->hp = pdat->hpmax;
    }
  ...
  //ゲージの増加
  if (!(pdat->aid & ACTID_ATTACK) &&                //攻撃中でない
      (pdat->aid != ACTID_DASHFJ) &&                //空中ダッシュ中でない
      (pdat->aid != ACTID_DASHBJ) && (pdat->hp > 0) //死んでない
  ) {
...
void CCharacterEL::InitAttackInfo() {
  ...
  aif[i].hit =
      HITINFO_FUTTOBI2 | HITINFO_SIV3 | HITINFO_MARK4 | HITINFO_SNDHIT3 | HITINFO_EFCTSINDO; //ヒット情報
  aif[i].guard = GUARDINFO_REACT3 | GUARDINFO_SIV3;                                          //ガード情報
  ...
  i = ATTACKINFO_SWORD1;
  aif[i].hit =
      HITINFO_REACT3 | HITINFO_SIV3 | HITINFO_MARK4 | HITINFO_SNDHIT3 | HITINFO_EFCTSINDO; //ヒット情報
  aif[i].guard = GUARDINFO_REACT3 | GUARDINFO_SIV3 | GUARDINFO_XJAMP;                      //ガード情報
  ...
  i = ATTACKINFO_SWORD3;
  aif[i].hit =
      HITINFO_TATAKI1A | HITINFO_SIV3 | HITINFO_MARK4 | HITINFO_SNDHIT3 | HITINFO_EFCTSINDO; //ヒット情報
  ...
  i = ATTACKINFO_JUDGE;
  aif[i].hit =
      HITINFO_FUTTOBI2 | HITINFO_MARK4 | HITINFO_SNDHIT3 | HITINFO_EFCTSINDO | HITINFO_EFCTBURN; //ヒット情報
