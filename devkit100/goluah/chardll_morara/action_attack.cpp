...
void CCharacter::act_att_sb2() {
  if (ccounter == 5)
    StartAttack(ATTACKINFO_STAND_B2);
...
void CCharacter::act_att_sc2() {
  if (ccounter == 0)
    PlayMySound(wave_kakato);
  if (pdat->counter == 0) {
    pdat->vy = -12;
    pdat->vx = 5;
  } //初期速度
  if (ccounter == 5)
    StartAttack(ATTACKINFO_STAND_C2);
  ...
  if (ccounter > 3)
    JiyuuRakka(1, TRUE, ACTID_LANDING);
}
...
void CCharacter::act_att_sc3() {
  if (ccounter == 0)
    PlayMySound(wave_punch);
  if (ccounter == 5)
    StartAttack(ATTACKINFO_STAND_C3);
...
void CCharacter::act_nage() {
  ...
  if (pdat->counter < 10 + 10) {
    DrawFront();
    pdat->cnow = CELL_JA2;
  } else if (pdat->counter < 13 + 10) {
    pdat->cnow = CELL_CHIBA2;
  } else if (pdat->counter < 16 + 10) {
    pdat->cnow = CELL_CHIBA3;
  } else if (pdat->counter < 19 + 10) {
    pdat->cnow = CELL_CHIBA4;
  } else if (pdat->counter < 21 + 10) {
    pdat->cnow = CELL_CHIBA5;
  } else if (pdat->counter < 24 + 10) {
    pdat->cnow = CELL_CHIBA6;
  } else if (pdat->counter < 27 + 10) {
    pdat->cnow = CELL_CHIBA7;
  } else if (pdat->counter < 30 + 10) {
    pdat->cnow = CELL_CHIBA8;
  } else if (pdat->counter < 33 + 10) {
    pdat->cnow = CELL_CHIBA9;
  } else
...
void CCharacter::act_striker_atk1() {
  if (pdat->counter == 0) {
    ...
    if (efe_track) {
      efe_track->Startup(20.0f, 3, 5, PI * 0.25f, 0.8f, 0.12f, 80.0f, 0.00f, 80.0f);
    }
  }
  pdat->kasanari = FALSE;
  ...
  if (pdat->counter < 2)
    ...
    if (pdat->vy < 0)
      ...
      ChangeAction(ACTID_STRIKER_ATK2);
  }
...
void CCharacter::act_striker_atk2() {
  if (pdat->counter == 0) {
    ...
    if (efe_track) {
      efe_track->Startup(20.0f, 3, 8, PI * 0.25f, 0.8f, 0.05f, 120.0f, 0.00f, 50.0f, TRUE);
    }
  }
  pdat->kasanari = FALSE;
