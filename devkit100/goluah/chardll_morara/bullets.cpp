...
CChibasiri::CChibasiri(CCharacterBase *parent, BOOL ext) : CBulletBase(parent) { m_extended = ext; }

//設定
void CChibasiri::Startup(
    ATTACKINFO *patkinfo, DWORD hit_msg_id, DWORD grd_msg_id, BOOL muki, double x, double y, double vx) {
  atkinfo = *patkinfo;
...
void CMiniGunB::Setup(ATTACKINFO *patkinfo, DWORD hit_msg_id, DWORD grd_msg_id) {
  atkinfo = *patkinfo;
  SetHitMessage(hit_msg_id, 0);
  SetGuardMessage(hit_msg_id, 0);
}
...
void CMiniGunB::act_bomb() { Hide(); }

/****************************************************************
        ミニガン（エフェクト）
*****************************************************************/
CMiniGunE::CMiniGunE(CCharacterBase *parent) : CBulletBase(parent) { base_z = ZZAHYO_BULLET3; }
