...
DWORD CCharacter::Message(DWORD msg, LPVOID dat, DWORD prm) {
  ...
  switch (msg) {
  //飛道具ヒット・ガード
  case GOBJMSG_CHIBAHIT:
    AddPowerGauge(0.10);
    break;
  case GOBJMSG_CHIBAGUARD:
    AddPowerGauge(0.05);
    break;
...
BOOL CCharacter::Command_Hissatu(DWORD key) {
  ...
  } else                         //地上必殺技のコマンド判定
  {
    ...
    if (com623(30)) {
      if (key & KEYSTA_BC2) {
        pdat->aid = ACTID_GENOCIDE3G;
        return TRUE;
      } else if (key & KEYSTA_BB2) {
        pdat->aid = ACTID_GENOCIDE2G;
        return TRUE;
      } else if (key & KEYSTA_BA2) {
        pdat->aid = ACTID_GENOCIDE1G;
        return TRUE;
      }
    }
...
DWORD CCharacter::CommandCOM(DWORD wid) {
  if (wid == ACTID_MINIGUN && pdat->gauge < 1.0f)
...
DWORD CCharacter::Action() {
  ...
  case ACTID_ATT_SC3:
    act_att_sc3();
    ChainCombo(CHAIN_SC3);
    break;
  case ACTID_LANDING:
    act_landing();
    break;
  case ACTID_GENOCIDE1G:
    act_genocide1g();
    break;
  case ACTID_GENOCIDE1J:
    act_genocide1j();
    break;
  case ACTID_GENOCIDE2G:
    act_genocide2g();
    break;
  case ACTID_GENOCIDE2J:
    act_genocide2j();
    break;
  case ACTID_GENOCIDE3G:
    act_genocide3g();
    break;
  case ACTID_GENOCIDE3J:
    act_genocide3j();
  ...
  case ACTID_2ND_STEP:
    act_2nd_step();
    break;
  case ACTID_GENOCIDE_UP:
    act_genocide_up();
    break;
  case ACTID_GENOCIDE_DOWN:
    act_genocide_down();
    break;
  case ACTID_NAGE1:
    act_nage();
  ...
  case ACTID_STRIKER_IN:
    act_striker_in();
    break;
  case ACTID_STRIKER_ATK1:
    act_striker_atk1();
    break;
  case ACTID_STRIKER_ATK2:
    act_striker_atk2();
    break;
